...
type GatewayRoutingSuite struct {
	...
	ctx         context.Context
	client      *dbent.Client
	accountRepo *accountpostgres.AccountStore
}
...
func (s *GatewayRoutingSuite) SetupTest() {
	...
	tx := testEntTx(s.T())
	s.client = tx.Client()
	s.accountRepo = newAccountStoreContract(s.client, tx, nil)
}
...
func (s *GatewayRoutingSuite) TestListSchedulableByPlatforms_GeminiAndAntigravity() {
	// 创建各平台账户
	geminiAcc := mustCreateAccount(s.T(), s.client, &accountcore.Record{
		Name:        "gemini-oauth",
	...
	antigravityAcc := mustCreateAccount(s.T(), s.client, &accountcore.Record{
	...
	s.Require().True(platforms[capability.PlatformGemini], "应包含 gemini 账户")
	s.Require().True(platforms[capability.PlatformAntigravity], "应包含 antigravity 账户")
	s.Require().False(platforms[capability.PlatformAnthropic], "不应包含 anthropic 账户")
...
func (s *GatewayRoutingSuite) TestListSchedulableByGroupIDAndPlatforms_WithGroupBinding() {
	...
	// 创建账户
	boundAcc := mustCreateAccount(s.T(), s.client, &accountcore.Record{
		Name:        "bound-antigravity",
		Platform:    capability.PlatformAntigravity,
		Status:      billing.StatusActive,
		Schedulable: true,
	})
	unboundAcc := mustCreateAccount(s.T(), s.client, &accountcore.Record{
...
func (s *GatewayRoutingSuite) TestListSchedulableByPlatform_Antigravity() {
	...
	antigravity := mustCreateAccount(s.T(), s.client, &accountcore.Record{
		Name:        "antigravity-1",
		Platform:    capability.PlatformAntigravity,
		Status:      billing.StatusActive,
		Schedulable: true,
	...
	// 只查询 antigravity 平台
	accounts, err := s.accountRepo.ListSchedulableByPlatform(s.ctx, capability.PlatformAntigravity)

	s.Require().NoError(err)
	s.Require().Len(accounts, 1)
	s.Require().Equal(antigravity.ID, accounts[0].ID)
	s.Require().Equal(capability.PlatformAntigravity, accounts[0].Platform)
}

// TestSchedulableFilter_ExcludesInactive 验证不可调度账户被过滤
func (s *GatewayRoutingSuite) TestSchedulableFilter_ExcludesInactive() {
	// 创建可调度账户
	activeAcc := mustCreateAccount(s.T(), s.client, &accountcore.Record{
	...
	inactiveAcc := mustCreateAccount(s.T(), s.client, &accountcore.Record{
