...
func TestS06RefreshFailureLongerCooldownAndOutboxFailure(t *testing.T) {
	...
	row, err := client.Account.Create().SetName("refresh-failure-semantics").SetPlatform(account.PlatformOpenAI).SetType(account.AccountTypeOAuth).SetStatus(account.StatusActive).SetSchedulable(true).SetCredentials(map[string]any{}).SetTempUnschedulableUntil(until).SetTempUnschedulableReason("existing-longer").SetUpdatedAt(updatedAt).Save(ctx)
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, client.Account.DeleteOneID(row.ID).Exec(context.Background())) })
	before, err := client.Account.Get(ctx, row.ID)
	require.NoError(t, err)
