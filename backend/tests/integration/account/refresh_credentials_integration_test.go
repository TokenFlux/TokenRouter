...
func TestS06RefreshCredentialsCASAndOuterRollback(t *testing.T) {
	...
	row, err := client.Account.Create().SetName(fmt.Sprintf("s06-credential-%d", suffix)).SetPlatform(capability.PlatformOpenAI).SetType(capability.AccountTypeOAuth).SetCredentials(map[string]any{"refresh_token": "first"}).Save(ctx)
	...
	stale := readVersion()
	require.NoError(t, client.Account.UpdateOneID(row.ID).SetStatus(billing.StatusDisabled).Exec(ctx))
...
func TestS06RefreshCredentialsOutboxFailureRollsBack(t *testing.T) {
	...
	client := testEntClient(t)
	suffix := time.Now().UnixNano()
	row, err := client.Account.Create().SetName(fmt.Sprintf("s06-credential-outbox-%d", suffix)).SetPlatform(capability.PlatformGemini).SetType(capability.AccountTypeOAuth).SetCredentials(map[string]any{"refresh_token": "first"}).Save(ctx)
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, client.Account.DeleteOneID(row.ID).Exec(context.Background())) })
...
func (e *s06RefreshExchange) CanRefresh(a *accountcore.Record) bool {
	return a.Platform == capability.PlatformOpenAI && a.Type == capability.AccountTypeOAuth
}
func (e *s06RefreshExchange) NeedsRefresh(*accountcore.Record, time.Duration) bool { return true }
func (e *s06RefreshExchange) CacheKey(a *accountcore.Record) string {
	return fmt.Sprintf("s06-refresh:%d", a.ID)
}
func (e *s06RefreshExchange) Refresh(ctx context.Context, _ *accountcore.Record) (map[string]any, error) {
	e.calls++
	close(e.started)
	select {
	case <-e.release:
...
func TestS06RefreshPublicEntryPreservesConcurrentAdminCredentials(t *testing.T) {
	...
	defer cancel()
	client := testEntClient(t)
	row, err := client.Account.Create().SetName(fmt.Sprintf("s06-refresh-public-%d", time.Now().UnixNano())).SetPlatform(capability.PlatformOpenAI).SetType(capability.AccountTypeOAuth).SetCredentials(map[string]any{"refresh_token": "original"}).Save(ctx)
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, client.Account.DeleteOneID(row.ID).Exec(context.Background())) })
	repo := newAccountStoreContract(client, integrationDB, nil)
	...
	type result struct {
		value *accountcore.OAuthRefreshResult
		err   error
	}
	done := make(chan result, 1)
	go func() {
		v, e := accountcore.NewOAuthRefreshAPI(repo, nil, accountcore.RefreshOptions{Platform: accountcore.AccountRefreshPlatformPolicy()}).RefreshIfNeeded(ctx, original, executor, time.Minute)
		done <- result{v, e}
	}()
	select {
