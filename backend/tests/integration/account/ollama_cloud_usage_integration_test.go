...
func TestListDueOllamaCloudUsageAccountsOrderingLimitAndProxyHydration(t *testing.T) {
	...
	proxy := mustCreateProxy(t, tx.Client(), &egress.Proxy{
		...
		Username: "user", Password: "pass", Status: accountcore.StatusActive,
	})

	createAccount := func(name, baseURL string, proxyID *int64, snapshot map[string]any, lastUsed *time.Time) *accountcore.Record {
		t.Helper()
		extra := map[string]any{
			accountcore.OllamaCloudUsageSessionExtraKey:     "cipher:wos-session=fixture",
			accountcore.OllamaCloudUsageAutoRefreshExtraKey: true,
		}
		if snapshot != nil {
			extra[accountcore.OllamaCloudUsageSnapshotExtraKey] = snapshot
		}
		return mustCreateAccount(t, tx.Client(), &accountcore.Record{
			Name: name, Platform: capability.PlatformOpenAI, Type: capability.AccountTypeAPIKey,
...
//
//	TOKENROUTER_TEST_POSTGRES_IMAGE=postgres:15-alpine go test -tags integration ./tests/integration/account/
func TestListDueOllamaCloudUsageAccountsParsesAllRFC3339Precisions(t *testing.T) {
	...
	for name, fetchedAt := range notDue {
		_ = mustCreateAccount(t, tx.Client(), &accountcore.Record{
			Name: "ollama-precision-" + name, Platform: capability.PlatformOpenAI, Type: capability.AccountTypeAPIKey,
			Credentials: map[string]any{"api_key": "precision-" + name, "base_url": "https://ollama.com"},
			Extra: map[string]any{
				accountcore.OllamaCloudUsageSessionExtraKey:     "cipher:wos-session=fixture",
				accountcore.OllamaCloudUsageAutoRefreshExtraKey: true,
				accountcore.OllamaCloudUsageSnapshotExtraKey: map[string]any{
					"status":          accountcore.OllamaCloudUsageStatusOK,
	...
	due := mustCreateAccount(t, tx.Client(), &accountcore.Record{
		Name: "ollama-precision-due", Platform: capability.PlatformOpenAI, Type: capability.AccountTypeAPIKey,
...
func TestListDueOllamaCloudUsageAccountsUsesGroupMaxLastUsedAndFailsOpen(t *testing.T) {
	...
	leader := mustCreateAccount(t, tx.Client(), &accountcore.Record{
		Name: "ollama-group-leader", Platform: capability.PlatformOpenAI, Type: capability.AccountTypeAPIKey,
	...
	_ = mustCreateAccount(t, tx.Client(), &accountcore.Record{
		Name: "ollama-group-sibling", Platform: capability.PlatformAnthropic, Type: capability.AccountTypeAPIKey,
		Credentials: map[string]any{"api_key": "shared-key", "base_url": "https://www.ollama.com/v1"},
		LastUsedAt:  &newer,
	})
	invalid := mustCreateAccount(t, tx.Client(), &accountcore.Record{
		Name: "ollama-invalid-snapshot", Platform: capability.PlatformOpenAI, Type: capability.AccountTypeAPIKey,
		Credentials: map[string]any{"api_key": "invalid-key", "base_url": "https://ollama.com"},
		Extra: map[string]any{
			accountcore.OllamaCloudUsageSessionExtraKey:     "cipher:wos-session=fixture",
			accountcore.OllamaCloudUsageAutoRefreshExtraKey: true,
			accountcore.OllamaCloudUsageSnapshotExtraKey: map[string]any{
				"status": accountcore.OllamaCloudUsageStatusOK, "fetched_at": "2026-02-30T09:00:00.123456789Z",
			},
		},
	})
	idle := mustCreateAccount(t, tx.Client(), &accountcore.Record{
		Name: "ollama-idle-ok", Platform: capability.PlatformOpenAI, Type: capability.AccountTypeAPIKey,
...
func TestLockAndMergeAccountManagedExtraCoalescesNullableOllamaGroupIdentity(t *testing.T) {
	ctx := context.Background()
	tx := testEntTx(t)
	account := mustCreateAccount(t, tx.Client(), &accountcore.Record{
		Name: "ordinary-openai-without-base-url", Platform: capability.PlatformOpenAI, Type: capability.AccountTypeAPIKey,
		Credentials: map[string]any{"api_key": "sk-no-base-url"},
		Extra:       map[string]any{"custom": true},
	})
	loaded, err := newAccountStoreContract(tx.Client(), tx, nil).GetByID(ctx, account.ID)
	require.NoError(t, err)

	merged, err := newAccountStoreContract(tx.Client(), tx, nil).LockAndMergeAccountManagedExtra(ctx, tx.Client(), loaded)

	require.NoError(t, err, "a NULL Ollama eligibility expression must scan as false")
	require.NotContains(t, merged, accountcore.OllamaCloudUsageSessionExtraKey)
...
func TestOllamaCloudUsageGroupWritesAreAtomicAcrossPlatformsAndURLVariants(t *testing.T) {
	...
	tx := testEntTx(t)
	repo := newAccountStoreContract(tx.Client(), tx, nil)
	create := func(name, platform, apiKey, baseURL string) *accountcore.Record {
		t.Helper()
		return mustCreateAccount(t, tx.Client(), &accountcore.Record{
			Name: name, Platform: platform, Type: capability.AccountTypeAPIKey,
	...
	for _, id := range []int64{first.ID, second.ID} {
		...
		require.NoError(t, err)
		require.Equal(t, "cipher:shared", account.Extra[accountcore.OllamaCloudUsageSessionExtraKey])
	...
	require.NoError(t, err)
	require.NotContains(t, differentLoaded.Extra, accountcore.OllamaCloudUsageSessionExtraKey)
	...
	secondLoaded, err = repo.GetByID(ctx, second.ID)
	require.NoError(t, err)
	require.Equal(t, true, firstLoaded.Extra[accountcore.OllamaCloudUsageAutoRefreshExtraKey])
	require.Equal(t, true, secondLoaded.Extra[accountcore.OllamaCloudUsageAutoRefreshExtraKey])

	now := time.Now().UTC()
	snapshot := &accountcore.OllamaCloudUsageSnapshot{
		Status: accountcore.OllamaCloudUsageStatusOK, LastAttemptAt: now, NextRefreshAt: now.Add(time.Hour),
	}
	require.NoError(t, repo.UpdateOllamaCloudUsageSnapshot(ctx, firstLoaded, snapshot))
	secondLoaded, err = repo.GetByID(ctx, second.ID)
	require.NoError(t, err)
	storedSnapshot, ok := secondLoaded.Extra[accountcore.OllamaCloudUsageSnapshotExtraKey].(map[string]any)
	require.True(t, ok, "stored Ollama snapshot shape")
	require.Equal(t, accountcore.OllamaCloudUsageStatusOK, storedSnapshot["status"])
	...
	require.ErrorIs(t, repo.DisableOllamaCloudUsageAutoRefresh(ctx, staleSecond), accountcore.ErrOllamaCloudUsageIdentityChanged)
	firstLoaded, err = repo.GetByID(ctx, first.ID)
	require.NoError(t, err)
	secondLoaded, err = repo.GetByID(ctx, second.ID)
	require.NoError(t, err)
	require.Equal(t, "cipher:shared", firstLoaded.Extra[accountcore.OllamaCloudUsageSessionExtraKey])
	require.Equal(t, true, firstLoaded.Extra[accountcore.OllamaCloudUsageAutoRefreshExtraKey])
	require.NotContains(t, secondLoaded.Extra, accountcore.OllamaCloudUsageSessionExtraKey)
	require.NotContains(t, secondLoaded.Extra, accountcore.OllamaCloudUsageAutoRefreshExtraKey)

	require.NoError(t, repo.DeleteOllamaCloudUsageSession(ctx, firstLoaded))
	firstLoaded, err = repo.GetByID(ctx, first.ID)
	require.NoError(t, err)
	require.NotContains(t, firstLoaded.Extra, accountcore.OllamaCloudUsageSessionExtraKey)
}
...
func TestConcurrentOllamaCloudUsageSaveAndDeleteSerializeGroupState(t *testing.T) {
	...
	create := func(platform string) *accountcore.Record {
		t.Helper()
		return mustCreateAccount(t, client, &accountcore.Record{
			Name: fmt.Sprintf("%s-%s", apiKey, platform), Platform: platform, Type: capability.AccountTypeAPIKey,
			Credentials: map[string]any{"api_key": apiKey, "base_url": "https://ollama.com"},
			Extra: map[string]any{
				accountcore.OllamaCloudUsageSessionExtraKey:     "cipher:initial",
				accountcore.OllamaCloudUsageAutoRefreshExtraKey: true,
			},
		})
	}
	...
	secondLoaded, err := repo.GetByID(ctx, second.ID)
	require.NoError(t, err)
	managedState := func(account *accountcore.Record) map[string]any {
	...
	if len(firstState) > 0 {
		require.Equal(t, "cipher:replacement", firstState[accountcore.OllamaCloudUsageSessionExtraKey])
		require.Equal(t, true, firstState[accountcore.OllamaCloudUsageAutoRefreshExtraKey])
		require.NotContains(t, firstState, accountcore.OllamaCloudUsageSnapshotExtraKey)
	}
}

func accountIDs(accounts []accountcore.Record) []int64 {
...
func TestOllamaCloudUsageCredentialAndBulkUpdatesPreserveManagedStateOnlyWhenSafe(t *testing.T) {
	...
	repo := newAccountStoreContract(tx.Client(), tx, nil)
	now := time.Now().UTC()
	newAccount := func(name string) *accountcore.Record {
		return mustCreateAccount(t, tx.Client(), &accountcore.Record{
			Name: name, Platform: capability.PlatformOpenAI, Type: capability.AccountTypeAPIKey,
			...
			Extra: map[string]any{
				...
				accountcore.OllamaCloudUsageAutoRefreshExtraKey: true,
				accountcore.OllamaCloudUsageSnapshotExtraKey: map[string]any{
					"status": accountcore.OllamaCloudUsageStatusOK, "last_attempt_at": now, "next_refresh_at": now.Add(time.Hour),
				},
			},
		})
	}
	...
	require.NoError(t, err)
	require.NotContains(t, rawUpdated.Extra, accountcore.OllamaCloudUsageSessionExtraKey)
	require.NotContains(t, rawUpdated.Extra, accountcore.OllamaCloudUsageAutoRefreshExtraKey)
	require.NotContains(t, rawUpdated.Extra, accountcore.OllamaCloudUsageSnapshotExtraKey)

	bulkAccount := newAccount("ollama-bulk-credentials")
	rows, err := repo.BulkUpdate(ctx, []int64{bulkAccount.ID}, accountcore.AccountBulkUpdate{
	...
	require.NoError(t, err)
	require.Contains(t, bulkUnchanged.Extra, accountcore.OllamaCloudUsageSnapshotExtraKey)
	...
	require.NoError(t, err)
	require.NotContains(t, bulkIneligible.Extra, accountcore.OllamaCloudUsageSessionExtraKey)
	require.NotContains(t, bulkIneligible.Extra, accountcore.OllamaCloudUsageAutoRefreshExtraKey)
	require.NotContains(t, bulkIneligible.Extra, accountcore.OllamaCloudUsageSnapshotExtraKey)
}

func TestProxyIdentityUpdateInvalidatesOllamaSnapshotAndRejectsInFlightCAS(t *testing.T) {
	ctx := context.Background()
	tx := testEntTx(t)
	accountRepo := newAccountStoreContract(tx.Client(), tx, nil)
	proxyRepo := newProxyStoreContract(tx.Client(), tx)
	proxy := mustCreateProxy(t, tx.Client(), &egress.Proxy{
		Name: "ollama-identity-proxy", Protocol: "http", Host: "old.example", Port: 8080,
		Username: "old-user", Password: "old-pass", Status: accountcore.StatusActive,
	})
	now := time.Now().UTC()
	account := mustCreateAccount(t, tx.Client(), &accountcore.Record{
		Name: "ollama-proxy-account", Platform: capability.PlatformAnthropic, Type: capability.AccountTypeAPIKey,
	...
	require.NoError(t, err)
	require.NotContains(t, got.Extra, accountcore.OllamaCloudUsageSnapshotExtraKey)
	require.Equal(t, "cipher:wos-session=fixture", got.Extra[accountcore.OllamaCloudUsageSessionExtraKey])
	require.Equal(t, true, got.Extra[accountcore.OllamaCloudUsageAutoRefreshExtraKey])

	err = accountRepo.UpdateOllamaCloudUsageSnapshot(ctx, inFlight, &accountcore.OllamaCloudUsageSnapshot{
		Status: accountcore.OllamaCloudUsageStatusOK, LastAttemptAt: now, NextRefreshAt: now.Add(time.Hour),
	})
...
func TestUpdateCredentialsPreservesOllamaAndDiscardsDeprecatedExtra(t *testing.T) {
	...
	legacyAccount := mustCreateAccount(t, tx.Client(), &accountcore.Record{
		Name: "openai-legacy-extra", Platform: capability.PlatformOpenAI, Type: capability.AccountTypeAPIKey,
	...
	now := time.Now().UTC()
	ollamaAccount := mustCreateAccount(t, tx.Client(), &accountcore.Record{
		Name: "ollama-unchanged", Platform: capability.PlatformAnthropic, Type: capability.AccountTypeAPIKey,
	...
	require.NoError(t, err)
	require.Equal(t, "cipher:wos-session=fixture", ollamaLoaded.Extra[accountcore.OllamaCloudUsageSessionExtraKey])
	require.Equal(t, true, ollamaLoaded.Extra[accountcore.OllamaCloudUsageAutoRefreshExtraKey])
	require.Contains(t, ollamaLoaded.Extra, accountcore.OllamaCloudUsageSnapshotExtraKey)
	...
	ollamaLoaded, err = repo.GetByID(ctx, ollamaAccount.ID)
	require.NoError(t, err)
	require.NotContains(t, ollamaLoaded.Extra, accountcore.OllamaCloudUsageSessionExtraKey)
	require.NotContains(t, ollamaLoaded.Extra, accountcore.OllamaCloudUsageAutoRefreshExtraKey)
	require.NotContains(t, ollamaLoaded.Extra, accountcore.OllamaCloudUsageSnapshotExtraKey)
}

// TestListDueOllamaCloudUsageAccountsSQLDueRulesMatchService 验证 SQL 候选层会在 LIMIT 前
// 应用防抖、最大等待和失败退避规则，并与 account.OllamaCloudUsageIsAutoRefreshDue 保持一致；
...
func TestListDueOllamaCloudUsageAccountsSQLDueRulesMatchService(t *testing.T) {
	...
	createOK := func(name string, fetched, lastUsed time.Time) *accountcore.Record {
		t.Helper()
		return mustCreateAccount(t, tx.Client(), &accountcore.Record{
			Name: name, Platform: capability.PlatformOpenAI, Type: capability.AccountTypeAPIKey,
	...
	createFailed := func(name string, lastAttempt, lastUsed, nextRefresh time.Time, nextRefreshRaw string) *accountcore.Record {
		t.Helper()
		snapshot := map[string]any{
			"status":          accountcore.OllamaCloudUsageStatusFailed,
		...
		return mustCreateAccount(t, tx.Client(), &accountcore.Record{
			...
			Extra: map[string]any{
				accountcore.OllamaCloudUsageSessionExtraKey:     "cipher:wos-session=fixture",
				accountcore.OllamaCloudUsageAutoRefreshExtraKey: true,
				accountcore.OllamaCloudUsageSnapshotExtraKey:    snapshot,
			},
			LastUsedAt: &lastUsed,
		})
	}
	...
	// 以下夹具与 account.OllamaCloudUsageIsAutoRefreshDue 的语义一致；
