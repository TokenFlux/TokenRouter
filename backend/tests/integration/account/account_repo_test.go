...
func TestAccountsToService_LargeActiveAccountSetDoesNotExceedPostgresParameterLimit(t *testing.T) {
	...
	got, err := repo.RecordsFromEntities(context.Background(), accounts)
	require.NoError(t, err)
	require.Len(t, got, len(accounts))
}

func newParameterLimitAccountRepo(t *testing.T) *accountpostgres.AccountStore {
	t.Helper()
	...
	return newAccountStoreContract(client, nil, nil)
}
