...
type AccountRepoSuite struct {
	...
	ctx    context.Context
	client *dbent.Client
	repo   *accountpostgres.AccountStore
	sql    postgres.Executor
	cache  scheduler.SnapshotPublicationCache
}

type schedulerCacheRecorder struct {
	setAccounts []*accountcore.Record
	deleteIDs   []int64
	accounts    map[int64]*accountcore.Record
	setCtxErr   error
}

func (s *schedulerCacheRecorder) SetAccount(ctx context.Context, value scheduler.SnapshotAccount) error {
	account, err := codec.RecordValue(value)
	if err != nil {
		return err
	}
	s.setCtxErr = ctx.Err()
	...
	if s.accounts == nil {
		s.accounts = make(map[int64]*accountcore.Record)
	}
	if account != nil {
...
func (s *AccountRepoSuite) SetupTest() {
	...
	s.sql = tx
	s.cache = nil
	s.repo = newAccountStoreContract(s.client, tx, nil)
}
...
func (s *AccountRepoSuite) TestUpdate() {
	account := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "original"})
...
func (s *AccountRepoSuite) TestUpdate_SyncSchedulerSnapshotOnDisabled() {
	account := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "sync-update", Status: billing.StatusActive, Schedulable: true})
	cacheRecorder := &schedulerCacheRecorder{}
	s.cache = cacheRecorder
	s.repo.SetEvents(accountPublicationEvents(s.repo, cacheRecorder))
	...
	s.Require().Equal(billing.StatusDisabled, cacheRecorder.setAccounts[0].Status)
}

func (s *AccountRepoSuite) TestUpdate_SyncSchedulerSnapshotOnCredentialsChange() {
	account := mustCreateAccount(s.T(), s.client, &accountcore.Record{
...
func (s *AccountRepoSuite) TestUpdateCredentials_SyncsSnapshotAndDurableOutbox() {
	...
	err = postgres.ScanSingleRow(
		...
		[]any{scheduler.SchedulerOutboxEventAccountChanged, account.ID},
		&outboxCount,
	)
	s.Require().NoError(err)
...
func (s *AccountRepoSuite) TestDelete() {
	account := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "to-delete"})
...
func (s *AccountRepoSuite) TestDelete_RemovesSchedulerAccountSnapshot() {
	account := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "to-delete-cache"})
	cacheRecorder := &schedulerCacheRecorder{
		accounts: map[int64]*accountcore.Record{
			account.ID: {
				ID:          account.ID,
...
func (s *AccountRepoSuite) TestDelete_WithGroupBindings() {
	...
	account := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "acc-del"})
...
func (s *AccountRepoSuite) TestList() {
	mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "acc1"})
	mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "acc2"})
...
func (s *AccountRepoSuite) TestListOAuthRefreshCandidatePage_GrokCursorAndExclusions() {
	now := time.Now().UTC()
	valid1 := mustCreateAccount(s.T(), s.client, &accountcore.Record{
	...
	unschedulable := mustCreateAccount(s.T(), s.client, &accountcore.Record{
	...
	mustCreateAccount(s.T(), s.client, &accountcore.Record{
	...
	valid2 := mustCreateAccount(s.T(), s.client, &accountcore.Record{
	...
	valid3 := mustCreateAccount(s.T(), s.client, &accountcore.Record{
	...
	cooldown := mustCreateAccount(s.T(), s.client, &accountcore.Record{
	...
	options := accountcore.OAuthRefreshPageOptions{
...
func (s *AccountRepoSuite) TestListWithFilters() {
	tests := []struct {
		...
		privacyMode string
		wantCount   int
		validate    func(accounts []accountcore.Record)
	}{
		{
			name: "filter_by_platform",
			setup: func(client *dbent.Client) {
				mustCreateAccount(s.T(), client, &accountcore.Record{Name: "a1", Platform: capability.PlatformAnthropic})
				mustCreateAccount(s.T(), client, &accountcore.Record{Name: "a2", Platform: capability.PlatformOpenAI})
			},
			platform:  capability.PlatformOpenAI,
			wantCount: 1,
			validate: func(accounts []accountcore.Record) {
				s.Require().Equal(capability.PlatformOpenAI, accounts[0].Platform)
			},
		},
		{
			name: "filter_by_type",
			setup: func(client *dbent.Client) {
				mustCreateAccount(s.T(), client, &accountcore.Record{Name: "t1", Type: capability.AccountTypeOAuth})
				mustCreateAccount(s.T(), client, &accountcore.Record{Name: "t2", Type: capability.AccountTypeAPIKey})
			},
			accType:   capability.AccountTypeAPIKey,
			wantCount: 1,
			validate: func(accounts []accountcore.Record) {
				s.Require().Equal(capability.AccountTypeAPIKey, accounts[0].Type)
			},
		},
		{
			name: "filter_by_status",
			setup: func(client *dbent.Client) {
				mustCreateAccount(s.T(), client, &accountcore.Record{Name: "s1", Status: billing.StatusActive})
				mustCreateAccount(s.T(), client, &accountcore.Record{Name: "s2", Status: billing.StatusDisabled})
			},
			status:    billing.StatusDisabled,
			wantCount: 1,
			validate: func(accounts []accountcore.Record) {
				s.Require().Equal(billing.StatusDisabled, accounts[0].Status)
			},
		},
		{
			name: "filter_by_status_active_excludes_runtime_blocked_accounts",
			setup: func(client *dbent.Client) {
				mustCreateAccount(s.T(), client, &accountcore.Record{Name: "active-normal", Status: billing.StatusActive})
				rateLimited := mustCreateAccount(s.T(), client, &accountcore.Record{Name: "active-rate-limited", Status: billing.StatusActive})
				err := client.Account.UpdateOneID(rateLimited.ID).
					SetRateLimitResetAt(time.Now().Add(10 * time.Minute)).
					Exec(context.Background())
				s.Require().NoError(err)
				tempUnsched := mustCreateAccount(s.T(), client, &accountcore.Record{Name: "active-temp-unsched", Status: billing.StatusActive})
				err = client.Account.UpdateOneID(tempUnsched.ID).
					SetTempUnschedulableUntil(time.Now().Add(15 * time.Minute)).
					Exec(context.Background())
				s.Require().NoError(err)
				unsched := mustCreateAccount(s.T(), client, &accountcore.Record{Name: "active-unsched", Status: billing.StatusActive})
		...
		},
		{
			...
			setup: func(client *dbent.Client) {
				mustCreateAccount(s.T(), client, &accountcore.Record{Name: "active-normal", Status: billing.StatusActive, Schedulable: true})
				unsched := mustCreateAccount(s.T(), client, &accountcore.Record{Name: "active-unsched", Status: billing.StatusActive})
				err := client.Account.UpdateOneID(unsched.ID).
					SetSchedulable(false).
					Exec(context.Background())
		...
		},
		{
			...
			setup: func(client *dbent.Client) {
				tempUnsched := mustCreateAccount(s.T(), client, &accountcore.Record{Name: "active-temp-unsched", Status: billing.StatusActive, Schedulable: true})
				err := client.Account.UpdateOneID(tempUnsched.ID).
					SetTempUnschedulableUntil(time.Now().Add(15 * time.Minute)).
					Exec(context.Background())
				s.Require().NoError(err)
		...
		},
		{
			...
			setup: func(client *dbent.Client) {
				mustCreateAccount(s.T(), client, &accountcore.Record{Name: "alpha-account"})
				mustCreateAccount(s.T(), client, &accountcore.Record{Name: "beta-account"})
			},
			search:    "alpha",
			wantCount: 1,
			validate: func(accounts []accountcore.Record) {
		...
		},
		{
			...
			setup: func(client *dbent.Client) {
				...
				grouped := mustCreateAccount(s.T(), client, &accountcore.Record{Name: "grouped-account"})
				mustCreateAccount(s.T(), client, &accountcore.Record{Name: "ungrouped-account"})
		...
		},
		{
			...
			setup: func(client *dbent.Client) {
				mustCreateAccount(s.T(), client, &accountcore.Record{Name: "privacy-ok", Extra: map[string]any{"privacy_mode": openai.PrivacyModeTrainingOff}})
				mustCreateAccount(s.T(), client, &accountcore.Record{Name: "privacy-fail", Extra: map[string]any{"privacy_mode": openai.PrivacyModeFailed}})
			},
		...
		},
		{
			...
			setup: func(client *dbent.Client) {
				mustCreateAccount(s.T(), client, &accountcore.Record{Name: "privacy-unset", Extra: nil})
				mustCreateAccount(s.T(), client, &accountcore.Record{Name: "privacy-empty", Extra: map[string]any{"privacy_mode": ""}})
				mustCreateAccount(s.T(), client, &accountcore.Record{Name: "privacy-set", Extra: map[string]any{"privacy_mode": openai.PrivacyModeTrainingOff}})
			},
			privacyMode: accountcore.AccountPrivacyModeUnsetFilter,
			wantCount:   2,
			validate: func(accounts []accountcore.Record) {
	...
	for _, tt := range tests {
		s.Run(tt.name, func() {
			...
			tx := testEntTx(s.T())
			client := tx.Client()
			repo := newAccountStoreContract(client, tx, nil)
...
func (s *AccountRepoSuite) TestListByGroup() {
	...
	acc1 := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "a1", Status: billing.StatusActive, Priority: 2})
	acc2 := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "a2", Status: billing.StatusActive, Priority: 1})
...
func (s *AccountRepoSuite) TestListActive() {
	mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "active1", Status: billing.StatusActive})
	mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "inactive1", Status: billing.StatusDisabled})
...
func (s *AccountRepoSuite) TestListByPlatform() {
	mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "p1", Platform: capability.PlatformAnthropic, Status: billing.StatusActive})
	mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "p2", Platform: capability.PlatformOpenAI, Status: billing.StatusActive})

	accounts, err := s.repo.ListByPlatform(s.ctx, capability.PlatformAnthropic)
	s.Require().NoError(err, "ListByPlatform")
	s.Require().Len(accounts, 1)
	s.Require().Equal(capability.PlatformAnthropic, accounts[0].Platform)
}
...
func (s *AccountRepoSuite) TestGroupBinding_And_BindGroups() {
	...
	account := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "acc"})
...
func (s *AccountRepoSuite) TestBindGroups_EmptyList() {
	account := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "acc-empty"})
...
func (s *AccountRepoSuite) TestListSchedulable() {
	...
	okAcc := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "ok", Schedulable: true})
	mustBindAccountToGroup(s.T(), s.client, okAcc.ID, group.ID)

	future := now.Add(10 * time.Minute)
	overloaded := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "over", Schedulable: true, OverloadUntil: &future})
...
func (s *AccountRepoSuite) TestListSchedulableByGroupID_TimeBoundaries_And_StatusUpdates() {
	...
	rateLimited := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "rl", Schedulable: true})
...
func (s *AccountRepoSuite) TestListSchedulableCapacityByGroupIDs() {
	...
	shared := mustCreateAccount(s.T(), s.client, &accountcore.Record{
	...
	future := now.Add(10 * time.Minute)
	overloaded := mustCreateAccount(s.T(), s.client, &accountcore.Record{
...
func (s *AccountRepoSuite) TestListSchedulableByPlatform() {
	mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "a1", Platform: capability.PlatformAnthropic, Schedulable: true})
	mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "a2", Platform: capability.PlatformOpenAI, Schedulable: true})

	accounts, err := s.repo.ListSchedulableByPlatform(s.ctx, capability.PlatformAnthropic)
	s.Require().NoError(err)
	s.Require().Len(accounts, 1)
	s.Require().Equal(capability.PlatformAnthropic, accounts[0].Platform)
}
...
func (s *AccountRepoSuite) TestListSchedulableByGroupIDAndPlatform() {
	...
	a1 := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "a1", Platform: capability.PlatformAnthropic, Schedulable: true})
	a2 := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "a2", Platform: capability.PlatformOpenAI, Schedulable: true})
	mustBindAccountToGroup(s.T(), s.client, a1.ID, group.ID)
	mustBindAccountToGroup(s.T(), s.client, a2.ID, group.ID)

	accounts, err := s.repo.ListSchedulableByGroupIDAndPlatform(s.ctx, group.ID, capability.PlatformAnthropic)
	s.Require().NoError(err)
	s.Require().Len(accounts, 1)
...
func (s *AccountRepoSuite) TestSetSchedulable() {
	account := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "acc-sched", Schedulable: true})
	cacheRecorder := &schedulerCacheRecorder{}
	s.cache = cacheRecorder
	s.repo.SetEvents(accountPublicationEvents(s.repo, cacheRecorder))
...
func (s *AccountRepoSuite) TestBulkUpdate_SyncSchedulerSnapshotOnDisabled() {
	account1 := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "bulk-1", Status: billing.StatusActive, Schedulable: true})
	account2 := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "bulk-2", Status: billing.StatusActive, Schedulable: true})
	cacheRecorder := &schedulerCacheRecorder{}
	s.cache = cacheRecorder
	s.repo.SetEvents(accountPublicationEvents(s.repo, cacheRecorder))

	disabled := billing.StatusDisabled
	rows, err := s.repo.BulkUpdate(s.ctx, []int64{account1.ID, account2.ID}, accountcore.AccountBulkUpdate{
		Status: &disabled,
	})
	s.Require().NoError(err)
...
func (s *AccountRepoSuite) TestSetOverloaded() {
	account := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "acc-over"})
...
func (s *AccountRepoSuite) TestSetRateLimited() {
	account := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "acc-rl"})
...
func (s *AccountRepoSuite) TestSetRateLimitedIfLaterDoesNotShortenReset() {
	account := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "acc-rl-monotonic"})
...
func (s *AccountRepoSuite) TestClearRateLimitIfObservedProtectsRearmed429Generation() {
	...
	s.Require().NoError(err)
	s.Require().Equal(capability.AccountTypeAPIKey, retyped.Type)
...
func (s *AccountRepoSuite) TestClearRateLimit() {
	account := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "acc-clear"})
...
func (s *AccountRepoSuite) TestResetQuotaUsedAndClearRateLimitCooldownPreservesOtherRuntimeState() {
	...
	s.Require().NoError(newAccountUsageContract(s.sql, s.repo, s.cache).ResetQuotaUsedAndClearRateLimitCooldown(s.ctx, account.ID))
	...
	s.Require().NoError(postgres.ScanSingleRow(s.ctx, s.sql, `
		...
		)`, []any{scheduler.SchedulerOutboxEventAccountChanged, account.ID}, &pendingEventExists))
...
func (s *AccountRepoSuite) TestTempUnschedulableFieldsLoadedByGetByIDAndGetByIDs() {
	acc1 := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "acc-temp-1"})
	acc2 := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "acc-temp-2"})
...
func (s *AccountRepoSuite) TestSetTempUnschedulableSkipsOutboxWhenWindowDoesNotExtend() {
	account := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "acc-temp-noop"})
	cacheRecorder := &schedulerCacheRecorder{}
	s.cache = cacheRecorder
	s.repo.SetEvents(accountPublicationEvents(s.repo, cacheRecorder))
...
func (s *AccountRepoSuite) TestUpdateLastUsed() {
	account := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "acc-used"})
...
func (s *AccountRepoSuite) TestSetError() {
	account := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "acc-err", Status: billing.StatusActive, Schedulable: true})
	cacheRecorder := &schedulerCacheRecorder{}
	s.cache = cacheRecorder
	s.repo.SetEvents(accountPublicationEvents(s.repo, cacheRecorder))
	_, err := s.sql.ExecContext(s.ctx, "TRUNCATE scheduler_outbox")
	...
	got, err := s.repo.GetByID(s.ctx, account.ID)
	s.Require().NoError(err)
	s.Require().Equal(accountcore.StatusError, got.Status)
	s.Require().Equal("something went wrong", got.ErrorMessage)
	s.Require().False(got.Schedulable)
	s.Require().Len(cacheRecorder.setAccounts, 1)
	s.Require().Equal(account.ID, cacheRecorder.setAccounts[0].ID)
	s.Require().Equal(accountcore.StatusError, cacheRecorder.setAccounts[0].Status)
...
func (s *AccountRepoSuite) TestGrokOAuthConditionalMutation_DetachesBoundedSnapshotSync() {
	...
	cacheRecorder := &schedulerCacheRecorder{}
	repo := newAccountStoreContract(s.client, &cancelAfterAtomicMutationSQLExecutor{
		Executor: s.sql,
		cancel:   cancel,
	}, cacheRecorder)
...
func TestGrokOAuthConditionalMutationRollsBackWhenOutboxInsertFails(t *testing.T) {
	...
	repo := newAccountStoreContract(client, &failAtomicSchedulerOutboxSQLExecutor{Executor: integrationDB}, nil)
...
func (s *AccountRepoSuite) TestUpdateErrorStatusUnschedulesAccount() {
	account := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "acc-update-err", Status: billing.StatusActive, Schedulable: true})
...
func (s *AccountRepoSuite) TestClearError_SyncSchedulerSnapshotOnRecovery() {
	...
	s.Require().Len(cacheRecorder.setAccounts, 1)
	s.Require().Equal(account.ID, cacheRecorder.setAccounts[0].ID)
	s.Require().Equal(billing.StatusActive, cacheRecorder.setAccounts[0].Status)
}

// --- UpdateSessionWindow ---

func (s *AccountRepoSuite) TestUpdateSessionWindow() {
	account := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "acc-win"})
...
func (s *AccountRepoSuite) TestUpdateExtra_EmptyUpdates() {
	account := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "acc-extra-empty"})
	s.Require().NoError(s.repo.UpdateExtra(s.ctx, account.ID, map[string]any{}))
}

func (s *AccountRepoSuite) TestUpdateExtra_NilExtra() {
	account := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "acc-nil-extra", Extra: nil})
...
func (s *AccountRepoSuite) TestUpdateExtra_SchedulerNeutralSkipsOutboxAndSyncsFreshSnapshot() {
	...
	var outboxCount int
	s.Require().NoError(postgres.ScanSingleRow(s.ctx, s.sql, "SELECT COUNT(*) FROM scheduler_outbox", nil, &outboxCount))
	s.Require().Zero(outboxCount)
	s.Require().Len(cacheRecorder.setAccounts, 1)
	s.Require().NotNil(cacheRecorder.accounts[account.ID])
	s.Require().Equal(billing.StatusActive, cacheRecorder.accounts[account.ID].Status)
...
func (s *AccountRepoSuite) TestGetByCRSAccountID_ExcludesSparkShadow() {
	...
	parent := mustCreateAccount(s.T(), s.client, &accountcore.Record{
		Name: "crs-mother", Platform: capability.PlatformOpenAI, Type: capability.AccountTypeOAuth,
	})
	mustCreateAccount(s.T(), s.client, &accountcore.Record{
		Name: "crs-shadow", Platform: capability.PlatformOpenAI, Type: capability.AccountTypeOAuth,
...
func (s *AccountRepoSuite) TestListCRSAccountIDs_ExcludesSparkShadow() {
	parent := mustCreateAccount(s.T(), s.client, &accountcore.Record{
		Name: "crs-list-mother", Platform: capability.PlatformOpenAI, Type: capability.AccountTypeOAuth,
	})
	shadowCRSID := "crs-list-shadow-77"
	mustCreateAccount(s.T(), s.client, &accountcore.Record{
		Name: "crs-list-shadow", Platform: capability.PlatformOpenAI, Type: capability.AccountTypeOAuth,
		ParentAccountID: &parent.ID,
		QuotaDimension:  accountcore.QuotaDimensionSpark,
...
func (s *AccountRepoSuite) TestBulkUpdate() {
	a1 := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "bulk1", Priority: 1})
	a2 := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "bulk2", Priority: 1})

	newPriority := 99
	affected, err := s.repo.BulkUpdate(s.ctx, []int64{a1.ID, a2.ID}, accountcore.AccountBulkUpdate{
...
func (s *AccountRepoSuite) TestBulkUpdate_MergeCredentials() {
	a1 := mustCreateAccount(s.T(), s.client, &accountcore.Record{
		Name:        "bulk-cred",
		Credentials: map[string]any{"existing": "value"},
	})

	_, err := s.repo.BulkUpdate(s.ctx, []int64{a1.ID}, accountcore.AccountBulkUpdate{
...
func (s *AccountRepoSuite) TestBulkUpdate_EmptyIDs() {
	affected, err := s.repo.BulkUpdate(s.ctx, []int64{}, accountcore.AccountBulkUpdate{})
	s.Require().NoError(err)
	s.Require().Zero(affected)
}

func (s *AccountRepoSuite) TestBulkUpdate_EmptyUpdates() {
	a1 := mustCreateAccount(s.T(), s.client, &accountcore.Record{Name: "bulk-empty"})

	affected, err := s.repo.BulkUpdate(s.ctx, []int64{a1.ID}, accountcore.AccountBulkUpdate{})
	s.Require().NoError(err)
	s.Require().Zero(affected)
}

func idsOfAccounts(accounts []accountcore.Record) []int64 {
