...
func TestAccountRepository_ResetQuotaUsedAndClearRateLimitCooldown_NoRowsAffectedReturnsNotFoundWithoutOutbox(t *testing.T) {
	exec := &recordingSQLExecutor{result: rowsAffectedResult(0)}
	repo := billingpostgres.NewAccountUsageStore(exec, billingpostgres.AccountUsageOptions{})
...
func TestAccountRepository_GrokCredentialConditionalMutationsAreEligibleAndAtomicallyPropagated(t *testing.T) {
	...
	t.Run("permanent", func(t *testing.T) {
		...
		updated, err := repo.SetGrokCredentialErrorIfMatch(context.Background(), 42, snapshot, "revoked", string(forwardcore.GrokCredentialReasonProxyInvalid))
...
func TestAccountRepository_GrokCredentialCommitCarriesOutboxAcrossCallerCancellation(t *testing.T) {
	...
	tests := []struct {
		name   string
		mutate func(context.Context, *accountpostgres.AccountStore) (bool, error)
	}{
		{
			name: "permanent",
			mutate: func(ctx context.Context, repo *accountpostgres.AccountStore) (bool, error) {
				return repo.SetGrokCredentialErrorIfMatch(ctx, 42, snapshot, string(forwardcore.GrokCredentialReasonRevoked), string(forwardcore.GrokCredentialReasonProxyInvalid))
			},
		},
		{
			name: "transient",
...
func TestAccountRepository_ListOAuthRefreshCandidatePage_SQLFilter(t *testing.T) {
	...
	repo := newAccountStoreContract(nil, captureQuerySQL{db: db, captured: &capturedSQL, args: &capturedArgs}, nil)
