...
func ollamaCloudUsageRepositoryAccount() *accountcore.Record {
	return &accountcore.Record{
		ID: 17, Platform: capability.PlatformOpenAI, Type: capability.AccountTypeAPIKey,
		Credentials: map[string]any{"api_key": "key", "base_url": "https://ollama.com"},
		Extra: map[string]any{
			accountcore.OllamaCloudUsageSessionExtraKey:     "cipher:wos-session=secret",
...
func TestUpdateOllamaCloudUsageSnapshotRowsAffectedZeroIsIdentityConflict(t *testing.T) {
	...
	mock.ExpectRollback()
	repo := newAccountStoreContract(client, nil, nil)

	err := repo.UpdateOllamaCloudUsageSnapshot(context.Background(), ollamaCloudUsageRepositoryAccount(), &accountcore.OllamaCloudUsageSnapshot{
...
func TestOllamaCloudUsageManagedWriteRejectsChangedProxyIdentity(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)` + regexp.QuoteMeta("SELECT protocol, host, port") + `.*` + regexp.QuoteMeta("FOR SHARE")).
		...
		WillReturnRows(sqlmock.NewRows([]string{"protocol", "host", "port", "username", "password", "status"}).
			AddRow("http", "new.example", 3128, "user", "pass", accountcore.StatusActive))
	mock.ExpectRollback()
...
func TestSaveAndDeleteOllamaCloudUsageSessionKeepCiphertextOutOfSQL(t *testing.T) {
	...
	client := dbent.NewClient(dbent.Driver(entsql.OpenDB(dialect.Postgres, db)))
	t.Cleanup(func() { _ = client.Close() })
	repo := newAccountStoreContract(client, db, nil)
	account := ollamaCloudUsageRepositoryAccount()
	...
	account.Extra[accountcore.OllamaCloudUsageSessionExtraKey] = replacement
	mock.ExpectBegin()
...
func TestOllamaCloudBaseURLSQLRegexMatchesServiceSemantics(t *testing.T) {
	for _, baseURL := range []string{
		...
		t.Run(baseURL, func(t *testing.T) {
			matched, err := regexp.MatchString(accountpostgres.OllamaCloudBaseURLRegexSQL, baseURL)
			require.NoError(t, err)
			account := ollamaCloudUsageRepositoryAccount()
			account.Credentials["base_url"] = baseURL
			require.Equal(t, accountcore.IsOllamaCloudUsageAccount(account), matched)
		})
	}
}
...
func TestListOllamaCloudUsageGroupAccountsUsesOneStrictBatchQuery(t *testing.T) {
	...
	repo := newAccountStoreContract(nil, captureQuerySQL{db: db, captured: &capturedSQL}, nil)
	...
	accounts, err := repo.ListOllamaCloudUsageGroupAccounts(context.Background(), []*accountcore.Record{first, second})
	...
	require.Contains(t, query, accountpostgres.OllamaCloudBaseURLMatchesSQL("credentials ->> 'base_url'"))
...
func TestListDueOllamaCloudUsageAccountsFiltersOrdersAndLimits(t *testing.T) {
	...
	mock.ExpectQuery("WITH eligible AS").
		WithArgs(now.UTC(), debounce.Seconds(), maxWait.Seconds(), 20, accountcore.OllamaCloudUsageMinFetchInterval.Seconds()).
	...
	for _, clause := range []string{
		...
		"type = 'apikey'",
		accountpostgres.OllamaCloudBaseURLMatchesSQL("credentials ->> 'base_url'"),
...
func TestBulkUpdateOllamaIdentityCleanupIsValueConditional(t *testing.T) {
	...
	repo := newAccountStoreContract(nil, exec, nil)

	_, err := repo.BulkUpdate(context.Background(), []int64{17}, accountcore.AccountBulkUpdate{
	...
	require.Contains(t, query, "NOT ("+accountpostgres.OllamaCloudBaseURLMatchesSQL("credentials ->> 'base_url'"))
	require.Contains(t, query, accountpostgres.OllamaCloudBaseURLMatchesSQL("$1::jsonb ->> 'base_url'"))
	require.NotContains(t, query, "~*")
	...
	require.True(t, ok)
	require.NotContains(t, string(payload), accountcore.OllamaCloudUsageSnapshotExtraKey)
}
...
func TestUpdateCredentialsIdentityChangeClearsAllOllamaManagedExtra(t *testing.T) {
	...
	mock.ExpectExec(regexp.QuoteMeta("INSERT INTO scheduler_outbox")).
		WithArgs(scheduler.SchedulerOutboxEventAccountChanged, int64(17), nil, nil, sqlmock.AnyArg()).
