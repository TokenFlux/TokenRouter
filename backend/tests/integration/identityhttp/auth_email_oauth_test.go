...
func TestEmailOAuthCallbackRequiresPendingRegistrationWhenInvitationEnabled(t *testing.T) {
	...
	req.AddCookie(&http.Cookie{Name: identityhttp.EmailOAuthStateCookieName, Value: identityhttp.EncodeCookieValue(state)})
	req.AddCookie(&http.Cookie{Name: identityhttp.EmailOAuthRedirectCookie, Value: identityhttp.EncodeCookieValue("/dashboard")})
	req.AddCookie(&http.Cookie{Name: identityhttp.EmailOAuthProviderCookie, Value: identityhttp.EncodeCookieValue("github")})
	c.Request = req

	profile := &identityprovider.EmailOAuthProfile{
		Subject:       "github-123",
	...
	handler.Email.EmailOAuthCallbackWithProfile(c, "github", identity.EmailOAuthOptions{
		Enabled:             true,
	...
	completion, ok := identityhttp.ReadCompletionResponse(session.LocalFlowState)
	require.True(t, ok)
...
func TestEmailOAuthCallbackExistingEmailLogsInWhenInvitationEnabled(t *testing.T) {
	...
	handler.Email.EmailOAuthCallbackWithProfile(c, "google", identity.EmailOAuthOptions{
		Enabled:             true,
		ClientID:            "google-client",
		ClientSecret:        "google-secret",
		RedirectURL:         "https://app.example/api/v1/auth/oauth/google/callback",
		FrontendRedirectURL: "/auth/oauth/callback",
	}, "/auth/oauth/callback", "/dashboard", &identityprovider.EmailOAuthProfile{
		Subject:       "google-123",
...
func TestEmailOAuthCallbackCreatesPasswordRegistrationSessionWithAffiliateCode(t *testing.T) {
	...
	req.AddCookie(&http.Cookie{Name: identityhttp.EmailOAuthAffCookie, Value: identityhttp.EncodeCookieValue(" AFF123 ")})
	c.Request = req
...
func TestEmailOAuthStartAcceptsAffCodeQueryAlias(t *testing.T) {
	handler, _ := newOAuthPendingFlowTestHandlerWithDependencies(t, oauthPendingFlowTestHandlerOptions{
		settingValues: map[string]string{
			identity.SettingKeyGitHubOAuthEnabled:             "true",
			identity.SettingKeyGitHubOAuthClientID:            "github-client",
			identity.SettingKeyGitHubOAuthClientSecret:        "github-secret",
			identity.SettingKeyGitHubOAuthRedirectURL:         "https://api.example/api/v1/auth/oauth/github/callback",
			identity.SettingKeyGitHubOAuthFrontendRedirectURL: "/auth/oauth/callback",
		},
	})
	...
	require.Equal(t, http.StatusFound, recorder.Code)
	affCookie := findCookie(recorder.Result().Cookies(), identityhttp.EmailOAuthAffCookie)
	require.NotNil(t, affCookie)
	require.Equal(t, identityhttp.EncodeCookieValue("AFFALIAS"), affCookie.Value)
}
...
func TestEmailOAuthStartPreservesPromoCodeInPendingSession(t *testing.T) {
	...
	promoCookie := findCookie(startRecorder.Result().Cookies(), identityhttp.OauthPromoCodeCookieName)
	...
	handler.Email.EmailOAuthCallbackWithProfile(callbackCtx, "github", identity.EmailOAuthOptions{
		Enabled:             true,
		ClientID:            "github-client",
		ClientSecret:        "github-secret",
	...
	session, err := client.PendingAuthSession.Query().Only(ctx)
	require.NoError(t, err)
	require.Equal(t, "WELCOME2024", identityhttp.PendingOAuthPromoCode(identitypostgres.PendingAuthSessionFromEntity(session)))
}
...
func TestCompleteEmailOAuthRegistrationUsesAffiliateCodeFromPendingSession(t *testing.T) {
	...
	session, err := client.PendingAuthSession.Create().
		...
		SetIntent(identityhttp.OauthIntentLogin).
	...
	req.AddCookie(&http.Cookie{Name: identityhttp.OauthPendingBrowserCookieName, Value: identityhttp.EncodeCookieValue("browser-ref-key")})
	c.Request = req

	handler.Email.CompleteEmailOAuthRegistration(c, "google")
...
func TestCompleteEmailOAuthRegistrationRequiresPassword(t *testing.T) {
	...
	req.Header.Set("Content-Type", "application/json")
	req.AddCookie(&http.Cookie{Name: identityhttp.OauthPendingSessionCookieName, Value: identityhttp.EncodeCookieValue(session.SessionToken)})
	req.AddCookie(&http.Cookie{Name: identityhttp.OauthPendingBrowserCookieName, Value: identityhttp.EncodeCookieValue("browser-password-key")})
	c.Request = req

	handler.Email.CompleteEmailOAuthRegistration(c, "github")
