...
func quotaCoordinationFixture(t *testing.T) (int64, billing.UserPlatformQuotaRepository, billing.BillingCache, billing.BalanceReader, *redis.Client) {
	t.Helper()
	...
	integrationRedis := rediscontainer.New(t)
	...
	return id, repository, cache, quotaUsersForContract{postgres.NewUserStore(client, integrationDB)}, integrationRedis
}

// TestS04QuotaResetWaitsForFlusher 验证单实例中旧快照写回与管理重置不能交错。
func TestS04QuotaResetWaitsForFlusher(t *testing.T) {
	id, repository, cache, users, _ := quotaCoordinationFixture(t)
...
func TestS04QuotaLockCancellationReaddsDirty(t *testing.T) {
	id, repository, cache, _, integrationRedis := quotaCoordinationFixture(t)
	coordinator := billing.NewQuotaCoordinator()
