...
func TestS05AdminDeleteRollsBackKeyParticipant(t *testing.T) {
	...
	before := s05OutboxCount(t, integrationDB, ctx, key.Key)
	...
	require.Equal(t, before, s05OutboxCount(t, integrationDB, ctx, key.Key), "回滚不能留下成功失效事件")
...
func TestS05GroupReplacementRollsBackKeyParticipant(t *testing.T) {
	...
	require.Equal(t, before, s05OutboxCount(t, integrationDB, ctx, key.Key))
	mutations.KeysInTx = func(tx *dbent.Tx) identity.AdminKeyParticipant { return keys.LifecycleInTx(tx) }
...
func TestS05AdminKeyGrantRollsBackIdentityParticipant(t *testing.T) {
	...
	before := s05OutboxCount(t, integrationDB, ctx, old.Key)
	...
	require.Equal(t, before, s05OutboxCount(t, integrationDB, ctx, old.Key))
	mutations.Keys = keys
...
func s05OutboxCount(t *testing.T, integrationDB *sql.DB, ctx context.Context, key string) int {
	t.Helper()
	var n int
