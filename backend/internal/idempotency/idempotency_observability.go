...
func logIdempotencyAudit(endpoint, scope, keyHash, stateTransition string, replayed bool, attrs map[string]string) {
	...
	observe("service.idempotency", b.String())
}
...
func SetObserver(next Observer) func() {
	observerState.Lock()
	previous := observerState.value
	observerState.value = next
	observerState.Unlock()
	return func() { observerState.Lock(); observerState.value = previous; observerState.Unlock() }
}
func observe(component, message string) {
	observerState.RLock()
	o := observerState.value
	observerState.RUnlock()
	if o != nil {
		o.Observe(component, message)
	}
}
