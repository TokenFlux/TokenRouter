...
func TestS06ConfigurationTransactionAndOutbox(t *testing.T) {
	for _, mode := range []string{"outer_rollback", "outbox_failure"} {
		t.Run(mode, func(t *testing.T) {
			...
			row, err := client.Account.Create().SetName("before-config").SetPlatform(service.PlatformOpenAI).
				SetType(service.AccountTypeAPIKey).SetExtra(map[string]any{"quota_used": 2.0}).Save(ctx)
			require.NoError(t, err)
			t.Cleanup(func() { require.NoError(t, client.Account.DeleteOneID(row.ID).Exec(context.Background())) })
			store := accountpostgres.NewAccountStore(client, integrationDB, accountpostgres.AccountStoreOptions{OllamaIdentity: acctcore.IsOllamaCloudUsageAccount, Events: AccountEventBinding{}})
			var tx *dbent.Tx
