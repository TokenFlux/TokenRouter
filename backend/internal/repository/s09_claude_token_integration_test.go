...
func TestS09ClaudeTokenRefreshUsesOriginalCAS(t *testing.T) {
	for _, adminChange := range []bool{false, true} {
		t.Run(fmt.Sprintf("administrator=%v", adminChange), func(t *testing.T) {
			ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
			defer cancel()
			client := testEntClient(t)
			row, err := client.Account.Create().SetName(fmt.Sprintf("s09-claude-%d", time.Now().UnixNano())).SetPlatform(capability.PlatformAnthropic).SetType(capability.AccountTypeOAuth).SetCredentials(map[string]any{"access_token": "expired", "refresh_token": "original", "expires_at": time.Now().Add(-time.Hour).Unix()}).Save(ctx)
			...
			native := anthropic.NewOAuthClient()
			native.TokenURL = server.URL
			oauth := account.NewClaudeAuthorization(native, accountprovider.ClaudeAuthorizationOptions(nil))
			...
			executor := &account.ClaudeTokenRefresher{Authorization: oauth}
			provider := &account.ClaudeTokenSource{Options: account.ClaudeTokenOptions{
				Repository: store, Cache: cache, Policy: account.ClaudeProviderRefreshPolicy(),
			...
			cached, err := cache.GetAccessToken(ctx, account.ClaudeTokenCacheKey(service.AccountRecordView(value)))
			require.NoError(t, err)
