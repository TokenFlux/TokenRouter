...
func TestS09ClaudeTokenRefreshUsesOriginalCAS(t *testing.T) {
	for _, adminChange := range []bool{false, true} {
		t.Run(fmt.Sprintf("administrator=%v", adminChange), func(t *testing.T) {
			...
			row, err := client.Account.Create().SetName(fmt.Sprintf("s09-claude-%d", time.Now().UnixNano())).SetPlatform(service.PlatformAnthropic).SetType(service.AccountTypeOAuth).SetCredentials(map[string]any{"access_token": "expired", "refresh_token": "original", "expires_at": time.Now().Add(-time.Hour).Unix()}).Save(ctx)
			...
			oauth := service.NewOAuthService(nil, native)
			defer oauth.Stop()
			cache := NewGeminiTokenCache(testRedis(t))
			provider := service.NewClaudeTokenProvider(repo, cache, oauth)
			provider.SetRefreshAPI(service.NewOAuthRefreshAPI(repo, cache), service.NewClaudeTokenRefresher(oauth))
			type result struct {
				token string
				err   error
			}
			...
			cached, err := cache.GetAccessToken(ctx, service.ClaudeTokenCacheKey(value))
			require.NoError(t, err)
