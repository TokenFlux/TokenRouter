...
func s12RefundRefund(t *testing.T, status string) (*dbent.Client, *service.PaymentService, *dbent.PaymentOrder) {
	t.Helper()
	...
	s := service.NewPaymentService(c, payment.NewRegistry(), payment.NewDefaultLoadBalancer(paymentpostgres.NewInstanceStore(c), nil), nil, nil, nil, NewUserRepository(c, integrationDB), nil, nil)
	return c, s, o
}
...
func TestS12StaleRefundFailureOverwritesSuccess(t *testing.T) {
	...
	defer cancel()
	c, s, o := s12RefundRefund(t, service.OrderStatusRefundPending)
	...
	require.Equal(t, service.OrderStatusRefunded, current.Status, "迟到失败不得覆盖已完成退款")
}
func TestS12PendingRefundAuditLossSkipsDeduction(t *testing.T) {
	ctx := context.Background()
	c, s, o := s12RefundRefund(t, service.OrderStatusCompleted)
	s12RefundStripe(t, func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
	...
	require.Equal(t, service.OrderStatusRefunding, cur.Status)
...
func TestS12RefundPreparationAuditFailureStopsChannel(t *testing.T) {
	...
	assertRefundPostgresState(t, ctx, c, o.UserID, o.ID, 100, service.OrderStatusCompleted, 0)
...
func TestS12RefundPreparedRecoveryDoesNotRepeatChannelOrDeduction(t *testing.T) {
	...
	require.Nil(t, result)
	require.Error(t, err)
	assertRefundPostgresState(t, ctx, c, o.UserID, o.ID, 50, service.OrderStatusRefunding, 0)
	...
	require.NoError(t, err)
	require.True(t, result.Success)
	assertRefundPostgresState(t, ctx, c, o.UserID, o.ID, 50, service.OrderStatusRefunded, 1)
...
func TestS12RefundMissingRecoveryRequiresManualVerification(t *testing.T) {
	for _, status := range []string{service.OrderStatusRefundPending, service.OrderStatusRefunding} {
