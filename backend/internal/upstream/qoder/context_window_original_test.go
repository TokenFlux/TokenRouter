...
func TestQoderHighestContextFlowsThroughAllProtocols(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			payload, modelKey := qoder.BuildQoderPayloadWithOptions(request, "", request.Messages, true, true)
...
func TestQoderContextCapabilityUsesSiteAndRouteFallback(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			payload, _, err := qoder.BuildQoderPayloadFromChatCompletionsForSite([]byte(`{"model":"`+tt.model+`","messages":[{"role":"user","content":"hello"}]}`), "personal_standard", tt.site)
			require.NoError(t, err)
...
func TestQoderContextCapabilityMergesWithThinkingOverride(t *testing.T) {
	payload, _, err := qoder.BuildQoderPayloadFromChatCompletionsForSite([]byte(`{
