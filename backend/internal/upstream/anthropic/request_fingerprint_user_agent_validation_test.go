...
func TestIsAcceptableFingerprintUserAgent(t *testing.T) {
	...
	for _, testCase := range cases {
		t.Run(testCase.name, func(t *testing.T) {
			require.Equal(t, testCase.want, claude.IsAcceptableFingerprintUserAgent(testCase.userAgent))
		})
	}
}
...
func TestGetOrCreateFingerprintRejectsMalformedUserAgentOnCreate(t *testing.T) {
	...
	service := claude.NewRequestFingerprint(cache)
	...
	require.Equal(t, claude.DefaultFingerprint.UserAgent, fingerprint.UserAgent)
...
func TestGetOrCreateFingerprintRejectsSentinelVersionOnUpgrade(t *testing.T) {
	cache := &stubIdentityCache{fingerprint: &claude.Fingerprint{
...
func TestDefaultFingerprintUserAgentIsAcceptable(t *testing.T) {
	require.True(t, claude.IsAcceptableFingerprintUserAgent(claude.DefaultFingerprint.UserAgent))
}
