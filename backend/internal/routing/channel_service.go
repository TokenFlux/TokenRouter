...
func NewChannelService(repo ChannelRepository, authCacheInvalidator GroupAuthInvalidator, options ...ChannelOptions) *ChannelService {
	optionsValue := ChannelOptions{Now: time.Now}
	if len(options) > 0 {
	...
	s := &ChannelService{
		options: optionsValue, validation: ChannelValidation{LoadLocation: optionsValue.LoadLocation},
		repo:                 repo,
...
func (s *ChannelService) loadCache(ctx context.Context) (*channelCache, error) {
	if cached, ok := s.cache.Load().(*channelCache); ok && cached != nil {
		if s.options.Now().Sub(cached.loadedAt) < channelCacheTTL {
			return cached, nil
		}
	}
...
func (s *ChannelService) storeErrorCache() {
	...
	errorCache.loadedAt = s.options.Now().Add(-(channelCacheTTL - channelErrorTTL))
...
func (s *ChannelService) fetchChannelData(ctx context.Context) ([]Channel, map[int64]string, error) {
	...
	if err != nil {
		s.warn("failed to build channel cache", "error", err)
		s.storeErrorCache()
	...
	if len(allGroupIDs) > 0 {
		...
		if err != nil {
			s.warn("failed to load group platforms for channel cache", "error", err)
			s.storeErrorCache()
...
func (s *ChannelService) invalidateCache() {
	...
	if _, err := s.buildCache(context.Background()); err != nil {
		s.warn("failed to rebuild channel cache after invalidation", "error", err)
	}
}
...
func (s *ChannelService) GetChannelModelPricing(ctx context.Context, groupID int64, model string) *ChannelModelPricing {
	...
	if err != nil {
		s.warn("failed to load channel cache", "group_id", groupID, "error", err)
		return nil
	}
	if lk == nil {
		return nil
...
func (s *ChannelService) ResolveChannelMapping(ctx context.Context, groupID int64, model string) ChannelMappingResult {
	lk, err := s.lookupGroupChannel(ctx, groupID)
	if err != nil {
		s.warn("failed to load channel cache for mapping", "group_id", groupID, "error", err)
	}
	if lk == nil {
...
func (s *ChannelService) IsModelRestricted(ctx context.Context, groupID int64, model string) bool {
	lk, err := s.lookupGroupChannel(ctx, groupID)
	if err != nil {
		s.warn("failed to load channel cache for model restriction check", "group_id", groupID, "error", err)
	}
	if lk == nil {
		return false
	}
...
func (v ChannelValidation) ChannelConfig(pricing []ChannelModelPricing, mapping map[string]map[string]string) error {
	if err := v.PricingEntries(pricing); err != nil {
		return err
	}
...
func (v ChannelValidation) PricingEntries(pricing []ChannelModelPricing) error {
...
func (v ChannelValidation) PricingTime(pricing []ChannelModelPricing) error {
	for i := range pricing {
...
func (v ChannelValidation) AccountStatsPricing(pricing []ChannelModelPricing) error {
	for _, p := range pricing {
		if p.FastModeMultiplier != nil {
			return infraerrors.BadRequest(
...
func (s *ChannelService) Create(ctx context.Context, input *CreateChannelInput) (*Channel, error) {
	...
	if err := s.validation.ChannelConfig(channel.ModelPricing, channel.ModelMapping); err != nil {
		return nil, err
	}
...
func (s *ChannelService) getOldGroupIDs(ctx context.Context, channelID int64) []int64 {
	...
	if err != nil {
		s.warn("failed to get old group IDs for cache invalidation", "channel_id", channelID, "error", err)
	}
	return oldGroupIDs
}
...
func (s *ChannelService) Delete(ctx context.Context, id int64) error {
	...
	if err != nil {
		s.warn("failed to get group IDs before delete", "channel_id", id, "error", err)
	}
...
func (v ChannelValidation) TimePricing(config *ChannelTimePricing) error {
...
func (s *ChannelService) warn(message string, args ...any) {
