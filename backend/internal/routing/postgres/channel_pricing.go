...
func (r *ChannelStore) CreateModelPricing(ctx context.Context, pricing *routing.ChannelModelPricing) error {
	return createModelPricingExec(ctx, r.db, pricing)
}

func (r *ChannelStore) UpdateModelPricing(ctx context.Context, pricing *routing.ChannelModelPricing) error {
...
func (r *ChannelStore) DeleteModelPricing(ctx context.Context, id int64) error {
...
func (r *ChannelStore) ReplaceModelPricing(ctx context.Context, channelID int64, pricingList []routing.ChannelModelPricing) error {
...
func (r *ChannelStore) batchLoadModelPricing(ctx context.Context, channelIDs []int64) (map[int64][]routing.ChannelModelPricing, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, channel_id, platform, models, billing_mode, price_multiplier, fast_mode_multiplier, fast_multiplier, flex_multiplier, max_reasoning_effort_multiplier, input_price, output_price, cache_write_price, cache_write_1h_price, cache_read_price, image_input_price, image_output_price, per_request_price, time_pricing, created_at, updated_at
	...
	// 按 channelID 分组
	pricingMap := make(map[int64][]routing.ChannelModelPricing, len(channelIDs))
...
func (r *ChannelStore) batchLoadIntervals(ctx context.Context, pricingIDs []int64) (map[int64][]routing.PricingInterval, error) {
	rows, err := r.db.QueryContext(ctx,
...
func scanModelPricingRows(rows *sql.Rows) ([]routing.ChannelModelPricing, []int64, error) {
	var result []routing.ChannelModelPricing
	var pricingIDs []int64
	for rows.Next() {
		var p routing.ChannelModelPricing
		var modelsJSON []byte
...
func createModelPricingExec(ctx context.Context, exec dbExec, pricing *routing.ChannelModelPricing) error {
	modelsJSON, err := json.Marshal(pricing.Models)
	if err != nil {
		return fmt.Errorf("marshal models: %w", err)
	}
...
func marshalChannelTimePricing(config *routing.ChannelTimePricing) (any, error) {
...
func unmarshalChannelTimePricing(data []byte) (*routing.ChannelTimePricing, error) {
	if len(data) == 0 {
		return nil, nil
	}
	var config routing.ChannelTimePricing
...
func replaceModelPricingTx(ctx context.Context, exec dbExec, channelID int64, pricingList []routing.ChannelModelPricing) error {
