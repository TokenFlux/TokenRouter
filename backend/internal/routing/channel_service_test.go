...
func newTestChannelService(repo *mockChannelRepository) *ChannelService {
	return NewChannelService(repo, nil, ChannelOptions{LoadLocation: time.LoadLocation})
}

func newTestChannelServiceWithAuth(repo *mockChannelRepository, auth *mockChannelAuthCacheInvalidator) *ChannelService {
	return NewChannelService(repo, auth, ChannelOptions{LoadLocation: time.LoadLocation})
}
...
func TestValidateAccountStatsPricingEntries_RejectsFastModeMultiplier(t *testing.T) {
	err := (ChannelValidation{LoadLocation: time.LoadLocation}).AccountStatsPricing([]ChannelModelPricing{{
		Platform:           PlatformOpenAI,
		BillingMode:        BillingModeToken,
		FastModeMultiplier: testPtrFloat64(2),
		InputPrice:         testPtrFloat64(0.01),
...
func TestValidatePricingEntries_RejectsTimePricingForNonTokenMode(t *testing.T) {
	err := (ChannelValidation{LoadLocation: time.LoadLocation}).PricingEntries([]ChannelModelPricing{{
		Platform:        PlatformOpenAI,
		BillingMode:     BillingModePerRequest,
...
func TestNormalizeGroupModelPricingAcceptsTimePricing(t *testing.T) {
	_, err := (ChannelValidation{LoadLocation: time.LoadLocation}).NormalizeGroupPricing(PlatformOpenAI, []ChannelModelPricing{{
		Models: []string{"gpt-5"},
		TimePricing: &ChannelTimePricing{
			Timezone: "Asia/Shanghai",
			Periods:  []ChannelTimePricingPeriod{{StartTime: "09:00", EndTime: "12:00", Multiplier: 2}},
