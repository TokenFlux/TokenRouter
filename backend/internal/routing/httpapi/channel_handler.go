...
func NewChannelHandler(channels *routing.ChannelService, catalog *routing.ChannelCatalog) *ChannelHandler {
	return &ChannelHandler{channelService: channels, catalog: catalog}
}

// ChannelHandler 处理管理员渠道管理请求
type ChannelHandler struct {
	channelService *routing.ChannelService
	catalog        *routing.ChannelCatalog
}
...
func channelToResponse(ch *routing.Channel) *channelResponse {
	if ch == nil {
		return nil
	}
	...
	if resp.BillingModelSource == "" {
		resp.BillingModelSource = routing.BillingModelSourceChannelMapped
	}
...
func pricingToResponse(p *routing.ChannelModelPricing) channelModelPricingResponse {
	models := p.Models
	if models == nil {
		models = []string{}
	}
...
func timePricingToResponse(value *routing.ChannelTimePricing) *channelTimePricingResponse {
	if value == nil {
		return nil
	}
...
func pricingRequestToService(reqs []channelModelPricingRequest) []routing.ChannelModelPricing {
	result := make([]routing.ChannelModelPricing, 0, len(reqs))
	for _, r := range reqs {
		...
		result = append(result, routing.ChannelModelPricing{
			Platform:                     platform,
			Models:                       r.Models,
			BillingMode:                  billingMode,
...
func timePricingRequestToService(value *channelTimePricingRequest) *routing.ChannelTimePricing {
	if value == nil {
		return nil
	}
	periods := make([]routing.ChannelTimePricingPeriod, 0, len(value.Periods))
	for _, period := range value.Periods {
		periods = append(periods, routing.ChannelTimePricingPeriod{
			StartTime:  period.StartTime,
			EndTime:    period.EndTime,
			Multiplier: period.Multiplier,
		})
	}
	return &routing.ChannelTimePricing{
		Timezone:     value.Timezone,
		WeekdaysOnly: value.WeekdaysOnly,
		Periods:      periods,
	}
...
func (h *ChannelHandler) Create(c *gin.Context) {
	...
	channel, err := h.channelService.Create(c.Request.Context(), &routing.CreateChannelInput{
		Name:                       req.Name,
...
func (h *ChannelHandler) Update(c *gin.Context) {
	...
	input := &routing.UpdateChannelInput{
		Name:                       req.Name,
		Description:                req.Description,
		Status:                     req.Status,
		GroupIDs:                   req.GroupIDs,
