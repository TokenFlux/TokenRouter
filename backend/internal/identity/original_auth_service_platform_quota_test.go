...
func (f *fakeInsertRecorder) GetByUserPlatform(_ context.Context, _ int64, _ string) (*billing.UserPlatformQuotaRecord, error) {
	return nil, nil
}

func (f *fakeInsertRecorder) BulkInsertInitial(ctx context.Context, recs []billing.UserPlatformQuotaRecord) error {
	f.lastCtx = ctx
	if f.err != nil {
		return f.err
	}
...
func (f *fakeInsertRecorder) ListByUser(_ context.Context, _ int64) ([]billing.UserPlatformQuotaRecord, error) {
	return nil, nil
}

func (f *fakeInsertRecorder) UpsertForUser(_ context.Context, _ int64, _ []billing.UserPlatformQuotaRecord) error {
	return nil
}
...
func (f *fakeInsertRecorder) BatchSnapshotUsage(_ context.Context, _ []billing.UserPlatformQuotaSnapshot, _ time.Time) error {
	return nil
}

func TestSnapshotPlatformQuotaDefaults_PassesToRepoBulkInsert(t *testing.T) {
	fakeRepo := &fakeInsertRecorder{}
	s := identitytestkit.Auth(nil, &identity.AuthDependencies{Quotas: fakeRepo})

	five := 5.0
	plan := &identity.AuthSignupGrantPlan{
	...
	if err := s.AuthSnapshotPlatformQuotaDefaults(context.Background(), 999, plan); err != nil {
		t.Fatal(err)
	}
...
func TestSnapshotPlatformQuotaDefaults_DetachesCallerTransaction(t *testing.T) {
	...
	if err := s.AuthSnapshotPlatformQuotaDefaults(txCtx, 999, plan); err != nil {
...
func TestSnapshotPlatformQuotaDefaults_NilPlanIsNoop(t *testing.T) {
	fakeRepo := &fakeInsertRecorder{}
	s := identitytestkit.Auth(nil, &identity.AuthDependencies{Quotas: fakeRepo})
	if err := s.AuthSnapshotPlatformQuotaDefaults(context.Background(), 1, nil); err != nil {
...
func TestSnapshotPlatformQuotaDefaults_RepoErrorFailsOpen(t *testing.T) {
	...
	if err := s.AuthSnapshotPlatformQuotaDefaults(context.Background(), 1, plan); err != nil {
...
func TestSnapshotPlatformQuotaDefaults_NilRepoIsNoop(t *testing.T) {
	s := identitytestkit.Auth(nil, &identity.AuthDependencies{})
	five := 5.0
	plan := &identity.AuthSignupGrantPlan{
		PlatformQuotas: map[string]*identity.DefaultPlatformQuotaSetting{"a": {DailyLimitUSD: &five}},
	}
	if err := s.AuthSnapshotPlatformQuotaDefaults(context.Background(), 1, plan); err != nil {
...
func TestResolveSignupGrantPlan_GlobalQuotaLoadedBeforeAuthSource(t *testing.T) {
	...
	settings := map[string]string{
		...
		billing.SettingKeyDefaultPlatformQuotas: `{
	...
	plan := svc.AuthResolveSignupGrantPlan(context.Background(), "email")
...
func TestResolveSignupGrantPlan_DisabledAuthSourceStillCarriesGlobalQuota(t *testing.T) {
	settings := map[string]string{
		...
		billing.SettingKeyDefaultPlatformQuotas: `{"anthropic": {"daily": 10, "weekly": 50, "monthly": 200}}`,
	}
	svc := newAuthService(nil, settings, nil, nil)
	plan := svc.AuthResolveSignupGrantPlan(context.Background(), "email")
