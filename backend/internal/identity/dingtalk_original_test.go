...
func TestDecideDingTalkStep34Strategy_PolicyNone(t *testing.T) {
	step3Err := &identityprovider.DingTalkAPIError{Code: "60011", Message: "not in directory", HTTP: 403}

	shouldFallback, isFatal := identitycore.DingTalkStep34Strategy("none", step3Err)
...
func TestDecideDingTalkStep34Strategy_PolicyNoneEmpty(t *testing.T) {
	stepErr := &identityprovider.DingTalkAPIError{Code: "60011", Message: "not in directory", HTTP: 403}

	shouldFallback, isFatal := identitycore.DingTalkStep34Strategy("", stepErr)
...
func TestDecideDingTalkStep34Strategy_PolicyInternalOnly(t *testing.T) {
	...
	shouldFallback, isFatal := identitycore.DingTalkStep34Strategy("internal_only", step3Err)
...
func TestDecideDingTalkStep34Strategy_NoError(t *testing.T) {
	for _, policy := range []string{"none", "internal_only", ""} {
		shouldFallback, isFatal := identitycore.DingTalkStep34Strategy(policy, nil)
