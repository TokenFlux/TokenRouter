...
func TestTencentCaptchaVerifierMapsCredentialsRequestAndResponse(t *testing.T) {
	...
	result, err := verifier.VerifyTicket(context.Background(), identity.TencentCaptchaCredentials{
		AppID:          123456789,
	...
	}, identity.TencentCaptchaProof{Ticket: "ticket", Randstr: "@rand"}, "203.0.113.10")
	...
	require.Equal(t, &identity.TencentCaptchaVerifyResponse{
		CaptchaCode: 1,
		CaptchaMsg:  "OK",
		RequestID:   "request-id",
	}, result)
