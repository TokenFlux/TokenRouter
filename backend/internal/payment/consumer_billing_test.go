...
func TestPSValidateBillingInfo(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := payment.ValidateBillingInfo(tt.info, tt.fallbackMail)
			if tt.wantReason == "" {
				if err != nil {
...
func TestPSBillingInfoSnapshotTrimsAndOmitsEmptyValues(t *testing.T) {
	...
	snapshot := payment.BillingInfoSnapshot(&payment.BillingInfo{
		Name:      "  Buyer Inc  ",
...
func TestPSBillingInfoSnapshotReturnsNilForEmptyInfo(t *testing.T) {
	...
	if got := payment.BillingInfoSnapshot(&payment.BillingInfo{
		Name:    " ",
		Address: &payment.BillingAddress{Line1: " "},
	}); got != nil {
		t.Fatalf("snapshot = %#v, want nil", got)
	}
	if got := payment.BillingInfoSnapshot(nil); got != nil {
		t.Fatalf("snapshot = %#v, want nil", got)
	}
}
