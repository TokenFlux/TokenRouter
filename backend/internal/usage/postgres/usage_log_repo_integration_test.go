...
func (s *UsageLogRepoSuite) createUsageLog(user *service.User, apiKey *service.APIKey, account *service.Account, inputTokens, outputTokens int, cost float64, createdAt time.Time) *usage.UsageLog {
	log := &usage.UsageLog{
		UserID:       user.ID,
		APIKeyID:     apiKey.ID,
