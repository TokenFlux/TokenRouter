...
// upsertCapturingQuotaRepo 实现 billing.UserPlatformQuotaRepository，捕获 UpsertForUser 调用。
type upsertCapturingQuotaRepo struct {
	billing.UserPlatformQuotaRepository
	listRecords []billing.UserPlatformQuotaRecord
	listErr     error
...
func (r *upsertCapturingQuotaRepo) ListByUser(_ context.Context, _ int64) ([]billing.UserPlatformQuotaRecord, error) {
	return r.listRecords, r.listErr
}
func (r *upsertCapturingQuotaRepo) UpsertForUser(_ context.Context, userID int64, records []billing.UserPlatformQuotaRecord) error {
...
// billingCacheStub 实现 billing.BillingCache 中本测试关心的 Delete 方法；其他方法 panic。
...
func buildTestHandler(repo billing.UserPlatformQuotaRepository, cache billing.BillingCache) *QuotaHandler {
	return newTestQuotaHandler(repo, cache, newStubAdminService())
}
...
func TestUpdateUserPlatformQuotas_Success(t *testing.T) {
	...
	// 缓存失效：按全部允许平台统一失效。
	if len(cache.deleteCalls) != len(billing.AllowedQuotaPlatforms) {
		t.Errorf("expected %d cache delete calls, got %d: %+v", len(billing.AllowedQuotaPlatforms), len(cache.deleteCalls), cache.deleteCalls)
	}
}
...
func TestResetUserPlatformQuotaWindow_NotFound(t *testing.T) {
	// handler 检查 billing.ErrUserPlatformQuotaNotFound（由 adapter 包装而来）
	repo := &upsertCapturingQuotaRepo{resetErr: billing.ErrUserPlatformQuotaNotFound}
...
func TestUpdateUserPlatformQuotas_UserNotFound(t *testing.T) {
	...
	adminSvc.getUserErr = billing.ErrUserNotFound
	h := newTestQuotaHandler(repo, cache, adminSvc)
	body := `{"quotas":[{"platform":"anthropic","daily_limit_usd":10}]}`
	c, w := putReq(t, body)
	h.UpdateUserPlatformQuotas(c)
	if w.Code != http.StatusNotFound {
