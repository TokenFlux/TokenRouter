...
func TestUserPlatformQuotaCache_SetThenGet(t *testing.T) {
	...
	in := &billing.UserPlatformQuotaCacheEntry{
		DailyUsageUSD:    1.5,
		WeeklyUsageUSD:   3.0,
	...
	if got.SchemaVersion != billing.UserPlatformQuotaCacheSchemaV1 {
		t.Errorf("SchemaVersion = %d, want %d", got.SchemaVersion, billing.UserPlatformQuotaCacheSchemaV1)
	}
...
func TestUserPlatformQuotaCache_IncrHitAccumulates(t *testing.T) {
	...
	_ = c.SetUserPlatformQuotaCache(ctx, 1, "openai", &billing.UserPlatformQuotaCacheEntry{
		Version:       1,
		SchemaVersion: billing.UserPlatformQuotaCacheSchemaV1,
	}, time.Minute)
...
func TestUserPlatformQuotaCache_Delete(t *testing.T) {
	c, _ := newMiniRedisCache(t)
	ctx := context.Background()
	_ = c.SetUserPlatformQuotaCache(ctx, 1, "openai", &billing.UserPlatformQuotaCacheEntry{Version: 1}, time.Minute)
