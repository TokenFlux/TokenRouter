...
const (
	...
	// Rate limit window durations — must match billing.RateLimitWindow* constants.
...
func (c *Cache) GetUserBalance(ctx context.Context, userID int64) (float64, error) {
...
func (c *Cache) SetUserBalance(ctx context.Context, userID int64, balance float64) error {
	key := billingBalanceKey(userID)
	return c.rdb.Set(ctx, key, balance, jitteredTTL()).Err()
}

func (c *Cache) DeductUserBalance(ctx context.Context, userID int64, amount float64) error {
	key := billingBalanceKey(userID)
...
func (c *Cache) InvalidateUserBalance(ctx context.Context, userID int64) error {
	key := billingBalanceKey(userID)
	return c.rdb.Del(ctx, key).Err()
}

func (c *Cache) GetAPIKeyRateLimit(ctx context.Context, keyID int64) (*billing.APIKeyRateLimitCacheData, error) {
	...
	data := &billing.APIKeyRateLimitCacheData{}
...
func (c *Cache) SetAPIKeyRateLimit(ctx context.Context, keyID int64, data *billing.APIKeyRateLimitCacheData) error {
	if data == nil {
		return nil
	}
	key := billingRateLimitKey(keyID)
...
func (c *Cache) UpdateAPIKeyRateLimitUsage(ctx context.Context, keyID int64, cost float64) error {
	key := billingRateLimitKey(keyID)
	now := time.Now().Unix()
...
func (c *Cache) InvalidateAPIKeyRateLimit(ctx context.Context, keyID int64) error {
	key := billingRateLimitKey(keyID)
	return c.rdb.Del(ctx, key).Err()
}
...
// *billing.UserPlatformQuotaCacheEntry。空 map（key 不存在）返回 nil。
// GetUserPlatformQuotaCache 和 BatchGetUserPlatformQuotaCache 共用此函数，确保解析逻辑一致。
func parseUserPlatformQuotaHash(m map[string]string) *billing.UserPlatformQuotaCacheEntry {
	if len(m) == 0 {
		return nil
	}
	...
	return &billing.UserPlatformQuotaCacheEntry{
...
func (c *Cache) GetUserPlatformQuotaCache(ctx context.Context, userID int64, platform string) (*billing.UserPlatformQuotaCacheEntry, bool, error) {
...
func (c *Cache) SetUserPlatformQuotaCache(ctx context.Context, userID int64, platform string, entry *billing.UserPlatformQuotaCacheEntry, ttl time.Duration) error {
	if entry == nil {
		return nil
	}
	key := userPlatformQuotaCacheKey(userID, platform)
...
func (c *Cache) DeleteUserPlatformQuotaCache(ctx context.Context, userID int64, platform string) error {
...
func (c *Cache) IncrUserPlatformQuotaUsageCache(ctx context.Context, userID int64, platform string, cost float64, ttl time.Duration, markDirty bool) error {
	member := ""
	if markDirty {
	...
	_, err := c.rdb.Eval(ctx, updateUserPlatformQuotaUsageScript,
		...
		int(ttl.Seconds()),
		billing.UserPlatformQuotaCacheSchemaV1,
		member,
...
// billing.UserPlatformQuotaKey。解析失败返回 ok=false。
func parseUserPlatformQuotaDirtyMember(m string) (billing.UserPlatformQuotaKey, bool) {
	parts := strings.SplitN(m, ":", 2)
	if len(parts) != 2 {
		return billing.UserPlatformQuotaKey{}, false
	}
	uid, err := strconv.ParseInt(parts[0], 10, 64)
	if err != nil {
		return billing.UserPlatformQuotaKey{}, false
	}
	return billing.UserPlatformQuotaKey{UserID: uid, Platform: parts[1]}, true
}

// PopDirtyUserPlatformQuotaKeys 从脏集随机弹出最多 n 个 key。
// 脏集为空时返回 (nil, nil)。
func (c *Cache) PopDirtyUserPlatformQuotaKeys(ctx context.Context, n int) ([]billing.UserPlatformQuotaKey, error) {
	...
	keys := make([]billing.UserPlatformQuotaKey, 0, len(members))
...
// 空切片时直接返回 nil。
func (c *Cache) ReaddDirtyUserPlatformQuotaKeys(ctx context.Context, keys []billing.UserPlatformQuotaKey) error {
	if len(keys) == 0 {
		return nil
	}
...
func (c *Cache) BatchGetUserPlatformQuotaCache(ctx context.Context, keys []billing.UserPlatformQuotaKey) ([]*billing.UserPlatformQuotaCacheEntry, error) {
	if len(keys) == 0 {
		return nil, nil
	}
	pipe := c.rdb.Pipeline()
	...
	results := make([]*billing.UserPlatformQuotaCacheEntry, len(keys))
