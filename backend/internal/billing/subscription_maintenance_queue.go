...
func NewSubscriptionMaintenanceQueue(workerCount, queueSize int, observe func(string, ...any)) *SubscriptionMaintenanceQueue {
	if observe == nil {
		observe = func(string, ...any) {}
	}
	if workerCount <= 0 {
		...
		go func(workerID int) {
			...
			for fn := range q.queue {
				func() {
					defer func() {
						if r := recover(); r != nil {
							q.observe("SubscriptionMaintenance worker panic: %v", r)
						}
					}()
					fn()
				}()
