	...
	for _, tc := range []struct{ platform, base, alias string }{
		...
		t.Run(tc.alias, func(t *testing.T) {
			for _, hasCatalog := range []bool{true, false} {
				...
				repository := &routingtestkit.ChannelRows{Values: []routing.Channel{channel}, Platforms: map[int64]string{groupID: tc.platform}}
				...
				resolver := billingtestkit.PriceResolver(channels, newCalculator(&config.Config{}, catalog))
				...
				repository.Values = []routing.Channel{channel}
...
func TestResolveCatalogAliasesKeepChannelPlatformBoundary(t *testing.T) {
	...
	channels := routingtestkit.ChannelFromData(routingtestkit.ChannelDataFromRows([]routing.Channel{{
	...
	for _, tc := range []struct{ platform, base, alias string }{
		...
		t.Run(tc.alias, func(t *testing.T) {
			...
			for _, scope := range []string{pricing.PricingSourceGroup, pricing.PricingSourceChannel} {
				...
				repository := &routingtestkit.ChannelRows{Platforms: map[int64]string{group.ID: tc.platform}}
				channels := routing.NewChannelService(repository, nil, routing.ChannelOptions{Now: time.Now, LoadLocation: provider.LoadPricingLocation})
				resolver := billingtestkit.PriceResolver(channels, newCalculator(&config.Config{}, nil))
