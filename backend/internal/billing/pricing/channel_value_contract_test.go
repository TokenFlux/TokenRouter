...
func TestGetIntervalForContext_Empty(t *testing.T) {
	p := &pricing.ChannelModelPricing{Intervals: nil}
...
func TestChannelModelPricingClone(t *testing.T) {
	original := pricing.ChannelModelPricing{
		Models: []string{"a", "b"},
		Intervals: []pricing.PricingInterval{
			{MinTokens: 0, TierLabel: "tier1"},
		},
		TimePricing: &pricing.ChannelTimePricing{
			Timezone:     "Asia/Shanghai",
			WeekdaysOnly: true,
			Periods: []pricing.ChannelTimePricingPeriod{{
				StartTime:  "09:00",
				EndTime:    "12:00",
				Multiplier: 2,
			}},
...
func TestChannelModelPricingClone_EdgeCases(t *testing.T) {
	t.Run("nil models", func(t *testing.T) {
		original := pricing.ChannelModelPricing{Models: nil}
		cloned := original.Clone()
		require.Nil(t, cloned.Models)
	})

	t.Run("nil intervals", func(t *testing.T) {
		original := pricing.ChannelModelPricing{Intervals: nil}
		cloned := original.Clone()
		require.Nil(t, cloned.Intervals)
	})

	t.Run("empty models", func(t *testing.T) {
		original := pricing.ChannelModelPricing{Models: []string{}}
		cloned := original.Clone()
