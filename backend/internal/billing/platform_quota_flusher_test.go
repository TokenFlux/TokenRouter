...
func TestNewUserPlatformQuotaUsageFlusher_IntervalFallback(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			cfg := &flusherTestConfig{}
			cfg.Database.UserPlatformQuotaFlushIntervalMs = tc.inMs
			f := newFlusherFromTestConfig(cfg, nil, nil, nil)
...
// flusherTestConfig 保留原测试输入形状，避免让核心测试反向依赖 config。
type flusherTestConfig struct{ Database FlusherOptions }

func newFlusherFromTestConfig(c *flusherTestConfig, cache BillingCache, repo UserPlatformQuotaRepository, tw QuotaScheduler) *UserPlatformQuotaUsageFlusher {
	return NewUserPlatformQuotaUsageFlusher(c.Database, cache, repo, tw, NewQuotaCoordinator(), nil)
}
