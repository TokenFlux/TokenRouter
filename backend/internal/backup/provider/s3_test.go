...
func TestS3BackupStoreUploadFileWithProgressUsesConcurrentMultipart(t *testing.T) {
	...
	store, err := NewS3BackupStoreFactory()(context.Background(), &backup.BackupS3Config{
		Endpoint:          server.URL,
		Region:            "us-east-1",
		Bucket:            "bucket-a",
		AccessKeyID:       "ak",
	...
	uploader, ok := store.(backup.BackupObjectStoreProgressUploader)
	require.True(t, ok)
...
func TestS3BackupStoreUploadSizedUsesSinglePut(t *testing.T) {
	...
	require.NoError(t, err)
	uploader, ok := store.(backup.BackupObjectStoreSizedUploader)
	require.True(t, ok)
...
func TestS3BackupStoreUploadSpooledPutRemovesTemporaryFile(t *testing.T) {
	...
	store, err := NewS3BackupStoreFactory()(context.Background(), &backup.BackupS3Config{
		...
		SecretAccessKey: "sk",
		ForcePathStyle:  true,
		UploadMode:      backup.BackupS3UploadModeSpooledPut,
	})
	require.NoError(t, err)
...
func TestS3BackupStoreUploadSpooledPutRemovesTemporaryFileOnCancel(t *testing.T) {
	...
	store := &S3BackupStore{uploadMode: backup.BackupS3UploadModeSpooledPut}
