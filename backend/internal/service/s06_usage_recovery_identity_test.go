...
func TestUsageRecoveryCannotClearNewAdministratorState(t *testing.T) {
	for _, change := range []string{"none", "name", "credentials", "status", "error", "proxy"} {
		t.Run(change, func(t *testing.T) {
			observed := &Account{ID: 721, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusError, ErrorMessage: "token refresh failed", Credentials: map[string]any{"refresh_token": "observed"}}
			current := *observed
			switch change {
			case "name":
			...
			svc.tryClearRecoverableAccountError(context.Background(), observed)
