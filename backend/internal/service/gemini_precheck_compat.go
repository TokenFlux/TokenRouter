...
type legacyGeminiUsageReader struct{ source UsageLogRepository }
...
func LegacyGeminiUsageReader(source UsageLogRepository) account.GeminiQuotaUsageReader {
	if source == nil {
		return nil
	}
...
func (s *RateLimitService) GeminiPrecheckCore() *account.GeminiPrecheck {
	...
	s.geminiPrecheckOnce.Do(func() {
		s.geminiPrecheck = account.NewGeminiPrecheck(s.geminiQuotaService.Core(), LegacyGeminiUsageReader(s.usageRepo), account.GeminiPrecheckOptions{Now: time.Now, Location: geminiQuotaLocation(), Info: slog.Info})
	})
