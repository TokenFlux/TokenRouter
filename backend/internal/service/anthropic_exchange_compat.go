...
func (s *GatewayService) anthropicExchangeOptions(ctx context.Context, c *gin.Context, account *Account, token, tokenType, model string, stream, mimic bool, proxyURL string, tlsProfile *tlsfingerprint.Profile, replace func([]byte) error) claude.ExchangeOptions {
	o := claude.ExchangeOptions{AccountID: account.ID, AccountName: account.Name, Platform: account.Platform, Stream: stream, MaxAttempts: maxRetryAttempts, MaxElapsed: maxRetryElapsed, BudgetTokens: claude.BudgetRectifyBudgetTokens, BudgetMaxTokens: claude.BudgetRectifyMaxTokens, Context: detachStreamUpstreamContext, ReadErrorBody: s.readUpstreamErrorBody, Delay: forward.RetryDelay, SafeURL: logredact.SafeUpstreamURL, Sanitize: logredact.SanitizeUpstreamQueries, ErrorMessage: upstream.ExtractErrorMessage, IsBudgetError: claude.IsThinkingBudgetConstraintError, RectifyBudget: claude.RectifyThinkingBudget, ReplaceBody: replace}
	...
	o.DebugHeaders = account.Platform == capability.PlatformGemini && s.cfg != nil && s.cfg.Gateway.GeminiDebugResponseHeaders
	return o
}
