...
type SchedulerCache interface {
	...
	GetSnapshot(ctx context.Context, bucket scheduler.SchedulerBucket) ([]*gatewayprovider.ExecutionAccount, bool, error)
	...
	SetSnapshot(ctx context.Context, bucket scheduler.SchedulerBucket, token scheduler.SchedulerBucketWriteToken, accounts []gatewayprovider.ExecutionAccount) error
	...
	// GetAccount 获取单账号快照。
	GetAccount(ctx context.Context, accountID int64) (*gatewayprovider.ExecutionAccount, error)
	// SetAccount 写入单账号快照（包含不可调度状态）。
	SetAccount(ctx context.Context, account *gatewayprovider.ExecutionAccount) error
