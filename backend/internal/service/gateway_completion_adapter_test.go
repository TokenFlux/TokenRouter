...
func TestCompletionKeySnapshotPreservesSourceGroupAndExplicitZeroPrice(t *testing.T) {
	multiplier := 2.0
	group := &routing.Group{ID: 100, ModelPricing: []routing.ChannelModelPricing{{Models: []string{"claude-sonnet-4"}, FastMultiplier: &multiplier}}}
	...
	zero := 0.0
	group.ModelPricing = []routing.ChannelModelPricing{{Models: []string{"claude-sonnet-4"}, InputPrice: &zero}}
