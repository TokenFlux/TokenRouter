...
func resolveAntigravityForwardBaseURL(account *gatewayprovider.ExecutionAccount) string {
...
func (s *AntigravityGatewayService) antigravityRetryAdapter(p antigravityRetryLoopParams) (*antigravity.RetryAdapter, antigravity.RetryInput) {
	...
	return factory.Bind(accountprovider.AntigravityRetryRequest{Context: p.ctx, Account: value, ModelStore: p.accountRepo, Prefix: p.prefix, ProxyURL: p.proxyURL, AccessToken: p.accessToken, Action: p.action, Body: p.body, RequestedModel: p.requestedModel, Thinking: requeststate.HealthThinking(p.ctx), SingleAccount: isSingleAccountRetry(p.ctx), Sticky: p.isStickySession, UserAgent: p.userAgent, PolicyModelFallback: requeststate.HealthModel(p.ctx, nil),
		...
		Observe: func(o antigravity.RetryObservation) {
			gatewayhttp.AppendOpsUpstreamError(p.c, ops.OpsUpstreamErrorEvent{Platform: p.account.Record.Platform, AccountID: o.AccountID, AccountName: o.AccountName, UpstreamStatusCode: o.UpstreamStatusCode, UpstreamRequestID: o.UpstreamRequestID, UpstreamURL: o.UpstreamURL, Kind: o.Kind, Message: o.Message, Detail: o.Detail})
		},
...
func (s *AntigravityGatewayService) antigravityRetryBinding() *accountprovider.AntigravityRetry {
	...
	}, BodyLimit: s.upstreamErrorBodyReadLimit, TruncateString: logredact.TruncateUTF8, SafeURL: logredact.SafeUpstreamURL}
}
