...
func (s *OpenAIGatewayService) nativeChatBufferedFailure(c *gin.Context, account *Account, resp *http.Response, requestID, upstreamModel string, finalResponse *wire.ResponsesResponse, usage wire.ForwardUsage) error {
...
func (s *OpenAIGatewayService) nativeChatStreamFailure(c *gin.Context, account *Account, resp *http.Response, requestID, upstreamModel string, payloadBytes []byte, message string, clientOutputStarted bool) openai.ChatFailure {
	policyStatus, decision := s.applyOpenAIStreamFailedAccountPolicy(
		c.Request.Context(), account, upstreamModel, resp.Header, payloadBytes, message,
	)
	policyGeneric := decision.ShouldReturnGenericError()
	if !clientOutputStarted && decision.ShouldFailover(account, policyStatus, openai.OpenAIStreamFailedEventShouldFailover(payloadBytes, message)) {
		markOpenAIWSFailureSideEffectsApplied(c, policyStatus, decision.StopScheduling)
...
func (s *OpenAIGatewayService) nativeChatResponseOptions(c *gin.Context, account *Account, resp *http.Response, originalModel, billingModel, upstreamModel string) openai.ChatResponseOptions {
