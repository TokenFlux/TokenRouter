...
func (r *crsStaleCredentialRepo) Create(_ context.Context, v *Account) error {
	v.ID = 77
	r.current = crsStaleCopy(v)
	return nil
}
func (r *crsStaleCredentialRepo) GetByCRSAccountID(context.Context, string) (*Account, error) {
	return nil, nil
}
func (r *crsStaleCredentialRepo) GetByID(context.Context, int64) (*Account, error) {
...
func crsStaleCopy(v *Account) *Account {
	if v == nil {
		return nil
	}
	out := *v
...
func (c *crsStaleOAuthClient) RefreshTokenWithClientID(context.Context, string, string, string, ...OpenAIOAuthTokenRequestOptions) (*openai.TokenResponse, error) {
	c.calls++
...
func TestCRSRefreshPreservesConcurrentAdministratorCredentials(t *testing.T) {
	...
	oauth.SetPrivacyClientFactory(func(string) (*req.Client, error) { return nil, errors.New("local fixture: no enrichment") })
	...
	result, err := NewCRSSyncService(repo, nil, nil, oauth, nil, cfg).SyncFromCRS(context.Background(), SyncFromCRSInput{BaseURL: server.URL, Username: "local", Password: "local"})
