			...
			t.Run(fmt.Sprintf("advanced=%v/enabled=%v", advanced, enabled), func(t *testing.T) {
				...
				svc := withOpenAIExecutionCredentialsForTest(withSchedulerParametersForTest(&OpenAIGatewayService{
					...
					schedulerSnapshot: NewSchedulerSnapshotService(&openAISnapshotCacheStub{
						snapshotAccounts: []*gatewayprovider.ExecutionAccount{cached}, accountsByID: map[int64]*gatewayprovider.ExecutionAccount{cached.Record.ID: cached}}, nil, nil, nil, nil),
				}))
