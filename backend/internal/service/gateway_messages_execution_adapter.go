...
func newMessageExecutionAdapter(s *GatewayService, c *gin.Context, account *gatewayprovider.ExecutionAccount) *messageExecutionAdapter {
...
func (a *messageExecutionAdapter) DebugOriginal(body []byte, model string, stream bool) {
	a.s.debugLogGatewaySnapshot("CLIENT_ORIGINAL", a.c.Request.Header, body, map[string]string{"account": fmt.Sprintf("%d(%s)", a.account.Record.ID, a.account.Record.Name), "account_type": a.account.Record.Type, "model": model, "stream": strconv.FormatBool(stream)})
}
...
func (a *messageExecutionAdapter) Credential(ctx context.Context) error {
	token, kind, e := a.s.messageCredentials.Resolve(ctx, gatewayprovider.ExecutionRecord(a.account))
...
func (a *messageExecutionAdapter) Execute(ctx context.Context, in forwardcore.MessageExecution) (upstream.AttemptResult, error) {
	...
	target := &claude.Target{
		AccountID: a.account.Record.ID, Model: in.Model, Exchange: exchange, Response: a.s.anthropicResponseOptions(ctx, a.c, a.account, in.Model, false), StartedAt: in.StartedAt, MimicClaudeCode: in.Mimic,
