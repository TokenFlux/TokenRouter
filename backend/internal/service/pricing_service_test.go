...
func TestGetModelPricing_Gpt53UsesStaticFallbackWhenRemoteMissing(t *testing.T) {
	svc := &PricingService{
		pricingData: map[string]*LiteLLMModelPricing{
	...
	for _, model := range []string{"gpt-5.3", "gpt-5.3-spark"} {
		got := svc.GetModelPricing(model)
		require.NotNil(t, got, model)
		require.InDelta(t, 2.5e-6, got.InputCostPerToken, 1e-12, model)
		require.InDelta(t, 1.5e-5, got.OutputCostPerToken, 1e-12, model)
		require.InDelta(t, 2.5e-7, got.CacheReadInputTokenCost, 1e-12, model)
