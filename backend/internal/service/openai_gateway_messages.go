...
func (s *OpenAIGatewayService) handleAnthropicStreamingResponse(
	...
	result, err := nativeopenai.ReadMessagesStreaming(resp, upstream.NewDeferredOutputContext(gatewayhttp.ResponseSink{Writer: c.Writer}), s.nativeMessagesResponseOptions(c, account, resp, originalModel, billingModel, upstreamModel), originalModel, upstreamModel, startTime)
	return chatForwardResult(result, billingModel), err
}
