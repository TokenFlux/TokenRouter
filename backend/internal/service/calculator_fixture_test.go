...
// 测试只投影配置，不保存第二份计算器状态。
func NewBillingService(cfg *config.Config, catalog *provider.PricingService) *billing.Calculator {
	return newBillingServiceWithPrices(cfg, catalog, nil)
}

func newBillingServiceWithPrices(cfg *config.Config, catalog *provider.PricingService, prices map[string]*pricing.ModelPricing) *billing.Calculator {
	multiplier := 0.0
	if cfg != nil {
