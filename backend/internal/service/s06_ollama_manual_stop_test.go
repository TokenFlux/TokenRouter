...
func TestOllamaUsageStopOwnsManualRefresh(t *testing.T) {
	...
	value.Extra[OllamaCloudUsageSessionExtraKey] = "cipher:wos-session=fixture"
	repo := &ollamaUsageTestRepo{accountServiceTestRepo: &accountServiceTestRepo{accounts: map[int64]*Account{value.ID: value}}}
	...
	require.NotContains(t, value.Extra, OllamaCloudUsageSnapshotExtraKey)
}
