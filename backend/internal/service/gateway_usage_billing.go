...
func (s *GatewayService) getUserGroupRateMultiplier(ctx context.Context, userID, groupID int64, groupDefaultMultiplier float64) float64 {
	...
	if resolver == nil {
		resolver = billing.NewGroupRateResolver(
			s.userGroupRateRepo,
...
func resolveUsageBillingRequestID(ctx context.Context, upstreamRequestID string) string {
	return completion.ResolveRequestID(completionRequestIdentity(ctx, upstreamRequestID, ""), scheduler.GenerateRequestID)
}
...
func settlementEffects(deps *billingDeps) billing.SettlementEffects {
	...
	if deps.billingCacheService != nil {
		cache = deps.billingCacheService
	}
	return billing.SettlementEffects{Cache: cache, Quotas: deps.userPlatformQuotaRepo, FlusherEnabled: deps.cfg != nil && deps.cfg.Database.UserPlatformQuotaFlusherEnabled, Background: RunBackgroundTask, Observe: logging.LegacyPrintf, BalanceWarning: func(id int64, balance float64, err error) {
...
func writeUsageLogBestEffort(ctx context.Context, repo usage.UsageLogRepository, usageLog *usage.UsageLog, logKey string) {
	completion.NewRecorder(completion.Dependencies{Logs: completion.SnapshotLogWriter(repo), Observe: completionObserver}, completion.RecorderOptions{}).WriteUsage(ctx, querycache.Clone(usageLog), logKey)
}
