...
func (a invalidJSONAdapter) Health(ctx context.Context, status int, body []byte, models []string) forwardcore.ErrorDecision {
	...
	if a.rateLimit != nil && a.account != nil {
		if len(models) > 0 {
			d = gatewayprovider.ApplyExecutionHealth(ctx, a.rateLimit.UpstreamHealth(), a.account, gatewayprovider.HealthObservationFromContext(ctx, status, a.headers, body, []string{models[0]}))
		} else {
			d = gatewayprovider.ApplyExecutionHealth(ctx, a.rateLimit.UpstreamHealth(), a.account, gatewayprovider.HealthObservationFromContext(ctx, status, a.headers, body, nil))
		}
	}
