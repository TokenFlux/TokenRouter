...
func (p *openAIRawFallbackAdapter) errorWriter() func(*gin.Context, int, string, string) {
	...
	return writeOpenAIResponsesFallbackError
}
...
func (p *openAIRawFallbackAdapter) Endpoint(v string) { SetActualOpenAIUpstreamEndpoint(p.c, v) }
...
func (p *openAIRawFallbackAdapter) RawOptions(r *http.Response, billing, model string, tier *string) native.RawResponseOptions {
...
func (p *openAIRawFallbackAdapter) AnthropicToChat(r *protocolanthropic.AnthropicRequest) (*protocolopenai.ChatCompletionsRequest, error) {
	return apicompat.AnthropicToChatCompletionsRequest(r)
}
