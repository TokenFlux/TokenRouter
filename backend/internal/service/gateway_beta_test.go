...
func TestStripBetaTokens(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := claude.StripBetaTokens(tt.header, tt.tokens)
...
func TestMergeAnthropicBetaDropping_Context1M(t *testing.T) {
	...
	got := claude.MergeAnthropicBetaDropping(required, incoming, drop)
...
func TestMergeAnthropicBetaDropping_PreservesIncomingRedactThinking(t *testing.T) {
	...
	got := claude.MergeAnthropicBetaDropping(required, incoming, claude.DroppedBetaSet())
...
func TestDroppedBetaSet(t *testing.T) {
	...
	base := claude.DroppedBetaSet()
	require.Len(t, base, len(claude.DroppedBetas))

	// With extra tokens
	extended := claude.DroppedBetaSet(claude.BetaClaudeCode)
...
func TestBuildBetaTokenSet(t *testing.T) {
	got := claude.BuildBetaTokenSet([]string{"foo", "", "bar", "foo"})
	require.Len(t, got, 2)
	require.Contains(t, got, "foo")
	require.Contains(t, got, "bar")
	require.NotContains(t, got, "")

	empty := claude.BuildBetaTokenSet(nil)
...
func TestContainsBetaToken(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := claude.ContainsBetaToken(tt.header, tt.token)
			require.Equal(t, tt.want, got)
		})
	}
}
...
func TestStripBetaTokensWithSet_EmptyDropSet(t *testing.T) {
	header := "oauth-2025-04-20,interleaved-thinking-2025-05-14"
	got := claude.StripBetaTokensWithSet(header, map[string]struct{}{})
