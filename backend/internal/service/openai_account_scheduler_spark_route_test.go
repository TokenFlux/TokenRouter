...
func TestSparkRoutingByModel(t *testing.T) {
	...
	newScheduler := func(snapshot map[int64]*gatewayprovider.ExecutionAccount) *defaultOpenAIAccountScheduler {
		return &defaultOpenAIAccountScheduler{service: withSchedulerParametersForTest(&OpenAIGatewayService{
...
func TestParentHealthSchedulerIntegration(t *testing.T) {
	...
	makeScheduler := func(parent *gatewayprovider.ExecutionAccount) *defaultOpenAIAccountScheduler {
...
func TestParentHealthSchedulerFallsBackToRepoWhenSnapshotMissesParent(t *testing.T) {
	...
	scheduler := &defaultOpenAIAccountScheduler{service: withOpenAIExecutionCredentialsForTest(withSchedulerParametersForTest(&OpenAIGatewayService{
		accountRepo: repo,
