...
func TestResolveBedrockModelRoute_RegionMatrix(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			account := &Account{Platform: capability.PlatformAnthropic, Type: capability.AccountTypeBedrock, Credentials: map[string]any{"aws_region": tc.region}}
			if tc.global {
...
func TestBedrockModelRegionRules_DefaultCatalogAndSourceConsistency(t *testing.T) {
	t.Parallel()
	for _, modelID := range bedrock.DefaultBedrockModelMapping {
		require.Contains(t, bedrock.BedrockModelRegionRules, bedrock.BedrockBaseModelID(modelID))
	}
	for baseID, rule := range bedrock.BedrockModelRegionRules {
		...
		for _, profile := range rule.GeoProfiles {
			require.Equal(t, baseID, bedrock.BedrockBaseModelID(profile.Id))
