...
func (s *tierIdentityAdmin) GetAccount(context.Context, int64) (*service.Account, error) {
	v := s.current
...
func (s *tierIdentityAdmin) UpdateAccount(_ context.Context, _ int64, input *service.UpdateAccountInput) (*service.Account, error) {
	if input.ExpectedCredentials != nil && !account.MatchesCredentialVersion(service.AccountRecordView(&s.current), *input.ExpectedCredentials) {
...
func TestTierRefreshDoesNotOverwriteNewCredentials(t *testing.T) {
	...
	admin := &tierIdentityAdmin{current: service.Account{ID: 984, Platform: service.PlatformGemini, Type: service.AccountTypeOAuth, Status: service.StatusActive, Credentials: map[string]any{"oauth_type": "google_one", "access_token": "old", "tier_id": "old"}, Extra: map[string]any{"admin_setting": "old"}}}
	gemini := service.NewGeminiOAuthService(nil, nil, nil, tierIdentityDrive{admin}, &config.Config{})
	h := NewAccountHandler(admin, nil, nil, nil, gemini, nil, nil, nil, nil, nil, nil, nil, nil, nil)
	router := gin.New()
