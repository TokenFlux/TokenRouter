...
func TestCodexImportIndexSnapshotIsolation(t *testing.T) {
	original := service.Account{ID: 7, Credentials: map[string]any{"access_token": "token", "refresh_token": "refresh", "chatgpt_account_id": "team", "nested": map[string]any{"value": "original"}}}
	index := buildCodexAccountIndex([]service.Account{original})
