...
func (p countTokensHTTPBackend) Execution(c *gin.Context, key *apikey.APIKey, parsed *requeststate.ParsedRequest, hash string, log *zap.Logger) textflow.CountPorts {
	return &countTokensAttemptBridge{h: p.h, c: c, key: apikey.CopyAPIKey(key), parsed: parsed, hash: hash, log: log}
}
...
func (b *countTokensAttemptBridge) SelectionFailed(err error, last *textflow.AttemptFailure) {
	...
	cls := classifyNoAccountErrorFromGin(b.c, b.h.gatewayService, b.key, b.parsed.Model, b.parsed.Model, capability.PlatformAnthropic)
...
func (b *countTokensAttemptBridge) Canceled() { gatewayhttp.FailoverClientGone(b.c) }
