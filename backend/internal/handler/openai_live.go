...
func liveEnabledForAPIKey(key *service.APIKey) bool {
	return key != nil && key.Group != nil && key.Group.Platform == service.PlatformOpenAI && key.Group.AllowLive
}
...
func (a legacyLiveHTTP) APIKey(c *gin.Context) (*gatewayhttp.LiveAPIKey, bool) {
	...
	result := &gatewayhttp.LiveAPIKey{ID: key.ID, UserID: key.UserID, TeamID: key.TeamID, GroupID: key.GroupID, ModelMapping: service.CloneModelMapping(key.ModelMapping)}
	if key.Group != nil {
...
func (a legacyLiveHTTP) Redirect(ctx context.Context, key *gatewayhttp.LiveAPIKey, model string) (context.Context, string) {
	...
	return apiKeyModelRedirectContext(ctx, &service.APIKey{ID: key.ID, ModelMapping: key.ModelMapping}, model)
}
func (a legacyLiveHTTP) Moderate(c *gin.Context, key *gatewayhttp.LiveAPIKey, subject gatewayhttp.LiveSubject, clientModel, model string, body []byte) bool {
	...
	decision := a.handler.checkContentModeration(c, log, actualKey, actualSubject, service.ContentModerationProtocolOpenAIResponses, model, body)
	if decision != nil && decision.Blocked {
		a.handler.errorResponse(c, contentModerationStatus(decision), contentModerationErrorCode(decision), decision.Message)
		return false
	}
	return true
}
...
func (a legacyLiveHTTP) InboundEndpoint(c *gin.Context) string { return GetInboundEndpoint(c) }
