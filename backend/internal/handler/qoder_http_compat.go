...
func (h *QoderGatewayHandler) QoderWaitObserver(c *gin.Context, stream bool, started *bool) scheduler.WaitObserver {
	return gatewayhttp.WaitObserver(c, gatewayhttp.SSEPingFormatComment, gatewayhttp.DefaultPingInterval, stream, started, true)
}
...
func (h *QoderGatewayHandler) SubmitQoderCompletion(c *gin.Context, task completion.UsageRecordTask) {
...
func ObserveQoderSelection(c *gin.Context, accountID int64, platform string) {
	gatewayhttp.SetOpsSelectedAccount(c, accountID, platform)
}
...
func QoderEndpoints(c *gin.Context, platform string) (string, string) {
	return gatewayhttp.GetInboundEndpoint(c), gatewayhttp.GetUpstreamEndpoint(c, platform)
}
...
func (h *QoderGatewayHandler) QoderClientFailure(c *gin.Context, err error) *gatewayhttp.HTTPFailure {
	...
	case gateway.FailureBilling:
		result.Status, result.Type, result.Message, result.RetryAfter = gatewayhttp.BillingErrorDetails(failure.Cause)
	...
	case gateway.FailureUserSlot, gateway.FailureAccountSlot:
		if failure.Stage == gateway.FailureAccountSlot && failure.Cause != nil && failure.Cause.Error() == "no available accounts" {
			gatewayhttp.MarkOpsRoutingCapacityLimited(c)
			result.Status = 503
		...
		result.Status, result.Type, _, result.Message = gatewayhttp.ConcurrencyErrorResponse(failure.Cause, slot)
	...
	case gateway.FailureSelection:
		gatewayhttp.MarkOpsRoutingCapacityLimitedIfNoAvailable(c, failure.Cause)
	...
	case gateway.FailureUpstream, gateway.FailureExhausted:
		...
		if ok {
			...
			result.Type = kind
			result.Message = message
			gatewayhttp.SetOpsUpstreamError(c, upstreamStatusFromError(failure.Cause), message, "")
