...
// enum.
func SanitizeToolSchemaPatterns(body []byte) ([]byte, bool, error) {
...
// SanitizeToolParameterTypes replaces an explicitly null type
...
// unchanged.
func SanitizeToolParameterTypes(body []byte) ([]byte, bool, error) {
	if len(body) == 0 {
		return body, false, nil
	}
