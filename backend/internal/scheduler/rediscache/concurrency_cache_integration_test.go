...
type ConcurrencyCacheSuite struct {
	...
	cache    scheduler.ConcurrencyCache
...
func (s *ConcurrencyCacheSuite) TestLiveLease_CountsTowardRegularAccountAndUserLimits() {
	liveCache, ok := s.cache.(scheduler.LiveConcurrencyCache)
	require.True(s.T(), ok)
...
func (s *ConcurrencyCacheSuite) TestGetAccountsLoadBatch() {
	...
	// Query batch load
	accounts := []scheduler.AccountWithConcurrency{
...
func (s *ConcurrencyCacheSuite) TestGetAccountsLoadBatch_Empty() {
	...
	loadMap, err := s.cache.GetAccountsLoadBatch(s.ctx, []scheduler.AccountWithConcurrency{})
	require.NoError(s.T(), err)
