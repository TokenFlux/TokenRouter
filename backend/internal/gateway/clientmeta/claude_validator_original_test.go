...
func TestClaudeCodeValidator_ProbeBypass(t *testing.T) {
	...
	ok := validator.Validate(originalClaudeCodeInput(req), map[string]any{
...
func TestClaudeCodeValidator_SecurityMonitorWithoutBillingBlock(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Equal(t, tt.wantAccept, validator.Validate(originalClaudeCodeInput(req), tt.body))
		})
	}
}
...
func TestClaudeCodeValidator_NonMessagesPathUAOnly(t *testing.T) {
	...
	ok := validator.Validate(originalClaudeCodeInput(req), nil)
	require.True(t, ok)
}
...
// originalClaudeCodeInput 只将原 HTTP 夹具投影为纯识别输入，不解析报文或复制规则。
func originalClaudeCodeInput(r *http.Request) clientmeta.ClaudeCodeValidationInput {
