...
func newCompactBridgeTestService() *OpenAIResponseOutput {
	...
	output.Corrector = openai.NewCodexToolCorrector()
	return output
}
...
func TestHandleNonStreamingResponse_CompactClientStreamBridgesToSSE(t *testing.T) {
	...
	require.Equal(t, "text/event-stream", rec.Header().Get("Content-Type"))
	events := httpapitestkit.ParseCompactSSE(t, rec.Body.String())
	require.Len(t, events, 2)
	require.Equal(t, "response.output_item.done", events[0][0])
...
func TestSupplementCompactionItemFromSSE_Gating(t *testing.T) {
	...
	require.Equal(t, string(finalResponse), string(responseprotocol.SupplementCompactionItemFromSSE(IsOpenAIResponsesCompactPath(c), finalResponse, bodyText)))
	...
	require.Equal(t, string(already), string(responseprotocol.SupplementCompactionItemFromSSE(IsOpenAIResponsesCompactPath(c2), already, bodyText)))

	// compact 路径 + 终态非空缺 compaction：补入到末尾。
	missing := []byte(`{"id":"r3","output":[{"type":"message"}]}`)
	patched := responseprotocol.SupplementCompactionItemFromSSE(IsOpenAIResponsesCompactPath(c2), missing, bodyText)
