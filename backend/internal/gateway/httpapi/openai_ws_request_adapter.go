...
func (p *wsRequestAdapter) ImagePolicy(ctx context.Context, body []byte) gatewayws.ImagePolicy {
	apiKey := GetExecutionAPIKey(p.request)
	...
	bridge := p.isCodex && !gatewayprovider.ImageIntent().IsOpenAIResponsesLiteWebSocketPayload(body) && allowed && explicit != accountconfig.CodexImagePolicyStrip && p.service.ImageBridge.Enabled(ctx, p.account, apiKey)
