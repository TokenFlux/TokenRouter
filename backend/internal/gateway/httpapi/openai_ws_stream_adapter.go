...
func (p *wsStreamAdapter) MappedModel(model string) string {
	return gatewayprovider.ExecutionModelPolicy(p.account).NormalizeOpenAI(accountcore.ResolveForwardMappedModel(gatewayprovider.ExecutionRecord(p.account), model, accountprovider.ModelDefaults()))
}
