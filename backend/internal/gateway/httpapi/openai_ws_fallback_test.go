...
func TestOpenAIWSRetryBackoff(t *testing.T) {
	...
	svc.options.WS.RetryBackoffMaxMS = 400
	svc.options.WS.RetryJitterRatio = 0
...
func TestOpenAIWSRetryTotalBudget(t *testing.T) {
	svc := newWSFixture(wsFixtureInputs{options: &wsFixtureOptions{}})
	svc.options.WS.RetryTotalBudgetMS = 1200
	require.Equal(t, 1200*time.Millisecond, svc.openAIWSRetryTotalBudget())

	svc.options.WS.RetryTotalBudgetMS = 0
...
func TestOpenAIWSStoreDisabledConnMode(t *testing.T) {
	svc := newWSFixture(wsFixtureInputs{options: &wsFixtureOptions{}})
	svc.options.WS.StoreDisabledForceNewConn = true
	require.Equal(t, openAIWSStoreDisabledConnModeStrict, svc.openAIWSStoreDisabledConnMode())

	svc.options.WS.StoreDisabledConnMode = "adaptive"
	require.Equal(t, openAIWSStoreDisabledConnModeAdaptive, svc.openAIWSStoreDisabledConnMode())

	svc.options.WS.StoreDisabledConnMode = ""
	svc.options.WS.StoreDisabledForceNewConn = false
...
func TestShouldLogOpenAIWSPayloadSchema(t *testing.T) {
	...
	svc.options.WS.PayloadLogSampleRate = 0
	...
	svc.options.WS.PayloadLogSampleRate = 1
