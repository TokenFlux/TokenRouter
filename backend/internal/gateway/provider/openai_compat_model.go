...
// NormalizeOpenAICompatRequestedModel 保留兼容入口的模型后缀裁定。
...
// ApplyOpenAICompatModelNormalization 在没有显式 effort 时从模型后缀派生。
...
func splitOpenAICompatReasoningModel(model string) (normalizedModel string, reasoningEffort string, ok bool) {
	...
	case "max":
		...
		if !capability.OpenAIModelSupportsReasoningEffort(modelID, last) {
			return trimmed, "", false
		}
		reasoningEffort = last
	default:
