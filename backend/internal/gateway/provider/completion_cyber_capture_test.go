...
func TestCompletionCyberInputFreezesLegacyProjection(t *testing.T) {
	...
	account := &accountcore.Record{ID: 3, Platform: capability.PlatformOpenAI, Type: capability.AccountTypeAPIKey, RateMultiplier: &rate}
	...
	input := CaptureCyber(context.Background(), CyberCapture{APIKey: key, Account: account, Subscription: sub, Model: " model ", RequestID: "request", InputTokens: 5, NativeCompactionV2: true})
