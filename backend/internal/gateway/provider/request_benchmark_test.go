...
func BenchmarkGenerateSessionHash_Metadata(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		benchmarkStringSink = session.GenerateSessionHash(parsed, slog.Info)
	}
}
...
func BenchmarkOpenAIResponses_LargeInputMeta(b *testing.B) {
	for _, size := range benchmarkBodySizes() {
		b.Run(size.name, func(b *testing.B) {
			...
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				model, stream, promptCacheKey := requeststate.OpenAIRequestMetaFromBody(body)
...
func BenchmarkOpenAIResponses_LargeInputRawPatch(b *testing.B) {
	for _, size := range benchmarkBodySizes() {
		b.Run(size.name, func(b *testing.B) {
			...
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				view := requeststate.NewOpenAIRequestView(body)
...
func BenchmarkOpenAIResponses_LargeInputImageBillingRaw(b *testing.B) {
	for _, size := range benchmarkBodySizes() {
		b.Run(size.name, func(b *testing.B) {
			...
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				cfg, err := gatewayprovider.ImageIntent().ResolveOpenAIResponsesImageBillingConfigDetailedFromBody(body, "gpt-5.4")
				if err != nil {
...
func BenchmarkExtractCacheableContent_System(b *testing.B) {
	...
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		benchmarkStringSink = session.ExtractCacheableContent(req)
	}
}
