...
func TestOpenAISelectAccountForModelWithExclusions_ChannelMappedRestrictionRejectsEarly(t *testing.T) {
	...
	svc := newCompatibleSelectionForTest(CompatibleDependencies{
		Reads: Reads{
			Accounts: selectionAccountFixture{accounts: []gatewayprovider.ExecutionAccount{{Record: accountcore.Record{LoadLocation: time.LoadLocation,
	...
	svc := newCompatibleSelectionForTest(CompatibleDependencies{
		Reads: Reads{
			Accounts: selectionAccountFixture{accounts: []gatewayprovider.ExecutionAccount{{Record: accountcore.Record{LoadLocation: time.LoadLocation,
				...
				{Record: accountcore.Record{LoadLocation: time.LoadLocation, ID: 2, Platform: capability.PlatformOpenAI, Status: billing.StatusActive,
