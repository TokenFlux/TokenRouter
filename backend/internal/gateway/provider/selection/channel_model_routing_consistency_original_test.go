...
func TestGatewayAccountLayerUsesChannelMappedModelForSupportAndRateLimit(t *testing.T) {
	...
	svc := newGenericSelectionForTest(GenericDependencies{
		...
		Reads: Reads{},
		Shared: Shared{Channels: routingtestkit.Channel(groupID, capability.PlatformAnthropic,
...
func TestAdvancedSchedulerUsesRoutingModelAndKeepsRequestedModel(t *testing.T) {
	...
	scheduler := &compatiblePicker{service: newCompatibleSelectionForTest(CompatibleDependencies{Reads: Reads{},
	...
	require.Equal(t, "channel-model", requestRoutingModel(req))
...
func TestOpenAIHTTPPassthroughIgnoresStoredAccountModelRules(t *testing.T) {
	account := gatewayprovider.ExecutionAccount{Record: accountcore.Record{LoadLocation: time.LoadLocation, ID: 76,
		Platform:    capability.PlatformOpenAI,
	...
	scheduler := &compatiblePicker{service: newCompatibleSelectionForTest(CompatibleDependencies{Reads: Reads{},
		...
		Shared: Shared{}}, nil), stats: schedulercore.NewRuntimeStats(time.Now)}
	...
	require.True(t, gatewayprovider.NewModelAvailability(gatewaytestkit.AvailabilityStore{Source: repo}, svc.channelService, false, true).DiagnoseCompatibleRouting(plainCtx, nil, "client-model", capability.PlatformOpenAI).HasModelSupport)
	require.True(t, gatewayprovider.NewModelAvailability(gatewaytestkit.AvailabilityStore{Source: repo}, svc.channelService, false, true).DiagnoseCompatibleRouting(passthroughCtx, nil, "client-model", capability.PlatformOpenAI).HasModelSupport)
}
