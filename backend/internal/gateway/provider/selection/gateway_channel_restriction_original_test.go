...
func TestIsModelSupportedByAccountWithContext_QoderUsesChannelMappedAccountLayerModel(t *testing.T) {
	...
	channelSvc := routingtestkit.ChannelWithRepository(routingtestkit.StandardChannelRepository(ch, map[int64]string{10: capability.PlatformQoder}))
	svc := newGenericSelectionForTest(GenericDependencies{Reads: Reads{}, Shared: Shared{Channels: channelSvc}}, nil)
...
func TestCheckChannelPricingRestriction_NilGroupID(t *testing.T) {
	...
	svc := newGenericSelectionForTest(GenericDependencies{
		Reads:  Reads{},
		Shared: Shared{Channels: routing.NewChannelService(nil, nil, routing.ChannelOptions{Warn: slog.Warn, Now: time.Now, LoadLocation: pricingprovider.LoadPricingLocation})},
	}, nil)
...
func TestCheckChannelPricingRestriction_ChannelMapped_Restricted(t *testing.T) {
	...
	channelSvc := routingtestkit.ChannelWithRepository(routingtestkit.StandardChannelRepository(ch, map[int64]string{10: "anthropic"}))
	svc := newGenericSelectionForTest(GenericDependencies{Reads: Reads{}, Shared: Shared{Channels: channelSvc}}, nil)
...
func TestCheckChannelPricingRestriction_NoChannel(t *testing.T) {
	...
	channelSvc := routingtestkit.ChannelWithRepository(repo)
	svc := newGenericSelectionForTest(GenericDependencies{Reads: Reads{}, Shared: Shared{Channels: channelSvc}}, nil)
