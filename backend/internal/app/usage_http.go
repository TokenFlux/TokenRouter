...
func provideUsageKeys(wrapper *service.APIKeyService) ports.KeyReader {
	keys := wrapper.APIKeyService
	return ports.KeyQueries{Lookup: func(ctx context.Context, id int64) (*ports.KeyReference, error) {
		v, e := keys.GetByID(service.KeyRequestContext(ctx), id)
		if v == nil {
			return nil, e
		}
		...
		return keys.VerifyOwnership(service.KeyRequestContext(ctx), id, ids)
	}, Search: func(ctx context.Context, id int64, q string, n int) ([]ports.KeyReference, error) {
		rows, e := keys.SearchAPIKeys(service.KeyRequestContext(ctx), id, q, n)
		if e != nil {
			return nil, e
		}
...
func provideAdminUsageHTTP(s *usage.UsageService, keys ports.KeyReader, users ports.UserReader, cleanup *usage.UsageCleanupService, ops *service.OpsService) *usageadmin.UsageHandler {
	return usageadmin.NewUsageHandler(s, keys, users, cleanup, ops)
}
func provideDashboardHTTP(s *usage.DashboardService) *usageadmin.DashboardHandler {
	return usageadmin.NewDashboardHandler(s)
}
