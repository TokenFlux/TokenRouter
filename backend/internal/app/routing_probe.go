...
func provideGroupProbeRunner(repo routing.GroupAvailabilityProbeRepository, tests *account.TestService, gateway *service.GatewayService, openai *service.OpenAIGatewayService, gemini *service.GeminiMessagesCompatService, cfg *config.Config) *routing.GroupAvailabilityProbeRunnerService {
	location := time.Local
	if cfg != nil {
	...
	executor := service.NewGroupProbeExecution(tests, gateway, openai, gemini)
