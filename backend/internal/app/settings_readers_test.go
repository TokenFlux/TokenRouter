...
func TestSettingsReadersSharePublishedState(t *testing.T) {
	...
	prompts := promptpolicy.New(store, settings.ErrSettingNotFound, nil)
	searchRuntime := search.NewConfigService(store, nil, nil, search.NewRegistry())
	readers := provideGatewayRuntimeReaders(store, nil, gatewayRuntime, account, quota, routing, moderation, prompts, searchRuntime)
	t.Cleanup(func() {
	...
	require.Same(t, prompts, readers.Prompts)
