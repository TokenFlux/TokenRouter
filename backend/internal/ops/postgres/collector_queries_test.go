...
func TestOpsMetricsCollectorQueryErrorCountsExcludesCountTokens(t *testing.T) {
	...
	collector := &MetricsQueries{db: db}
	...
	errorTotal, businessLimited, errorSLA, upstreamExcl429529, upstream429, upstream529, err := collector.QueryErrorCounts(context.Background(), start, end, ops.DefaultOpsIgnoredStatusCodes())
	require.NoError(t, err)
