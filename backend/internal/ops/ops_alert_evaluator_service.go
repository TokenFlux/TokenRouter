...
func (s *OpsAlertEvaluatorService) evaluateOnce(interval time.Duration) {
	...
	if err != nil {
		...
		reportOptions(s.cfg, "[OpsAlertEvaluator] list rules failed: %v", err)
		return
	}
	...
	for _, rule := range rules {
		...
		if err != nil {
			reportOptions(s.cfg, "[OpsAlertEvaluator] get active event failed (rule=%d): %v", rule.ID, err)
			continue
		}
		...
		if breachedNow && consecutive >= required {
			...
			if err != nil {
				reportOptions(s.cfg, "[OpsAlertEvaluator] get latest event failed (rule=%d): %v", rule.ID, err)
				continue
			}
			...
			if err != nil {
				reportOptions(s.cfg, "[OpsAlertEvaluator] create event failed (rule=%d): %v", rule.ID, err)
				continue
			}
		...
		if activeEvent != nil {
			...
			if err := s.opsRepo.UpdateAlertEventStatus(ctx, activeEvent.ID, OpsAlertStatusResolved, &resolvedAt); err != nil {
				reportOptions(s.cfg, "[OpsAlertEvaluator] resolve event failed (event=%d): %v", activeEvent.ID, err)
			} else {
				eventsResolved++
			}
		}
...
func (s *OpsAlertEvaluatorService) maybeSendAlertEmail(ctx context.Context, runtimeCfg *OpsAlertRuntimeSettings, rule *OpsAlertRule, event *OpsAlertEvent) bool {
	...
	for _, to := range emailCfg.Alert.Recipients {
		...
		if s.emailService.TemplatesEnabled {
			if err := s.emailService.SendTemplate(ctx, Notification{
				Event:          "ops.alert",
				RecipientEmail: addr,
				RecipientName:  s.emailService.RecipientName(addr),
				...
				anySent = true
				continue
			} else if !s.emailService.ShouldFallback(err) {
				continue
			}
		}
...
func (s *OpsAlertEvaluatorService) tryAcquireLeaderLock(ctx context.Context, lock OpsDistributedLockSettings) (func(), bool) {
	...
	if s.redisClient == nil {
		s.warnNoRedisOnce.Do(func() {
			reportOptions(s.cfg, "[OpsAlertEvaluator] redis not configured; running without distributed lock")
		})
		return nil, true
	}
	...
	ok, err := s.redisClient.Claim(ctx, key, s.instanceID, ttl)
	if err != nil {
		...
		s.warnNoRedisOnce.Do(func() {
			reportOptions(s.cfg, "[OpsAlertEvaluator] leader lock SetNX failed; skipping this cycle: %v", err)
		})
		return nil, false
	}
	if !ok {
	...
	return func() {
		...
		defer releaseCancel()
		_ = s.redisClient.Release(releaseCtx, key, s.instanceID)
	}, true
}
...
func (s *OpsAlertEvaluatorService) maybeLogSkip(key string) {
	...
	s.skipLogAt = now
	reportOptions(s.cfg, "[OpsAlertEvaluator] leader lock held by another instance; skipping (key=%q)", key)
}
...
type CompatOpsAlertRuleState = opsAlertRuleState

// CompatRequiredSustainedBreaches 保留旧入口兼容。
func CompatRequiredSustainedBreaches(sustainedMinutes int, interval time.Duration) int {
	return requiredSustainedBreaches(sustainedMinutes, interval)
}

// CompatParseOpsAlertRuleScope 保留旧入口兼容。
func CompatParseOpsAlertRuleScope(filters map[string]any) (platform string, groupID *int64, region *string) {
	return parseOpsAlertRuleScope(filters)
}

// CompatCompareMetric 保留旧入口兼容。
func CompatCompareMetric(value float64, operator string, threshold float64) bool {
	return compareMetric(value, operator, threshold)
}

// CompatBuildOpsAlertDimensions 保留旧入口兼容。
func CompatBuildOpsAlertDimensions(platform string, groupID *int64) map[string]any {
	return buildOpsAlertDimensions(platform, groupID)
}

// CompatBuildOpsAlertDescription 保留旧入口兼容。
func CompatBuildOpsAlertDescription(rule *OpsAlertRule, value float64, windowMinutes int, platform string, groupID *int64) string {
	return buildOpsAlertDescription(rule, value, windowMinutes, platform, groupID)
}

// CompatOpsAlertEmailVariables 保留旧入口兼容。
func CompatOpsAlertEmailVariables(rule *OpsAlertRule, event *OpsAlertEvent) map[string]string {
	return opsAlertEmailVariables(rule, event)
}

// CompatBuildOpsAlertEmailBody 保留旧入口兼容。
func CompatBuildOpsAlertEmailBody(rule *OpsAlertRule, event *OpsAlertEvent) string {
	return buildOpsAlertEmailBody(rule, event)
}

// CompatShouldSendOpsAlertEmailByMinSeverity 保留旧入口兼容。
func CompatShouldSendOpsAlertEmailByMinSeverity(minSeverity string, ruleSeverity string) bool {
	return shouldSendOpsAlertEmailByMinSeverity(minSeverity, ruleSeverity)
}

// CompatOpsEmailSeverityForOps 保留旧入口兼容。
func CompatOpsEmailSeverityForOps(severity string) string { return opsEmailSeverityForOps(severity) }

// CompatIsOpsAlertSilenced 保留旧入口兼容。
func CompatIsOpsAlertSilenced(now time.Time, rule *OpsAlertRule, event *OpsAlertEvent, silencing OpsAlertSilencingSettings) bool {
	return isOpsAlertSilenced(now, rule, event, silencing)
}

// CompatHtmlEscape 保留旧入口兼容。
func CompatHtmlEscape(s string) string { return htmlEscape(s) }

type CompatSlidingWindowLimiter = slidingWindowLimiter

// CompatNewSlidingWindowLimiter 保留旧入口兼容。
func CompatNewSlidingWindowLimiter(limit int, window time.Duration) *slidingWindowLimiter {
	return newSlidingWindowLimiter(limit, window)
}

// CompatComputeGroupAvailableRatio 保留旧入口兼容。
func CompatComputeGroupAvailableRatio(group *GroupAvailability) float64 {
	return computeGroupAvailableRatio(group)
}

// CompatCountAccountsByCondition 保留旧入口兼容。
func CompatCountAccountsByCondition(accounts map[int64]*AccountAvailability, condition func(*AccountAvailability) bool) int64 {
	return countAccountsByCondition(accounts, condition)
}
