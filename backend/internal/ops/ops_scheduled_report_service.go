...
func (s *OpsScheduledReportService) RunReport(ctx context.Context, report *opsScheduledReport, now time.Time) (int, error) {
	...
	for _, to := range recipients {
		...
		if s.emailService.TemplatesEnabled {
			locale = s.emailService.ResolveRecipientLocale(ctx, 0, addr)
...
func (s *OpsScheduledReportService) tryAcquireLeaderLock(ctx context.Context) (func(), bool) {
	...
	return func() {
		_ = s.redisClient.Release(ctx, key, s.instanceID)
	}, true
}
...
func (s *OpsScheduledReportService) setLastRunAt(ctx context.Context, reportType string, t time.Time) {
	...
	key := opsScheduledReportLastRunKeyPrefix + kind
	_ = s.redisClient.Put(ctx, key, strconv.FormatInt(t.UTC().Unix(), 10), 14*24*time.Hour)
}
...
type CompatOpsScheduledReport = opsScheduledReport

type CompatOpsScheduledReportContent = opsScheduledReportContent

// CompatOpsScheduledReportDeliverySourceID 保留旧入口兼容。
func CompatOpsScheduledReportDeliverySourceID(report *opsScheduledReport) string {
	return opsScheduledReportDeliverySourceID(report)
}

// CompatOpsScheduledReportEmailVariables 保留旧入口兼容。
func CompatOpsScheduledReportEmailVariables(report *opsScheduledReport, now time.Time) map[string]string {
	return opsScheduledReportEmailVariables(report, now)
}

// CompatOpsScheduledReportLocalizedEmailVariables 保留旧入口兼容。
func CompatOpsScheduledReportLocalizedEmailVariables(report *opsScheduledReport, now time.Time, locale string) map[string]string {
	return opsScheduledReportLocalizedEmailVariables(report, now, locale)
}

// CompatOpsScheduledReportLocalizedName 保留旧入口兼容。
func CompatOpsScheduledReportLocalizedName(report *opsScheduledReport, locale string) string {
	return opsScheduledReportLocalizedName(report, locale)
}

// CompatIsOpsSummaryReport 保留旧入口兼容。
func CompatIsOpsSummaryReport(report *opsScheduledReport) bool { return isOpsSummaryReport(report) }

// CompatOpsSummaryReportEmailVariables 保留旧入口兼容。
func CompatOpsSummaryReportEmailVariables(report *opsScheduledReport, now time.Time, overview *OpsDashboardOverview, locale string) map[string]string {
	return opsSummaryReportEmailVariables(report, now, overview, locale)
}

// CompatFormatOpsReportInteger 保留旧入口兼容。
func CompatFormatOpsReportInteger(value int64) string { return formatOpsReportInteger(value) }

// CompatFormatOpsReportMilliseconds 保留旧入口兼容。
func CompatFormatOpsReportMilliseconds(value *int) string { return formatOpsReportMilliseconds(value) }

// CompatBuildOpsSummaryEmailHTML 保留旧入口兼容。
func CompatBuildOpsSummaryEmailHTML(title string, start, end time.Time, overview *OpsDashboardOverview) string {
	return buildOpsSummaryEmailHTML(title, start, end, overview)
}

// CompatBuildOpsErrorDigestEmailHTML 保留旧入口兼容。
func CompatBuildOpsErrorDigestEmailHTML(title string, start, end time.Time, list *OpsErrorLogList) string {
	return buildOpsErrorDigestEmailHTML(title, start, end, list)
}

// CompatBuildOpsAccountHealthEmailHTML 保留旧入口兼容。
func CompatBuildOpsAccountHealthEmailHTML(title string, start, end time.Time, avail *OpsAccountAvailability) string {
	return buildOpsAccountHealthEmailHTML(title, start, end, avail)
}

// CompatNormalizeEmails 保留旧入口兼容。
func CompatNormalizeEmails(in []string) []string { return normalizeEmails(in) }
