...
func TestProtocolCatalogRoutesAreIndependent(t *testing.T) {
	for _, protocol := range ProtocolCatalog() {
		if protocol.ID == ProtocolEmbeddings {
			...
			protocol.Platforms[0] = PlatformGrok
		}
	}
	...
	require.False(t, ok)
	require.Contains(t, SupportedGroupClientProtocols(PlatformOpenAI), ProtocolEmbeddings)
	require.NotContains(t, SupportedGroupClientProtocols(PlatformGrok), ProtocolEmbeddings)
}
