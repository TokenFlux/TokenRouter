...
func (r *AccountStore) ListOllamaCloudUsageGroupAccounts(ctx context.Context, accounts []*acctcore.Record) ([]acctcore.Record, error) {
	if r == nil || r.sql == nil {
	...
	for _, account := range accounts {
		if !r.options.OllamaIdentity(account) || account.Credentials == nil {
			continue
		}
	...
	result := make([]acctcore.Record, 0, len(hydrated))
...
func (r *AccountStore) SaveOllamaCloudUsageSession(ctx context.Context, account *acctcore.Record, ciphertext string, autoRefresh bool) error {
	return r.updateOllamaCloudUsageGroup(ctx, account, map[string]any{
		acctcore.OllamaCloudUsageSessionExtraKey:     ciphertext,
		acctcore.OllamaCloudUsageAutoRefreshExtraKey: autoRefresh,
	}, false)
}

func (r *AccountStore) DeleteOllamaCloudUsageSession(ctx context.Context, account *acctcore.Record) error {
	return r.updateOllamaCloudUsageGroup(ctx, account, map[string]any{}, false)
}

func (r *AccountStore) SetOllamaCloudUsageAutoRefresh(ctx context.Context, account *acctcore.Record, enabled bool) error {
	if !ollamaCloudUsageAccountHasSession(account) {
		return acctcore.ErrOllamaCloudUsageSessionRequired
	}
	payload := ollamaCloudUsageManagedPayload(account)
	payload[acctcore.OllamaCloudUsageAutoRefreshExtraKey] = enabled
	return r.updateOllamaCloudUsageGroup(ctx, account, payload, true)
}

func (r *AccountStore) UpdateOllamaCloudUsageSnapshot(ctx context.Context, account *acctcore.Record, snapshot *acctcore.OllamaCloudUsageSnapshot) error {
	if account == nil || snapshot == nil {
		return acctcore.ErrAccountNilInput
	}
	if !ollamaCloudUsageAccountHasSession(account) {
		return acctcore.ErrOllamaCloudUsageSessionRequired
	}
	payload := ollamaCloudUsageManagedPayload(account)
	payload[acctcore.OllamaCloudUsageSnapshotExtraKey] = snapshot
	return r.updateOllamaCloudUsageGroup(ctx, account, payload, true)
}

// DisableOllamaCloudUsageAutoRefresh 按共享身份组关闭自动刷新，并保留已加载身份的 CAS 校验。
// 账号更换 API Key 后，该操作不能误关新身份组。
func (r *AccountStore) DisableOllamaCloudUsageAutoRefresh(ctx context.Context, account *acctcore.Record) error {
	if !ollamaCloudUsageAccountHasSession(account) {
		return acctcore.ErrOllamaCloudUsageSessionRequired
	}
	payload := ollamaCloudUsageManagedPayload(account)
	payload[acctcore.OllamaCloudUsageAutoRefreshExtraKey] = false
	delete(payload, acctcore.OllamaCloudUsageSnapshotExtraKey)
	return r.updateOllamaCloudUsageGroup(ctx, account, payload, true)
}

func ollamaCloudUsageManagedPayload(account *acctcore.Record) map[string]any {
	payload := make(map[string]any, 3)
	if account == nil || account.Extra == nil {
		return payload
	}
	for _, key := range []string{
		acctcore.OllamaCloudUsageSessionExtraKey,
		acctcore.OllamaCloudUsageAutoRefreshExtraKey,
		acctcore.OllamaCloudUsageSnapshotExtraKey,
	} {
...
func ollamaCloudUsageAccountHasSession(account *acctcore.Record) bool {
	if account == nil || account.Extra == nil {
		return false
	}
	value, ok := account.Extra[acctcore.OllamaCloudUsageSessionExtraKey].(string)
	return ok && value != ""
}
...
func (r *AccountStore) updateOllamaCloudUsageGroup(
	ctx context.Context,
	...
	if r == nil || r.client == nil || !r.options.OllamaIdentity(account) {
		return acctcore.ErrOllamaCloudUsageUnavailable
	}
	apiKey, ok := account.Credentials["api_key"].(string)
	if !ok || apiKey == "" {
		return acctcore.ErrOllamaCloudUsageAccountInvalid
	}
	apply := func(txCtx context.Context, client *dbent.Client) error {
		...
		if !matchesProxy {
			return acctcore.ErrOllamaCloudUsageIdentityChanged
		}
		...
		if requireExpectedState {
			expectedSession, err := canonicalAccountExtraJSON(account, acctcore.OllamaCloudUsageSessionExtraKey)
			if err != nil {
				return err
			}
			expectedAuto, err := canonicalAccountExtraJSON(account, acctcore.OllamaCloudUsageAutoRefreshExtraKey)
			if err != nil {
				return err
			}
			expectedSnapshot, err := canonicalAccountExtraJSON(account, acctcore.OllamaCloudUsageSnapshotExtraKey)
			if err != nil {
				return err
			}
			stateMatches := false
...
func lockOllamaCloudUsageGroup(
	...
	if len(members) == 0 {
		return nil, acctcore.ErrOllamaCloudUsageIdentityChanged
	}
	return members, nil
}

func canonicalAccountExtraJSON(account *acctcore.Record, key string) (string, error) {
	var value any
	if account != nil && account.Extra != nil {
...
// 防止查询候选与执行刷新之间出现竞态。
//
// 规则与 acctcore.ollamaCloudUsageAutoRefreshDueAt 保持一致，两处修改必须同步：
//   - 快照或时间缺失/无效时按首次刷新处理；
...
func (r *AccountStore) ListDueOllamaCloudUsageAccounts(
	ctx context.Context,
	now time.Time,
	...
	minFetchIntervalSeconds := acctcore.OllamaCloudUsageMinFetchInterval.Seconds()
	rows, err := r.sql.QueryContext(ctx, `
		WITH eligible AS (
			SELECT id,
	...
	byID := make(map[int64]*acctcore.Record, len(accounts))
	for _, account := range accounts {
		if account != nil {
			byID[account.ID] = account
		}
	}
	result := make([]acctcore.Record, 0, len(rowsOut))
...
func lockAndMatchAccountProxyIdentity(ctx context.Context, client *dbent.Client, account *acctcore.Record) (bool, error) {
	if account.ProxyID == nil {
		return true, nil
	}
	rows, err := client.QueryContext(ctx, `
	...
	var current egress.ProxyConnectionIdentity
	if err := rows.Scan(&current.Protocol, &current.Host, &current.Port, &current.Username, &current.Password, &current.Status); err != nil {
		return false, err
	}
	return current == egress.ProxyConnectionIdentityFromProxy(account.Proxy), rows.Err()
}
