...
func NewScheduledTestPlanRepository(db *sql.DB) account.ScheduledTestPlanRepository {
	return &scheduledTestPlanRepository{db: db}
}

func (r *scheduledTestPlanRepository) Create(ctx context.Context, plan *account.ScheduledTestPlan) (*account.ScheduledTestPlan, error) {
...
func (r *scheduledTestPlanRepository) GetByID(ctx context.Context, id int64) (*account.ScheduledTestPlan, error) {
	row := r.db.QueryRowContext(ctx, `
...
func (r *scheduledTestPlanRepository) ListByAccountID(ctx context.Context, accountID int64) ([]*account.ScheduledTestPlan, error) {
...
func (r *scheduledTestPlanRepository) ListDue(ctx context.Context, now time.Time) ([]*account.ScheduledTestPlan, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT id, account_id, model_id, cron_expression, enabled, max_results, auto_recover, last_run_at, next_run_at, created_at, updated_at
...
func (r *scheduledTestPlanRepository) Update(ctx context.Context, plan *account.ScheduledTestPlan) (*account.ScheduledTestPlan, error) {
	row := r.db.QueryRowContext(ctx, `
...
func NewScheduledTestResultRepository(db *sql.DB) account.ScheduledTestResultRepository {
	return &scheduledTestResultRepository{db: db}
}

func (r *scheduledTestResultRepository) Create(ctx context.Context, result *account.ScheduledTestResult) (*account.ScheduledTestResult, error) {
	row := r.db.QueryRowContext(ctx, `
	...
	out := &account.ScheduledTestResult{}
	if err := row.Scan(
...
func (r *scheduledTestResultRepository) ListByPlanID(ctx context.Context, planID int64, limit int) ([]*account.ScheduledTestResult, error) {
	rows, err := r.db.QueryContext(ctx, `
	...
	var results []*account.ScheduledTestResult
	for rows.Next() {
		r := &account.ScheduledTestResult{}
		if err := rows.Scan(
...
func scanScheduledPlan(row scheduledScannable) (*account.ScheduledTestPlan, error) {
	p := &account.ScheduledTestPlan{}
	if err := row.Scan(
...
func scanScheduledPlans(rows *sql.Rows) ([]*account.ScheduledTestPlan, error) {
	var plans []*account.ScheduledTestPlan
	for rows.Next() {
