...
func TestNormalizeCodexImportEntryAcceptsAgentIdentityAuthJSON(t *testing.T) {
	...
	item, err := account.NormalizeCodexImportEntry(account.CodexImportEntry{
		Index: 1,
	...
	require.Equal(t, account.OpenAIAuthModeAgentIdentity, item.Credentials["auth_mode"])
...
func TestBuildCodexAgentIdentityKeysUseChatGPTAccountOnly(t *testing.T) {
	keys := account.BuildCodexAgentIdentityKeys("team-a")
...
func TestCodexAgentIdentityIndexSeparatesTeamsForSameUser(t *testing.T) {
	existing := account.Record{
		...
		Credentials: map[string]any{
			"auth_mode":          account.OpenAIAuthModeAgentIdentity,
			"chatgpt_account_id": "team-a",
			"chatgpt_user_id":    "same-user",
			"agent_runtime_id":   "runtime-a",
		},
	}
	index := account.BuildCodexAccountIndex([]account.Record{existing})

	teamBKeys := account.BuildCodexAgentIdentityKeys("team-b")
	matched, _ := index.Find(teamBKeys, "same-user")
	require.Nil(t, matched)

	teamAKeys := account.BuildCodexAgentIdentityKeys("team-a")
...
func TestImportCodexSessionsCreatesAgentIdentityWithoutOAuthExpiry(t *testing.T) {
	...
	require.Equal(t, account.OpenAIAuthModeAgentIdentity, created.Credentials["auth_mode"])
