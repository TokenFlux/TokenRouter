...
// QoderAuthorizationUseCase 只暴露账号授权操作，不允许 HTTP 直接访问状态存储或供应商客户端。
type QoderAuthorizationUseCase interface {
	ValidateSite(string) (string, error)
	GenerateAuthURLForSite(context.Context, string, *int64) (*account.QoderAuthURLResult, error)
	ExchangeCode(context.Context, *account.QoderExchangeCodeInput) (*account.QoderTokenInfo, error)
	Poll(context.Context, string, string, *int64) (*account.QoderPollResult, error)
}
...
func (h *QoderOAuthHandler) GenerateAuthURL(c *gin.Context) {
	...
	site, err := h.qoderOAuthService.ValidateSite(req.Site)
	if err != nil {
		response.BadRequest(c, "请求无效: "+err.Error())
		return
	}
...
func (h *QoderOAuthHandler) ExchangeCode(c *gin.Context) {
	...
	tokenInfo, err := h.qoderOAuthService.ExchangeCode(c.Request.Context(), &account.QoderExchangeCodeInput{
