...
func (s *batchDeleteAdminService) GetAccountsByIDs(_ context.Context, ids []int64) ([]*accountcore.Record, error) {
	accounts := make([]*accountcore.Record, 0, len(ids))
	for _, id := range ids {
		...
		accounts = append(accounts, &accountcore.Record{ID: id})
	}
	return accounts, nil
}
...
func setupAccountBatchDeleteRouter(adminSvc *batchDeleteAdminService) *gin.Engine {
	...
	router := gin.New()
	handler := NewManagementHandler(adminSvc, ManagementOptions{Batch: accountcore.NewManagementBatch(adminSvc, nil)})
