...
type GeminiAuthorizationUseCase interface {
	GetOAuthConfig() *account.GeminiOAuthCapabilities
	GenerateAuthURL(context.Context, *int64, string, string, string, string) (*account.GeminiAuthURLResult, error)
	ExchangeCode(context.Context, *account.GeminiExchangeCodeInput) (*account.GeminiTokenInfo, error)
}
...
func (h *GeminiOAuthHandler) ExchangeCode(c *gin.Context) {
	...
	tokenInfo, err := h.geminiOAuthService.ExchangeCode(c.Request.Context(), &account.GeminiExchangeCodeInput{
