...
func TestOpenAIQuotaServiceQueryUsageUsesCodexHeaders(t *testing.T) {
	...
	svc := newQuotaForTest(codexInviteResetAdminServiceStub{account: account}, upstream, nil, nil, nil)
...
func TestOpenAIQuotaServiceUsesTLSRouterInviteResetSettings(t *testing.T) {
	...
	svc := newQuotaForTest(codexInviteResetAdminServiceStub{account: account}, upstream, nil, profileService, routerReader)
...
func TestOpenAIQuotaServiceRejectsUnsupportedAccount(t *testing.T) {
	...
	svc := newQuotaForTest(codexInviteResetAdminServiceStub{account: account}, &codexInviteResetHTTPUpstreamStub{}, nil, nil, nil)

	_, err := svc.QueryUsage(context.Background(), account.ID)
	require.Error(t, err)
	require.Equal(t, http.StatusBadRequest, s15httpx.ErrorCode(err))
	require.Equal(t, "OPENAI_QUOTA_UNSUPPORTED_ACCOUNT", apperror.Reason(err))
