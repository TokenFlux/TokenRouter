...
func TestGrokQuotaFetcherBuildUsageInfoUnknownUntilFirstSnapshot(t *testing.T) {
	...
	usage := NewGrokQuotaView().BuildUsageInfo(&accountcore.Record{Platform: capability.PlatformGrok, Type: capability.AccountTypeOAuth})
...
func TestGrokQuotaFetcherDoesNotTreatGrok45ResponsesWindowAsHeavy(t *testing.T) {
	...
	account := &accountcore.Record{
		...
		Extra: map[string]any{
			accountcore.GrokQuotaSnapshotExtraKey: &xai.QuotaSnapshot{
	...
	usage := NewGrokQuotaView().BuildUsageInfo(account)
...
func TestGrokQuotaFetcherPrefersLiveJWTTierOverStaleBillingPlan(t *testing.T) {
	...
	account := &accountcore.Record{
		...
		Extra: map[string]any{
			accountcore.GrokUsageBillingExtraKey: &xai.BillingSummary{
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			account := &accountcore.Record{
				...
				Extra: map[string]any{
					...
					accountcore.GrokQuotaSnapshotExtraKey: tt.snapshot,
				},
			}
