...
func TestNormalizeTier(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := accountcore.NormalizeAntigravityTier(tt.raw)
			require.Equal(t, tt.expected, got, "accountcore.NormalizeAntigravityTier(%q)", tt.raw)
		})
	}
}
...
func TestBuildUsageInfo_BasicModels(t *testing.T) {
	fetcher := &accountcore.AntigravityQuota{}
...
func TestFetchQuotaUsesConfiguredModelsListBodyLimit(t *testing.T) {
	...
	fetcher := &accountcore.AntigravityQuota{Options: AntigravityQuotaOptions(8, nil)}
	_, err := fetcher.FetchQuota(context.Background(), &accountcore.Record{
