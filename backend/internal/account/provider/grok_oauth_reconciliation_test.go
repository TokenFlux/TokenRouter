...
type grokReconcileRepo struct {
	...
	getByIDOverrides        map[int64]accountcore.Record
	pageOverride            *accountcore.OAuthRefreshCandidatePage
	reauthorizeOnCAS        bool
	reauthorizeOnRefreshCAS bool
	conditionalCalls        int
}

func (r *grokReconcileRepo) GetByID(_ context.Context, id int64) (*accountcore.Record, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *grokReconcileRepo) ListOAuthRefreshCandidatePage(_ context.Context, options accountcore.OAuthRefreshPageOptions) (*accountcore.OAuthRefreshCandidatePage, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.requests = append(r.requests, options)
	if r.pageOverride != nil {
		page := *r.pageOverride
		page.Accounts = append([]accountcore.Record(nil), r.pageOverride.Accounts...)
		return &page, nil
	}
	accounts := append([]accountcore.Record(nil), r.accounts...)
	sort.Slice(accounts, func(i, j int) bool { return accounts[i].ID < accounts[j].ID })
	page := make([]accountcore.Record, 0, options.Limit)
	for _, account := range accounts {
		...
		if !platformAllowed || options.ActiveOnly && account.Status != accountcore.StatusActive {
			continue
		}
		if options.IncludeSetupToken {
			if account.Type != capability.AccountTypeOAuth && account.Type != capability.AccountTypeSetupToken {
				continue
			}
		} else if account.Type != capability.AccountTypeOAuth {
			continue
		}
	...
	result := &accountcore.OAuthRefreshCandidatePage{Accounts: page, HasMore: len(page) == options.Limit}
	if len(page) > 0 {
...
func (r *grokReconcileRepo) UpdateCredentials(_ context.Context, id int64, credentials map[string]any) error {
	...
	for i := range r.accounts {
		if r.accounts[i].ID == id {
			r.accounts[i].Credentials = accountcore.MergeCredentials(r.accounts[i].Credentials, credentials)
		}
	}
	return nil
}
...
func (r *grokReconcileRepo) SetGrokOAuthErrorIfCredentialsUnchanged(
	...
	for i := range r.accounts {
		...
		if account.Platform != capability.PlatformGrok || account.Type != capability.AccountTypeOAuth || account.Status != accountcore.StatusActive ||
...
func (r *grokReconcileRepo) snapshot() ([]accountcore.OAuthRefreshPageOptions, []int64, []int64, []string) {
	r.mu.Lock()
	defer r.mu.Unlock()
	return append([]accountcore.OAuthRefreshPageOptions(nil), r.requests...), append([]int64(nil), r.setErrorIDs...), append([]int64(nil), r.updatedCredIDs...), append([]string(nil), r.setErrorMessage...)
}
...
func (b *reconcileRuntimeBlocker) BlockAccountScheduling(account *accountcore.Record, _ time.Time, _ string) {
	b.mu.Lock()
	defer b.mu.Unlock()
	if account != nil {
...
func (i *reconcileInvalidator) InvalidateToken(_ context.Context, account *accountcore.Record) error {
	i.mu.Lock()
	defer i.mu.Unlock()
...
func newGrokReconcileService(repo *grokReconcileRepo, refresher *poolHealthRefresher, invalidator accountcore.TokenCacheInvalidator, observers ...accountcore.RefreshFailureObserver) *accountcore.BackgroundRefreshService {
	...
	var observer accountcore.RefreshFailureObserver
	if len(observers) > 0 {
		observer = observers[0]
	}
	prepare := func(value *accountcore.Record) func(time.Time, string) {
		return accountcore.PrepareRefreshFailureNotice(observer, value)
	}
	attempts := backgroundAttemptOptions(repo, tuning)
	attempts.PrepareFailure = prepare
	var invalidate func(context.Context, *accountcore.Record) error
	if invalidator != nil {
		invalidate = invalidator.InvalidateToken
	}
	attempts.Invalidate = invalidate
	post := &accountcore.RefreshPostActions{Now: time.Now, Info: func(string, ...any) {}, Warn: func(string, ...any) {}, Debug: func(string, ...any) {}, Invalidate: invalidate, ClearBlock: func(int64) {}, NeedsReauth: accountcore.GrokNeedsReauth,
	...
	return accountcore.NewBackgroundRefreshService(accountcore.BackgroundRefreshOptions{
		...
		Reconciliation: accountcore.GrokReconciliationOptions{Reader: repo, ConditionalError: repo, PrepareFailure: prepare, Invalidate: invalidate, Now: time.Now, Skew: accountcore.GrokTokenRefreshSkew},
	})
}

func grokReconcileFixtures() []accountcore.Record {
	now := time.Now().UTC()
...
func TestTokenRefreshService_ReconcileGrokOAuthDefaultsToDryRunAndSanitizedPlan(t *testing.T) {
	...
	result, err := svc.ReconcileGrokOAuth(context.Background(), accountcore.GrokOAuthReconcileInput{})
...
func TestTokenRefreshService_ReconcileGrokOAuthApplyIsIdempotent(t *testing.T) {
	...
	first, err := svc.ReconcileGrokOAuth(context.Background(), accountcore.GrokOAuthReconcileInput{Apply: true, Limit: 50})
	require.NoError(t, err)
	...
	second, err := svc.ReconcileGrokOAuth(context.Background(), accountcore.GrokOAuthReconcileInput{Apply: true, Limit: 50})
	require.NoError(t, err)
...
func TestTokenRefreshService_ReconcileGrokOAuthCursorResumesWithoutDuplicates(t *testing.T) {
	...
	first, err := svc.ReconcileGrokOAuth(context.Background(), accountcore.GrokOAuthReconcileInput{Limit: 2})
	require.NoError(t, err)
	require.True(t, first.HasMore)
	require.Equal(t, int64(2), first.NextAfterID)
	require.Len(t, first.Items, 2)

	second, err := svc.ReconcileGrokOAuth(context.Background(), accountcore.GrokOAuthReconcileInput{AfterID: first.NextAfterID, Limit: 2})
	require.NoError(t, err)
...
func TestTokenRefreshService_ReconcileGrokOAuthCursorUsesRawPageAfterHydrationGap(t *testing.T) {
	...
	repo := &grokReconcileRepo{pageOverride: &accountcore.OAuthRefreshCandidatePage{
		Accounts:    []accountcore.Record{account},
		NextAfterID: account.ID + 1,
		HasMore:     true,
	}}
	svc := newGrokReconcileService(repo, &poolHealthRefresher{}, nil)

	result, err := svc.ReconcileGrokOAuth(context.Background(), accountcore.GrokOAuthReconcileInput{Limit: 2})
...
func TestTokenRefreshService_ReconcileGrokOAuthRejectsConflictingApplyMode(t *testing.T) {
	...
	_, err := svc.ReconcileGrokOAuth(context.Background(), accountcore.GrokOAuthReconcileInput{Apply: true, DryRun: true})

	require.ErrorIs(t, err, accountcore.ErrGrokOAuthReconcileMode)
}
...
func TestTokenRefreshService_ReconcileGrokOAuthSkipsStaleBlockAfterConcurrentReauthorization(t *testing.T) {
	...
	repo := &grokReconcileRepo{
		accounts:         []accountcore.Record{stale},
		getByIDOverrides: map[int64]accountcore.Record{stale.ID: latest},
	}
	svc := newGrokReconcileService(repo, &poolHealthRefresher{}, &reconcileInvalidator{})

	result, err := svc.ReconcileGrokOAuth(context.Background(), accountcore.GrokOAuthReconcileInput{Apply: true, Limit: 50})

	require.NoError(t, err)
	require.Zero(t, result.Blocked)
	require.Equal(t, 1, result.Skipped)
	require.Equal(t, accountcore.GrokOAuthReconcileOutcomeSkipped, result.Items[0].Outcome)
...
func TestTokenRefreshService_ReconcileGrokOAuthDoesNotRuntimeBlockWhenReauthorizationWinsConditionalMutation(t *testing.T) {
	...
	require.Equal(t, accountcore.StatusActive, latest.Status)
...
func TestTokenRefreshService_ReconcileGrokOAuthReportsPermanentRefreshMutationAsBlocked(t *testing.T) {
	...
	repo := &grokReconcileRepo{accounts: []accountcore.Record{account}}
	...
	require.Equal(t, accountcore.GrokOAuthReconcileActionBlock, result.Items[0].Action)
	require.Equal(t, accountcore.GrokOAuthReconcileReasonCredentialRejected, result.Items[0].Reason)
	require.Equal(t, accountcore.GrokOAuthReconcileOutcomeApplied, result.Items[0].Outcome)
	_, setErrorIDs, _, _ := repo.snapshot()
...
func TestTokenRefreshService_ReconcileGrokOAuthReportsInvalidationFailureAsPartial(t *testing.T) {
	...
	require.Zero(t, result.Failed)
	require.Equal(t, accountcore.GrokOAuthReconcileOutcomePartial, result.Items[0].Outcome)
}
