...
func TestOpenAIUsageProbeCannotWriteNewAdministratorIdentity(t *testing.T) {
	observed := &account.Record{ID: 923, Platform: capability.PlatformOpenAI, Type: capability.AccountTypeOAuth, Status: account.StatusActive, Credentials: map[string]any{"access_token": "observed"}}
	current := *observed
	...
	svc := newOAuthUsageFixture(oauthUsageFixtureOptions{accountRepo: repo, httpUpstream: upstream})
