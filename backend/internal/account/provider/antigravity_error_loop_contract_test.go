...
func newRetryParams(account *acct.Record, upstream *epFixedUpstream, handleError func(int, http.Header, []byte)) AntigravityRetryRequest {
...
func TestRetryLoop_ErrorPolicy_CustomErrorCodes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			p := newRetryParams(account, upstream, func(int, http.Header, []byte) {
				handleErrorCount++
			})
...
func TestRetryLoop_ErrorPolicy_TempUnschedulable(t *testing.T) {
	tempRulesAccount := func(rules []any) *acct.Record {
		return &acct.Record{
			ID:          200,
...
func (r *epAccountRepo) SetModelRateLimit(context.Context, int64, string, time.Time, ...string) error {
	return nil
}
