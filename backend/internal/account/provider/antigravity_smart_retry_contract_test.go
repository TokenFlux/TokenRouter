...
func TestHandleSmartRetry_NonAntigravityAccount_ContinuesDefaultLogic(t *testing.T) {
	account := &acct.Record{
		ID:       4,
		Name:     "acc-4",
		Type:     capability.AccountTypeAPIKey, // 非 Antigravity 平台账号
	...
	require.NotNil(t, result)
	require.Equal(t, antigravity.SmartRetryActionContinue, result.Action, "non-Antigravity platform account should continue default logic")
	require.Nil(t, result.Resp)
	require.Nil(t, result.Err)
	require.Nil(t, result.SwitchError)
}
...
// ---------------------------------------------------------------------------
// 以下测试覆盖本次改动：
// 1. antigravity.AntigravitySmartRetryMaxAttempts = 1（仅重试 1 次）
...
func TestHandleSmartRetry_ShortDelay_StickySession_FailedRetry_ClearsSession(t *testing.T) {
	...
	params := AntigravityRetryRequest{
		...
		ModelStore:  repo,
		Sticky:      true,
		ClearSticky: func() { _ = cache.DeleteSessionAccountID(context.Background(), 42, "sticky-hash-abc") },
		HandleError: func(int, http.Header, []byte) {
		},
	}
...
func TestHandleSmartRetry_ShortDelay_StickySession_SuccessRetry_NoDeleteSession(t *testing.T) {
	...
	params := AntigravityRetryRequest{
		...
		Sticky:      true,
		ClearSticky: func() { _ = cache.DeleteSessionAccountID(context.Background(), 42, "sticky-hash-success") },
		HandleError: func(int, http.Header, []byte) {
		},
	}
...
func TestHandleSmartRetry_LongDelay_StickySession_ClearsSession(t *testing.T) {
	...
	params := AntigravityRetryRequest{
		...
		ModelStore:  repo,
		Sticky:      true,
		ClearSticky: func() { _ = cache.DeleteSessionAccountID(context.Background(), 42, "sticky-hash-long-delay") },
		HandleError: func(int, http.Header, []byte) {
		},
	}
...
func TestHandleSmartRetry_ShortDelay_NetworkError_StickySession_ClearsSession(t *testing.T) {
	...
	params := AntigravityRetryRequest{
		...
		ModelStore:  repo,
		Sticky:      true,
		ClearSticky: func() { _ = cache.DeleteSessionAccountID(context.Background(), 99, "sticky-net-error") },
		HandleError: func(int, http.Header, []byte) {
		},
	}
...
func TestHandleSmartRetry_ShortDelay_503_StickySession_FailedRetry_ClearsSession(t *testing.T) {
	...
	params := AntigravityRetryRequest{
		...
		ModelStore:  repo,
		Sticky:      true,
		ClearSticky: func() { _ = cache.DeleteSessionAccountID(context.Background(), 77, "sticky-503-short") },
		HandleError: func(int, http.Header, []byte) {
		},
	}
...
func TestAntigravityRetryLoop_SmartRetryFailed_StickySession_SwitchErrorPropagates(t *testing.T) {
	...
	adapter, input := svc.Bind(AntigravityRetryRequest{
		...
		ModelStore:  repo,
		Sticky:      true,
		ClearSticky: func() { _ = cache.DeleteSessionAccountID(context.Background(), 55, "sticky-loop-test") },
		HandleError: func(int, http.Header, []byte) {
		},
	})
	result, err := adapter.AntigravityRetryLoop(input)
...
func newAntigravityRetryFixture() *AntigravityRetry {
	...
	return &AntigravityRetry{SafeURL: logredact.SafeUpstreamURL, TruncateString: logredact.TruncateUTF8, Health: &acct.AntigravityHealth{ModelKeys: AntigravityModelLimitKeys, Logf: noop, Info: noop, Warn: noop, Error: noop},
