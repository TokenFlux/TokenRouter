...
func newTempUnschedCacheTest(t *testing.T) (account.TempUnschedCache, *redis.Client, *miniredis.Miniredis) {
	t.Helper()
...
func TestTempUnschedCacheShorterStateDoesNotReplaceLongerState(t *testing.T) {
	...
	now := time.Now().Unix()
	longer := &account.TempUnschedState{
		UntilUnix:       now + 300,
		TriggeredAtUnix: now,
		StatusCode:      429,
	...
	shorter := &account.TempUnschedState{
		UntilUnix:       now + 120,
...
func TestTempUnschedCacheExpiredStateIsNotStored(t *testing.T) {
	cache, _, _ := newTempUnschedCacheTest(t)
	ctx := context.Background()
	expired := &account.TempUnschedState{
...
func TestTempUnschedCacheDeleteIsIdempotent(t *testing.T) {
	cache, _, _ := newTempUnschedCacheTest(t)
	ctx := context.Background()
	state := &account.TempUnschedState{
