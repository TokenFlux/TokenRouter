...
func (c *tempUnschedCache) SetTempUnsched(ctx context.Context, accountID int64, state *account.TempUnschedState) error {
...
func (c *tempUnschedCache) GetTempUnsched(ctx context.Context, accountID int64) (*account.TempUnschedState, error) {
	key := fmt.Sprintf("%s%d", tempUnschedPrefix, accountID)
	...
	var state account.TempUnschedState
...
func (c *tempUnschedCache) openAIAPIKeyHealthKey(accountID int64) string {
	// 原 hash tag 让滚动窗口与动态 sequence 键保持同一 Redis Cluster 槽。
