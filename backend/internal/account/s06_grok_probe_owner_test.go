...
func TestGrokProbeDetachedWorkHasStopOwner(t *testing.T) {
	...
	defer cancel()
	go func() {
		_, err := svc.RunProbeFlight(ctx, "local-owner", func(shared context.Context) (*account.GrokQuotaProbeResult, error) {
			close(entered)
			defer close(exited)
			select {
			case <-shared.Done():
			...
			case <-release:
				return &account.GrokQuotaProbeResult{Source: "local"}, nil
			}
		})
		done <- err
	}()
