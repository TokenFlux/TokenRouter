...
func TestAccountCompactionControlledByAdministrator(t *testing.T) {
	for _, mode := range []string{"", "force_on", "force_off"} {
		for _, probed := range []bool{false, true} {
			a := &accountcore.Record{Platform: capability.PlatformOpenAI, Extra: map[string]any{"openai_compact_mode": mode, "openai_compact_supported": probed, accountcore.OpenAINativeCompactionV2ModeExtraKey: mode, "openai_native_compaction_v2_supported": probed}}
			want := mode != "force_off"
	...
	for _, a := range []*accountcore.Record{nil, {Platform: capability.PlatformAnthropic}} {
