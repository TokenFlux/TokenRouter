...
func TestGetBaseRPM(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			a := &accountcore.RuntimeConfig{Extra: tt.extra}
...
func TestCheckRPMSchedulability(t *testing.T) {
	tests := []struct {
		...
		extra      map[string]any
		currentRPM int
		expected   accountcore.WindowCostSchedulability
	}{
		{"disabled", map[string]any{}, 100, accountcore.WindowCostSchedulable},
		{"green zone", map[string]any{"base_rpm": 15}, 10, accountcore.WindowCostSchedulable},
		{"yellow zone tiered", map[string]any{"base_rpm": 15}, 15, accountcore.WindowCostStickyOnly},
		{"red zone tiered", map[string]any{"base_rpm": 15}, 18, accountcore.WindowCostNotSchedulable},
		{"sticky_exempt at limit", map[string]any{"base_rpm": 15, "rpm_strategy": "sticky_exempt"}, 15, accountcore.WindowCostStickyOnly},
		{"sticky_exempt over limit", map[string]any{"base_rpm": 15, "rpm_strategy": "sticky_exempt"}, 100, accountcore.WindowCostStickyOnly},
		{"custom buffer", map[string]any{"base_rpm": 10, "rpm_sticky_buffer": 5}, 14, accountcore.WindowCostStickyOnly},
		{"custom buffer red", map[string]any{"base_rpm": 10, "rpm_sticky_buffer": 5}, 15, accountcore.WindowCostNotSchedulable},
		{"base_rpm=1 green", map[string]any{"base_rpm": 1}, 0, accountcore.WindowCostSchedulable},
		{"base_rpm=1 yellow (at limit)", map[string]any{"base_rpm": 1}, 1, accountcore.WindowCostStickyOnly},
		{"base_rpm=1 red (at limit+buffer)", map[string]any{"base_rpm": 1}, 2, accountcore.WindowCostNotSchedulable},
		{"negative currentRPM", map[string]any{"base_rpm": 15}, -1, accountcore.WindowCostSchedulable},
		{"base_rpm negative disabled", map[string]any{"base_rpm": -5}, 10, accountcore.WindowCostSchedulable},
		{"very high currentRPM", map[string]any{"base_rpm": 10}, 9999, accountcore.WindowCostNotSchedulable},
		{"sticky_exempt very high currentRPM", map[string]any{"base_rpm": 10, "rpm_strategy": "sticky_exempt"}, 9999, accountcore.WindowCostStickyOnly},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			a := &accountcore.RuntimeConfig{Extra: tt.extra}
...
func TestGetRPMStickyBuffer(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			a := &accountcore.RuntimeConfig{Concurrency: tt.concurrency, Extra: tt.extra}
