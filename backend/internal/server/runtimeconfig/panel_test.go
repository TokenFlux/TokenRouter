...
func (r *panelRateLimitSettingRepo) Get(_ context.Context, key string) (*settings.Setting, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func newPanelRateLimitTestService(repo PanelSettingsRepository) *PanelSettings {
	return NewPanelSettings(repo)
}
