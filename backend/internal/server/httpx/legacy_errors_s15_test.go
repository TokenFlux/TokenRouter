...
func TestLegacyAndCategoryErrorCompatibility(t *testing.T) {
	for _, code := range []int{200, 400, 401, 403, 404, 409, 418, 429, 499, 500, 502, 503, 504, 529} {
		t.Run(fmt.Sprint(code), func(t *testing.T) {
			...
			oldCode, oldBody := httpx.ToHTTP(old)
			newCode, newBody := httpx.ToHTTP(wrapped)
			if oldCode != code || newCode != code || !reflect.DeepEqual(oldBody, newBody) || httpx.ErrorCode(wrapped) != code {
	...
	if httpx.ErrorCode(nil) != 200 || apperror.Reason(nil) != "" || apperror.Message(nil) != "" {
