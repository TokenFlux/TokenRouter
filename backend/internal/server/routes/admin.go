...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	...
	{
		accounts.GET("", h.Admin.AccountManagement.List)
		accounts.GET("/ollama-cloud-usage/settings", h.Admin.AccountOllama.GetOllamaCloudUsageSettings)
		accounts.PUT("/ollama-cloud-usage/settings", h.Admin.AccountOllama.UpdateOllamaCloudUsageSettings)
		accounts.GET("/:id", h.Admin.AccountManagement.GetByID)
		accounts.POST("", h.Admin.AccountManagement.Create)
		accounts.POST("/:id/duplicate", h.Admin.AccountManagement.Duplicate)
		accounts.POST("/check-mixed-channel", h.Admin.AccountManagement.CheckMixedChannel)
		accounts.POST("/import/codex-session", h.Admin.AccountCodexImport.ImportCodexSession)
		accounts.POST("/sync/crs", h.Admin.AccountCRS.SyncFromCRS)
		accounts.POST("/sync/crs/preview", h.Admin.AccountCRS.PreviewFromCRS)
		accounts.PUT("/:id", h.Admin.AccountManagement.Update)
		accounts.GET("/:id/advanced-scheduler-score", h.Admin.SchedulerDiagnostics.GetAdvancedSchedulerScore)
		accounts.POST("/:id/advanced-scheduler-score/preview", h.Admin.SchedulerDiagnostics.PreviewAdvancedSchedulerScore)
		accounts.GET("/:id/ollama-cloud-usage", h.Admin.AccountOllama.GetOllamaCloudUsage)
		accounts.PUT("/:id/ollama-cloud-usage/session", h.Admin.AccountOllama.SaveOllamaCloudUsageSession)
		accounts.DELETE("/:id/ollama-cloud-usage/session", h.Admin.AccountOllama.DeleteOllamaCloudUsageSession)
		accounts.PUT("/:id/ollama-cloud-usage/auto-refresh", h.Admin.AccountOllama.SetOllamaCloudUsageAutoRefresh)
		accounts.POST("/:id/ollama-cloud-usage/refresh", h.Admin.AccountOllama.RefreshOllamaCloudUsage)
		accounts.DELETE("/:id", h.Admin.AccountManagement.Delete)
		accounts.POST("/:id/test", h.Admin.AccountTests.Test)
		accounts.POST("/:id/recover-state", h.Admin.AccountManagement.RecoverState)
		accounts.POST("/:id/refresh", h.Admin.AccountManagement.Refresh)
		accounts.POST("/:id/apply-oauth-credentials", h.Admin.AccountManagement.ApplyOAuthCredentials)
		accounts.POST("/:id/set-privacy", h.Admin.AccountManagement.SetPrivacy)
		accounts.POST("/:id/refresh-tier", h.Admin.AccountManagement.RefreshTier)
		accounts.GET("/:id/stats", h.Admin.AccountManagement.GetStats)
		accounts.POST("/:id/clear-error", h.Admin.AccountManagement.ClearError)
		accounts.POST("/:id/revert-proxy-fallback", h.Admin.AccountManagement.RevertProxyFallback)
		accounts.GET("/:id/usage", h.Admin.AccountOAuthUsage.GetUsage)
		accounts.GET("/:id/today-stats", h.Admin.AccountOAuthUsage.GetTodayStats)
		accounts.POST("/usage/batch", h.Admin.AccountOAuthUsage.GetBatchUsage)
		accounts.POST("/today-stats/batch", h.Admin.AccountOAuthUsage.GetBatchTodayStats)
		accounts.POST("/:id/clear-rate-limit", h.Admin.AccountManagement.ClearRateLimit)
		accounts.POST("/:id/reset-quota", h.Admin.AccountManagement.ResetQuota)
		accounts.POST("/:id/upstream-usage/query", h.Admin.UpstreamUsage.QueryUpstreamUsage)
		accounts.POST("/upstream-usage/query/batch", h.Admin.UpstreamUsage.QueryBatchUpstreamUsage)
		accounts.GET("/:id/temp-unschedulable", h.Admin.AccountManagement.GetTempUnschedulable)
		accounts.DELETE("/:id/temp-unschedulable", h.Admin.AccountManagement.ClearTempUnschedulable)
		accounts.POST("/:id/schedulable", h.Admin.AccountManagement.SetSchedulable)
		accounts.POST("/models/sync-upstream-preview", h.Admin.AccountManagement.SyncUpstreamModelsPreview)
		accounts.GET("/:id/models", h.Admin.AccountManagement.GetAvailableModels)
		accounts.POST("/:id/models/sync-upstream", h.Admin.AccountManagement.SyncUpstreamModels)
		accounts.POST("/batch", h.Admin.AccountManagement.BatchCreate)
		// 账号导出泄露上游凭证原文——要求 step-up 2FA
		accounts.GET("/data", gin.HandlerFunc(stepUpAuth), h.Admin.AccountArchive.ExportData)
		accounts.POST("/data", h.Admin.AccountArchive.ImportData)
		accounts.POST("/batch-update-credentials", h.Admin.AccountManagement.BatchUpdateCredentials)
		accounts.POST("/batch-refresh-tier", h.Admin.AccountManagement.BatchRefreshTier)
		accounts.POST("/bulk-update", h.Admin.AccountManagement.BulkUpdate)
		accounts.POST("/batch-delete", h.Admin.AccountManagement.BatchDelete)
		accounts.POST("/batch-clear-error", h.Admin.AccountManagement.BatchClearError)
		accounts.POST("/batch-refresh", h.Admin.AccountManagement.BatchRefresh)
		...
		// Antigravity 默认模型映射
		accounts.GET("/antigravity/default-model-mapping", h.Admin.AccountManagement.GetAntigravityDefaultModelMapping)
