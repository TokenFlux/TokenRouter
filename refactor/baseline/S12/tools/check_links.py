...
root=Path(__file__).resolve().parents[4];out=root/'refactor/baseline/S12';missing=[];checked=[]
...
def anchors(p):
 ...
 return found
files=[root/p for p in changed+new if p.endswith('.md') and (p.startswith('docs/')or p=='refactor/S12-promotion-payment.md'or p.startswith('refactor/baseline/S12/'))]
for p in files:
