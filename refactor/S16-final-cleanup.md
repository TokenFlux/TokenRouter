...
### OpenAI 兼容协议与共用请求能力大批次（2026-09-25）

- Messages、Chat、Raw Chat、原生 Anthropic 与 passthrough 接入 `OpenAITextExecutor`，固定请求构造、Header、TLS 和客户端策略由 `OpenAIRequests` 复用；生产消费者、测试、装配和精确门禁同步切换。完整 Responses 与 WS/Live 等剩余入口继续按后续能力批次处理。
- 兼容会话只保留原生 CompatResponses 的一份状态；Codex 快照观察复用原节流与应用后台任务。没有新建重试循环、资金实现或共享客户端。同步结果投影归原生 provider，保留公开字段和 HTTP/WS 恢复载荷边界。
- 34 个原测试完成名称、标签和断言核对，无缺失或断言减少。晚绑定传输替身、旧会话种子和空准入夹具已按原断言改接新依赖。原读取错误、取消、无关模型不阻断、敏感字段和请求 Header 契约继续验证。
...
### 图片执行、回填及账号冷却批次（2026-09-25）

- Images API Key/OAuth、响应读写和URL回填由 OpenAIImagesExecutor 接入；模型级工具冷却归 account/provider，app直接将同一执行器注入媒体运行时。旧Service图片方法与图片实例字段均删除，复用已有请求、输出、HTTP池和活动屏障。
- 101个原测试完成名称、构建标签及断言对应，无遗漏、无减少；图片计数的无消费者旧函数由生产SSE计数器承接原完成事件断言，心跳等待保留原提交断言。最后一个账号旧替身消费者迁出后，同批删除空闲替身。
...
### Live创建、sideband与观察者批次（2026-09-25）

- OpenAILiveExecutor 直接持有Live技术端口及唯一观察者取消/等待状态；app投影密钥与预算，直接登记原OpenAILiveObservers关闭顺序。创建、控制权、租约和关闭继续复用gateway/live，停止不提前结束远端会话。旧Live结果实体、usage依赖和相关纯投影已删除。
- Live与WS逐轮模型资格复核归原生Compatible选择器，HTTP使用消费者侧窄接口。零费用用量直接复用已有完成记录器，保持尽力写入与同步兜底。23个原测试按名称、标签及断言核对，无遗漏或减少；观察重试间隔改为测试实例选项。
- 普通3,519条、unit5,146条通过，各2项既有跳过；Live/资格定向race46条、最终相关消费者44条通过。全仓unit测试编译通过，只计可编译证据。补齐了上一Responses批次遗漏的middleware测试调用，原行为断言保持。
- 36/36门禁、8个构建集合、三套受影响lint0/0/0、Wire两次稳定和生产构建通过。[验证摘要](baseline/S16/service-openai-live-owner-verification.json)单列初次编译/lint诊断、迁移回归与外部环境限制。
- 旧service生产47→40、测试172→169；SQL、Ent、早期冻结资料、原计划和51个保护文件保持。剩余WS与旧聚合资源、混合测试及最终验收继续执行。

### 四个稳定能力批次合并全仓检查（2026-09-25）
...
### 唯一组合根退出旧聚合工厂（2026-09-25）

- Wire分别构造文本、Responses、WS、Images与辅助执行器，共用请求、输出、凭据协调器、连接、会话与任务实例；旧OpenAIGatewayService不再进入生产图。3个旧app工厂/投影文件删除，全部生产及生成代码的旧service import为0。
- 路由直接接收原生执行端口、RoutePlanner和会话存储。阻断、后台任务及兼容计数直接绑定已有原生拥有者；app测试夹具只保存实际组件，不重建旧服务规则或状态。
- app普通460条、unit race660条通过，无失败或跳过；74个受影响测试的名称、标签与断言数量保持。初次空依赖continuation夹具缺失原懒初始化状态已修复并完整复验。36/36门禁、8个构建集合、三套装配lint0/0/0、Wire两次稳定及生产构建通过。
- [装配验证摘要](baseline/S16/service-root-assembly-verification.json)记录生产引用扫描与剩余事项。旧service仍有24个生产文件和138个测试文件，供待迁测试使用；本批不宣称旧包删除或S16完成。SQL、Ent、早期冻结资料、原计划与保护文件保持。

### 资金定价合同与测试输入归属（2026-09-25）

- 纯账号统计匹配/计算断言进入billing/pricing，实际规则直接调用原生函数。渠道/市场展示、Key快照、完成器、任务单价及资金分配的跨模块合同进入tests/integration/pricing_contract，保留unit标签；通用用量资格断言归gateway/provider。
- 105个原测试按名称、构建条件与断言数量核对，无遗漏或减少。unit集合1,868条、最终定向race1,190条、普通相关集合617条通过，无失败或跳过；集合重叠不求和。输入夹具只装配原生实现，未修改生产算法。
- 24/24门禁、8个构建集合、三套相关lint0/0/0与全仓unit测试编译通过。新合同目录默认拒绝旧包，纯规则目录限制保持，没有扩大例外。SQL、Ent、Wire、原计划及早期冻结/保护资料未改。
- [验证摘要](baseline/S16/service-money-contracts-verification.json)记录路径映射与范围。旧service生产24个不变，测试138→117，21个旧测试/私有包装文件删除。S16最终验收仍待完成。

### 普通文本协议与转换合同完整批次（2026-09-25）

- Messages、Chat、Responses、Raw Chat的协议、读取失败、故障转移、取消与终态断言直接构造原生HTTP执行器；纯流终态及用量JSON测试归protocol/openai，Ollama账号选项合同归gateway/provider。18个原测试文件的176个测试名称、构建条件与断言数量对应，无缺失或减少。
- 定向race336条通过，无失败或跳过；受影响普通3,667、unit4,962条通过，各2项既有跳过。首轮Grok夹具遗漏HTTP目标许可已修正，原失败日志保留；未修改生产算法或扩大历史修复范围。
- 三套受影响lint0/0/0、27/27门禁和8个构建集合通过。阻塞读取与关闭观察替身由gateway/testkit共用，没有为测试构造完整旧服务。SQL、Ent、Wire、早期冻结资料、原计划与51个保护文件保持。
- [验证摘要](baseline/S16/service-http-contracts-verification.json)记录实际结果。旧service生产24个不变，测试117→97，20个旧测试/包装文件删除；尚余Grok、OAuth/身份、健康反馈及共享替身依赖，S16最终验收继续执行。

### 协议纯函数、模型投影与路径合同批次（2026-09-25）

- 9个混合测试文件按实际符号拆回protocol、upstream、apikey、gateway/provider和HTTP；85个原测试名称、标签及断言数量保持。Bedrock平台报文与账号模型映射分开，OpenAI纯JSON清理不依赖平台，账号资格与入口路径分别验证实际拥有者。
- 普通277条、unit281条、定向race281条通过，无失败或跳过；三套受影响lint0/0/0、24/24门禁、8个构建集合通过。测试拆分编译发现的同体Bedrock辅助函数已合为一份，原断言未变。
- 删除无消费者的旧模型映射函数与流常量别名，旧service生产24→23、测试97→87。[验证摘要](baseline/S16/service-native-contracts-verification.json)记录精确路径和实际结果。未改生产算法、SQL、Ent、Wire、早期冻结资料、计划原文或保护文件；本批不重复全仓检查，待后续稳定批次合并执行。S16尚未完成。

### HTTP身份、透传与首输出边界批次（2026-09-25）

- OAuth/PAT/Agent Identity、Codex身份、TLS、容量错误和首输出超时合同直接使用原生HTTP组件；106个原测试按名称、标签和断言数量核对，无缺失或减少。注册地址按测试实例注入，保留实际签名、一次恢复与凭据写回断言。
- 定向160条、最终定向race184条通过，无失败或跳过；受影响普通2584、unit3476条通过，各2项既有跳过。首轮普通构建暴露共享健康替身的unit标签遗漏，已使同一I/O替身覆盖实际普通消费者；原测试标签不变。
- 三套受影响lint0/0/0、33/33门禁与8个构建集合通过。SQL、Ent、Wire、早期冻结资料、原计划及51个保护文件保持；未扩大历史修复范围。
- [验证摘要](baseline/S16/service-http-identity-verification.json)保留实际证据。旧service生产23个不变，测试87→71，16个旧测试/夹具文件删除；OAuth旧文件只剩其他未迁测试使用的HTTP记录器；429健康测试与纯重试状态规则一并迁出，旧私有重试算法与常量转接已删除。Grok、剩余选号/定价合同和最终验收继续执行。

### Grok执行合同与共享凭据图收尾（2026-09-25）

- 119个Grok/OpenCode原测试与1个影子账号合同直接验证原生实现。纯平台报文、HTTP组合、媒体型号、请求状态及协议类型按实际符号分开；名称、标签和断言数量保持，无遗漏或减少。媒体失败仍通过公开执行入口，配额镜像替身保留原回读和写回行为。
- 定向首轮199条通过；新测试与共享夹具的直接消费者合并验证，普通479、unit912、race912条通过，无失败或跳过。未重复运行无关WS完整集合，最终全仓验收仍保留。
- 三套受影响lint0/0/0、30/30门禁、8个构建集合与全仓unit编译通过。工具流的bridge值类型许可精确到实际合同文件；同目录新文件仍拒绝，未增加忽略项。[验证摘要](baseline/S16/service-grok-final-verification.json)记录实际命令与路径。
- 旧凭据组合、Agent Identity绑定和无消费者Grok/Raw Chat包装一并删除。旧service生产23→22、测试71→52；SQL、Ent、Wire、早期冻结资料、原计划和51个保护文件保持。剩余普通网关聚合、定价/选号及查询合同继续按能力收尾，S16最终验收未完成。

### 七个稳定能力批次合并全仓检查（2026-09-25）

- 在`b5299ff18`串行完成全仓普通、unit、integration（-p=4）及三套lint；验证期间源码与HEAD保持不变。普通12,013、unit20,047、integration13,022条通过，跳过分别4/8/4，按测试名与上一次全仓记录逐项核对一致。三套lint均为0。
- 本轮合并WS、唯一组合根、资金合同及后续四个测试归属批次。[合并验证摘要](baseline/S16/service-contract-group-verification.json)记录命令、包、测试事件及既有跳过。
- 当前旧service仍有22个生产文件、52个测试文件，剩余普通网关和领域合同尚未退出旧测试图；该检查点不替代S16最终前端、跨平台、进程、资金销项和全量验收，不更新17/17。

### 账号查询、渠道价卡与用量合同批次（2026-09-25）

- CN监控、PAT、临时调度和Ollama活动合同直接使用账号记录和原生端口；渠道值/价卡、峰值规则、协议usage与完成计费按所属模块拆分。71个原测试的名称、构建条件和断言数量对应，无遗漏或减少。
- 新归属普通77、unit182、race182条通过；剩余旧service普通206、unit211条回归通过，无失败或跳过。峰值夹具由修改全局UTC改为显式设置完成快照Location，初次时区差异日志保留，原费用断言不变。
- 三套受影响lint0/0/0、30/30门禁、8个构建集合及全仓unit编译通过。[验证摘要](baseline/S16/service-domain-contracts-verification.json)保留命令、归属和边界。SQL、Ent、Wire、早期冻结资料、原计划和51个保护文件保持。
- 删除旧峰值与Ollama转接及失去消费者的夹具，旧service生产22→20、测试52→35。剩余普通网关聚合、定价/选号/会话合同继续收尾；S16最终验收尚未完成。

### 旧service与最后测试图清零（2026-09-25）

- 普通HTTP、Fast策略、定价/选号、会话隔离与app快照合同全部改接实际拥有者。123个原业务测试的名称、标签及断言数量对应，无遗漏或减少；旧TestMain随空包退出，原生HTTP测试进程继续统一初始化Gin。
- 相关普通811、unit1,224、race1,224条通过，无失败或跳过。全仓unit编译、三套完整lint0/0/0、36/36门禁及8个构建集合通过；普通/全量跨平台等最终验收仍单独执行。
- service目录与全部Go import清零，删除20个旧生产文件、35个旧测试/夹具和3个未使用资源。旧档位转接由完成器私有归一化承接，保留nil结果边界；未增加公共API。app最后的存储测试直接调用快照codec。
- 删除35条迁出文件许可及7条无匹配旧规则；原生角色门禁继续拒绝旧包、非法子包和核心反向依赖。现有架构、项目地图、接口、推广与开发文档同步实际归属，稳定锚点保持。[验证摘要](baseline/S16/service-exit-verification.json)保留执行与迁移反馈。
- SQL、Ent、Wire、S00—S15、原计划和51个保护文件保持。此处仅表示旧service清零；生成稳定性、完整测试/前端/平台/进程、资金销项与最终文档审查尚待验收，roadmap仍为16/17。

## S16 最终完成记录（2026-09-25）

- S16.0—S16.7 已完成。总计划112个旧目录逐项核对，service/repository/handler/domain/model、旧工具/platform/routes与legacybridge清零；原生模块、HTTP、存储、测试与app装配直接协作。逐批已按用户后续要求提交，没有切换分支或推送。
- 最终代码验收基线为 `30f4992eb`。普通12,013、unit20,047、integration13,022条通过；跳过4/8/4按名称与既有记录一致。三套全仓lint为0/0/0；L01固定299项在最终清单逐项保留原文件、规则与消息。
- 真实PostgreSQL资金/退款/推广/身份/团队竞争定向race：20个顶层测试、27条通过事件，无失败或跳过。此前各能力批次共享状态race及原断言映射继续保留，不扩大为全仓race或benchmark。
- 前端14个测试文件、199个测试通过，构建通过；真实前端产物下embed测试通过。普通/嵌入构建、两个维护命令及发布矩阵Linux amd64/arm64、Windows amd64、Darwin amd64/arm64全部构建通过。跨平台编译仅算可构建证据。
- standard/simple真实SIGTERM、版本注入、CLI/Web/AUTO_SETUP、监听失败与连接回收在最终integration中实际通过；Linux arm64隔离容器中管理员重启返回HTTP 200，约6.24秒内完成清理并以0退出，用量队列先于Redis/SQL关闭。两次夹具环境尝试分别为内部网络主机端口未发布、数据库TCP就绪检查不足；修正夹具后补验通过，业务代码未修改。
- 693条HTTP路由、295个设置字段和19个参与者契约通过。Ent和Wire各重新生成两次，无差异；Ent表定义与已发布SQL保持。此前执行记录的“342个SQL”应为迁移目录342个文件，其中309个.sql、32个Go文件及1个Markdown；最终按309个.sql逐文件核对原字节与SHA-256。
- 十二组资金写入的218个方法均有原生符号、文件、归属、事务与提交后边界，见[资金销项](baseline/S16/final-funding-ownership.md)。初始化与恢复保留操作级长期权限；identity/apikey/team等Adapter通过同连接billing参与能力完成跨模块资金操作，任务投影由各任务模块的PostgreSQL参与者承担。
- 111个代码文档锚点检查通过；受影响Project Doc、目录链接、构建标签和E2E命令已同步。原计划前16,353字节、S00—S15冻结资料、309个SQL、Ent表定义及51个保护文件保持不变。旧业务路径在8个构建集合中不存在，迁出许可清理后的合法/违规夹具通过。
- [最终验收摘要](baseline/S16/final-verification.json)、[112目录归属](baseline/S16/final-package-ownership.json)、[完整性校验](baseline/S16/final-integrity.json)、[L01销项](baseline/S16/final-lint-closure.json)及最终归档保留命令、退出码、测试事件和限制。

### 保留限制与回退

- 真实供应商、硬件、本地授权文件及外部TLS/E2E环境沿用既有未执行或跳过记录，不记作行为通过。平台额度协调仅限单服务进程，outbox依赖周期重建恢复，Live仍为零费用记录。未承诺邮件或外部退款恰好一次，也未增加分布式资金提交或持久任务恢复机制。
- 本阶段未升级依赖、修改SQL/schema或缓存格式。回退按已提交能力批次恢复对应代码、规则、装配与测试；不删除退款、任务或备份事实，不在活跃恢复或二进制替换期间切换实现。用户其他任务内容和SYNC.md均未提交。
