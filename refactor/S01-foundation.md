...
### 2026-09-11T02:09:56.057474+08:00 · Review 修复开始

- 基于已提交的 `a8b9d1f0b`，修复每请求重定向回调被缓存覆盖，以及 protocol 标准库 I/O 准入漏口。
- 保持连接池 key、共享 transport 和平台适配顺序；补充策略切换、清空、并发隔离与门禁夹具。原阶段证据保留，新增验证记录放在 [review-fixes](baseline/S01/review-fixes/)。

### 2026-09-11T02:19:54.240763+08:00 · Review 两项修复完成

- 每次 Do 派生客户端并设置当前重定向回调，再由 PrepareClient 适配；nil 清空、平台覆盖顺序、连接复用和失败释放均有回归验证。
- protocol 六组生产/测试规则改为精确标准库许可，其余 509 条规则和既有依赖例外未变；普通/unit/integration/wireinject/embed/Linux 的合法与拒绝夹具全部符合预期，并已清理。
- 连接池普通/unit/integration 各 46 项通过；旧平台适配分别 65 / 71 / 66 项通过；定向 unit race 87 项通过，均无跳过。新回归已记录修复前失败和修复后通过。
- 三组全量 lint 仍为 2 / 68 / 10 项原有诊断，逐文件/规则/消息对齐后无新增。验证范围与完整证据见 [review 修复报告](baseline/S01/review-fixes/verification.md)。
- Project Doc 已同步，原 S00/S01 证据保留；本次修复尚未提交或推送，其他工作区改动保留。

### 2026-09-11T02:21:26.527827+08:00 · 用户授权提交 review 修复

- 用户明确要求提交本次两项修复；提交范围包含连接池实现、回归测试、protocol 门禁、相关 Project Doc 与增量验证证据。
- 提交前源码及日志摘要与已完成验证一致，沿用本次测试和 lint 结果；其他任务改动不纳入提交，不推送。
