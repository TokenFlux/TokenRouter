...
价卡及金额算法的唯一实现位于 `billing/pricing`。旧 ModelPricingResolver 按原顺序读取分组和渠道，纯规则先判断是否需要基础价；显式分组价格不增加渠道查询，按次/媒体价卡不增加 token 基础价查询。旧 BillingService 投影用量、时刻、服务层级、最终 effort、倍率及模型策略，费用与市场展示委托同一纯算法，资金/额度/缓存仍在旧结算边界。`PricingAt` 为零继续禁用渠道分时；模型峰谷所需默认时刻单独投影，不借此启用原本未启用的分时倍率。时区加载由 provider 完成，纯计算只接受显式 Location。
...
图片和视频不再使用分组级独立单价或独立倍率。普通网关统一按分组价卡、渠道价卡、内置默认价解析；图片模式乘实际张数，视频模式乘输出总秒数，显式按次价卡乘输出数量，token 价卡按实际用量计费。创作台和批量图片仍按张预占、结算，图片/按次价卡提供单张价格，仅有 token 价卡时回退内置按张默认价，不能直接使用每 token 单价；已提交任务按原定价与资金快照完成结算。
