...
高级分组的有效参数按字段合并，而不是把一整套全局参数复制到分组：高级调度覆盖值、深复制、校验和配置合并由 `internal/scheduler/policy` 的纯叶子实现。`routing/accessview` 和旧 domain/service 类型保留别名或委托；动态设置读取、评分、反馈、抽样及调度执行仍在旧 service，未迁为新的 scheduler 运行图。
