...
本分类记录系统边界、运行时组件、依赖关系和跨模块请求流程。具体业务规则路由到领域文档，对外字段与配置路由到接口文档，部署步骤路由到运维文档。

## 文档

- [后端模块地图](backend_modules.md)：后端目录、逐包职责、静态依赖和主要模块协作图。读取时机：定位后端代码、判断包归属、修改模块接口或依赖时读取。
...
- [前端 UI 规范](frontend_ui_conventions.md)：圆角层级、间距网格、控件尺寸、开关、菜单与浮层、弹窗、z-index 层级、断点、动画时长、表格密度、图表主题和字号下限的强制约定与校验入口。读取时机：修改前端组件、样式、`tailwind.config.js` 或 `style.css` 时读取。
