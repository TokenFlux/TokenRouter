...
Messages、Claude 兼容转换和计数的固定执行依赖由 app 注入 `gateway/provider/messageforward`，生产图已删除通用 `GatewayService`。其路由计划、会话、冷却与完成处理分别直接绑定原生拥有者；调试文件由 requestdebug 共享并登记关闭。Gemini 与 Antigravity 的请求准备已进入 `gateway/provider/googleforward`，HTTP 输出由 `GeminiExecutor`、`AntigravityExecutor` 接入。账号健康、凭据与平台重试复用原生拥有者，旧两个平台 Service 已删除。Grok 的文本桥接、媒体和 Voice 已接入原生 HTTP 执行器。OpenAI 兼容的 Messages、Chat、Raw Chat、Anthropic 转换与 passthrough 由 `OpenAITextExecutor` 执行，共用请求构造由 `OpenAIRequests` 持有固定依赖。搜索、嵌入与计数入口已由 OpenAIAuxiliary 接入；图片 API Key/OAuth 分支由 OpenAIImagesExecutor 执行，模型级工具冷却归 account/provider。Responses 主请求由 OpenAIResponsesExecutor 固定组合准备、转换、HTTP执行和协议分派，失效密文状态与 WS 共享同一存储。Live 创建与观察者已由 OpenAILiveExecutor 持有，app 直接登记其关闭。WS 传输适配及连接资源已归 OpenAIWebSocketExecutor/OpenAIWSConnections。生产图已不再引用旧service，app分别构造文本、Responses、WS、Images和辅助执行器，共用原生请求、输出和资源。旧包仍保留待迁测试及其辅助入口，S16 尚未完成最终验收。
...
Grok 请求凭据的共享入口直接绑定 `gateway/provider.RequestCredentials`。账号模块持有凭据失败时的条件写入、回读确认和运行时回滚，HTTP 只保存请求预算并记录原失败事件。该实例供文本、媒体与 WS 共同使用，不再从 OpenAIGatewayService 获取凭据或互斥状态。 Grok 额度快照和上游错误状态由 `account/provider.GrokHealth` 持有，网关只投影当次模型、响应和请求级错误分类。装配复用选号的运行阻断与模型冷却，并将同一个 `account.WriteThrottle` 用于 Grok/OpenAI 快照。 共享工具改写、namespace 清理和响应 usage/终态操作直接绑定 protocol；HTTP 使用显式 ResponseTools 区分尝试与 WS turn。Compact 恢复直接绑定原生 Recovery 和 HTTP Executor，旧错误载体与工具 Context 包装已删除。 共用响应执行由 `gateway/httpapi.OpenAIResponseOutput` 组合上游读取器与固定端口；OpenAI/Grok 的健康写入由账号 provider 执行，代理断流反馈使用原 egress 熔断实例，输出层不持有选号或结算循环。推理历史通过 session 的窄缓存能力读取和写入。 GrokExecutor 复用这些响应能力与原凭据、健康和传输实例，视频归属/计费认领对象由 app 直接绑定 media。传输失败先记录 Ops，取消不更新账号状态；持久网络故障仍先写运行阻断，再用五秒独立预算保存十分钟暂停。 兼容会话的 response ID、turn state 和禁用标记归同一个 `session.CompatResponses`；Codex 额度头由账号 provider 的 `CodexUsageObserver` 使用共享节流和应用任务端口写回。
...
请求跟踪只包装 Handler，保留原 ResponseWriter 的 Flush/Hijack 能力；客户端断开后仍按原策略收集用量的 handler 必须先完成，不能直接以连接断开替代请求清理完成。现有异步额度写入、通知、探针和快照任务通过消费者侧的完成接口登记，保留其 context、并发和参数求值语义；每层关闭后等待该层派生的副作用，再关闭下层依赖。OpenAILiveObservers 仍在阶段5停止，OpenAIWSConnections 在阶段10关闭，替代原OpenAIGatewayService关闭登记。Live 观察停止只关闭本地资源，不因进程退出提前把远端会话判定为已结束。错误透传、TLS profile/router 和鉴权缓存订阅会等待最后一次回调结束。运维错误日志队列会处理完已入队批次；QPS WS 缓存的空闲定时器和按需刷新也在 HTTP 收尾后停止。WS 池关闭后禁止按需重建或重新预热，已租赁连接保留原请求收尾策略，在租约归还时释放。
