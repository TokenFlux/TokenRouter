...
1. 分组的显式单价或有效区间优先于渠道价卡。
2. 没有分组显式价卡时使用渠道价格；没有渠道价格时使用内置模型价格。
3. 分组与渠道仅填写 Fast/Flex、Max 推理或分时倍率时，保留继承的全部基础价格，只覆盖对应倍率。
4. 显式价卡的未填字段沿用通用回退规则，显式 `0` 仍表示免费。空价卡不遮蔽内置价格。

模型市场、管理界面的默认价格填充和实际结算共用上述语义。有内置价格的公开别名可以正常展示并扣费；没有任何 token 基础价的路由键仍显示未定价。图片请求与其他平台一样使用通用图片计费回退。渠道的模型白名单只取决于模型匹配，不再要求 Qoder 白名单条目必须填写价格。

账号统计仍优先使用自定义规则，再按“应用模型定价到账号统计”开关决定是否采用客户计费基数；否则与其他平台一样查询上游模型的内置价格，不因 Qoder 请求别名而禁止回退。Qoder 自定义统计规则现有的请求名、渠道映射名、上游名匹配顺序保持不变。
