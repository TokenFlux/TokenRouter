...
`account/provider` 统一投影文本与媒体端点：保留显式自定义地址，OAuth 的官方 CLI 主机在媒体请求中转为官方媒体 API。实际请求仍执行原 URL 信任检查；该选择不影响官方授权与刷新端点。
...
模型重定向、分组映射和响应模型恢复遵守共同模型链，媒体专用路由模型只用于能力选择，不能覆盖用户账单中的 requested/upstream model。视频单价统一按分组模型价卡、共享模型价卡、内置每秒默认价解析。`video` 价卡按分辨率选择每秒单价，按次价卡按输出数量计算；媒体使用普通分组、用户及订阅倍率。
