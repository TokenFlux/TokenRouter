...
管理员订单页对 `REFUND_PENDING` 和 `REFUNDING` 提供原退款查单动作。渠道已退款而本地成功记录写入失败时，应先查单恢复，避免直接再次退款。系统只在有效恢复资料支持下完成本地状态与权益处理；渠道不支持查询、结果不明或旧资料缺失时，按返回提示人工核实。数据库中的退款准备和成功记录必须保留；降级到旧版本前先处理或登记未完成退款。详见[退款事务与恢复边界](../../domains/payments_and_entitlements.md#退款)。
