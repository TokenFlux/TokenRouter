...
执行入口、管理接口和调度快照共用原生账号规则，但不共用公开数据形状。旧 Account/AccountGroup 与 repository 转接包已删除；执行目标由原生 Record 和请求路线组成，原有复制边界保留自引用关联与 map 隔离。账号与 billing 的 SQL 实例仍只在 app 构造一次，配置更新、CAS 与消费累计各由原存储负责。
