    ...
    accounts: {
      ...
      advancedSchedulerScore: {
        ...
        filterReasons: {
          ...
          group_upstream_restricted: '分组上游模型限制',
      ...
      openai: {
        ...
        textRouteMode: '上游文本协议',
        textRouteModeDesc: '由管理员选择上游支持的文本协议。',
        textProtocolsHint: '两项均选时保留客户端协议；只选一项时转换为所选协议。至少选择一项。',
        testProtocol: '测试协议',
        ...
        codexImageToolDesc:
          '统一控制 Codex /responses 文本请求的 hosted image_generation 桥接和客户端图片工具声明。hosted 工具自动注入仅适用于非 Responses Lite 请求；分组协议控制优先，其次是账号策略和全局配置，不影响独立图片生成接口。',
        codexImageToolInherit: '跟随默认设置',
        codexImageToolInheritDesc: '不写入账号覆盖；非 Lite 请求是否注入 hosted 工具由分组协议控制或全局设置决定，客户端显式携带的 hosted 工具和本地 image_gen 声明照常放行。',
        ...
        codexImageToolBadgeInherit: '默认策略',
        ...
        compactModeDesc:
          '启用此账号的旧版 /responses/compact 端点，不影响原生 V2 压缩。',
        ...
        nativeCompactV2ModeDesc:
          '启用此账号的原生 remote_compaction_v2 压缩。',
        testMode: '测试模式',
      ...
      anthropic: {
        ...
        webSearchEmulationDesc:
          '为该 API Key 账号启用 web search 模拟。客户端发送纯 web_search 请求时，由网关调用第三方搜索 API 并构造响应返回。默认跟随分组默认值配置。',
        webSearchDefault: '默认',
        webSearchEnabled: '开启',
