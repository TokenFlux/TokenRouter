...
function validatePricingEntry(entry: PricingFormEntry, t: TranslateFn): string | null {
  ...
  if (prices.some(value => value != null && value !== '' && (!Number.isFinite(Number(value)) || Number(value) < 0))) {
    return t('admin.pricing.form.invalidPrice')
  }
